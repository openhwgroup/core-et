...
module vpu_tensorfma (
  ...
  output logic                                        fma_en,
  output logic [`INST_RANGE]                          fma_inst_next,
  output logic [`VPU_LANE_NUM-1:0]                    fma_mask_next,
  input  logic [`INST_RANGE]                          fma_inst,
...
`RST_EN_FF(clock, reset, txfma_clock_valid_delay_en, txfma_clock_valid_delay,     txfma_clock_valid_delay_next,     {`TXFMA_CLOCK_EN_DELAY{1'b0}})
`RST_EN_FF(clock, reset, tima_clock_valid_delay_en,  tima_clock_valid_delay,      tima_clock_valid_delay_next,      {`TIMA_CLOCK_EN_DELAY{1'b0}})
`RST_EN_FF(clock, reset, working,                    enabled_int,                 enabled_next,                     1'b0)
`RST_EN_FF(clock, reset, working,                    fma_exec_cd,                 fma_exec_cd_next,                 3'b0)
`EN_FF    (clock,        working,                    first_pass,                  first_pass_next)
`RST_EN_FF(clock, reset, working,                    scoreboard_pend_sticky,      scoreboard_pend_sticky_next,      1'b0)
`RST_EN_FF(clock, reset, working,                    tquant_pend_sticky,          tquant_pend_sticky_next,          1'b0)
`RST_EN_FF(clock, reset, working,                    tstore_pend_sticky,          tstore_pend_sticky_next,          1'b0)
`EN_FF    (clock,        conv_bits_up,               conv_bits,                   f0_core_ctrl.tensorfma_conv_bits)
`RST_FF   (clock, reset,                             pending_vpu_rf_write,        pending_vpu_rf_write_next,        1'b0)
`RST_FF   (clock, reset,                             pending_vpu_tenc_vrf_write,  pending_vpu_tenc_vrf_write_next,  1'b0)
`FF       (clock,                                    conv_bits_pending,           conv_bits_pending_next)
`RST_EN_FF(clock, reset, enabled_flush_up,           enabled_flush,               enabled_flush_next,               {`TFMA_ENABLED_FLUSH_BITS{1'b0}})
...
always_comb begin
    ...
    // pending bit
    if (conv_bits_pending && f0_core_ctrl.tensorfma_conv_bits_ready) begin
        conv_bits_pending_next = 1'b0;
        conv_bits_up           = 1'b1;
    end
    ...
    pending_vpu_rf_write_next = enabled_next || 
        (working && (!req_int8 || f0_core_ctrl.tensorfma_ctrl.to_vrf));
    ...
    if (start) begin
        ...
        // Clear sticky bit when write done
        if ((enabled_int && tquant_pend_sticky && !tensorquant_pend) 
           || f0_core_ctrl.tensorquant_start) 
            tquant_pend_sticky_next = 1'b0;
        // Clear sticky bit when write done
        if ((enabled_int && tstore_pend_sticky && !tensorstore_pend) 
           || f0_core_ctrl.tensorstore_start) 
...
logic [`DCACHE_SET_RANGE]                     dc_read_scp_dest_set;        // Scratchpad destination set
logic [`DCACHE_WAY_RANGE]                     dc_read_scp_dest_way;        // Scratchpad destination way
...
logic [`VPU_TENC_ADDR]                        s1_tima_tenc_raddr;          // TENC read addr in S1
...
always_comb begin
    ...
    dcache_ctrl.scp_req.addr    = dc_read_addr_mod[`DCACHE_LRAM_ADDR_RANGE];
...
always_comb begin
    ...
    dc_read_req_a = (dcache_ctrl.scp_req.read_en || 
        (enabled_int && dc_read_a_skip)) && dc_read_doing_a;
...
//         CLK    RST    EN                            DOUT                DIN                      DEF
`RST_EN_FF(clock, reset, working,                      dc_read_a_req_sent, dc_read_a_req_sent_next, 'b0)
`RST_EN_FF(clock, reset, dc_read_b_req_sent_up,        dc_read_b_req_sent, dc_read_b_req_sent_next, {`VPU_TENB_SIZE/2{2'b0}})
`RST_EN_FF(clock, reset, dcache_scp_resp.fill_is_tenb, tenb_fill_pass,     ~tenb_fill_pass,         1'b0)
`EN_FF    (clock,        working,                      dc_read_a_wr_ptr,   dc_read_a_wr_ptr_next)
`EN_FF    (clock,        working,                      dc_read_b_wr_ptr,   dc_read_b_wr_ptr_next)
`EN_FF    (clock,        working,                      dc_read_scp_a_addr, dc_read_scp_a_addr_next)
`EN_FF    (clock,        working,                      dc_read_scp_b_addr, dc_read_scp_b_addr_next)
...
always_comb begin
    ...
    tenb_credit_s0 = 1'b0;
    if (s0_fma_en && (fma_exec_a_row == f0_core_ctrl.tensorfma_ctrl.rows_a)) begin
    ...
    if (f0_core_ctrl.tensorfma_ctrl.scp_a >= `DCACHE_TL_SCP_MAX_IDX)
      dc_read_scp_a_addr_next = f0_core_ctrl.tensorfma_ctrl.scp_a - `DCACHE_TL_SCP_MAX_IDX;
    if (f0_core_ctrl.tensorfma_ctrl.scp_b >= `DCACHE_TL_SCP_MAX_IDX) 
      dc_read_scp_b_addr_next = f0_core_ctrl.tensorfma_ctrl.scp_b - `DCACHE_TL_SCP_MAX_IDX;
end
...
//         CLK    RST    EN       DOUT                    DIN                          DEF
`RST_EN_FF(clock, reset, working, dc_read_a_col,          dc_read_a_col_next,          4'b0)
`RST_EN_FF(clock, reset, working, dc_read_a_row,          dc_read_a_row_next,          4'b0)
`RST_EN_FF(clock, reset, working, dc_read_a_done,         dc_read_a_done_next,         1'b1)
`RST_EN_FF(clock, reset, working, dc_read_start_a_is_odd, dc_read_start_a_is_odd_next, 1'b0)
...
always_comb begin
    ...
    if (dc_read_req_a) begin
        // Move to next row
        dc_read_a_row_next = dc_read_a_row + 4'b1;
        // If done the last row, move to next col
        if (dc_read_a_row == f0_core_ctrl.tensorfma_ctrl.rows_a) begin
...
//         CLK    RST    EN       DOUT            DIN
`RST_EN_FF(clock, reset, working, dc_read_b_col,  dc_read_b_col_next,  1'b0)
`RST_EN_FF(clock, reset, working, dc_read_b_row,  dc_read_b_row_next,  4'b0)
`RST_EN_FF(clock, reset, working, dc_read_b_done, dc_read_b_done_next, 1'b1)
...
//     CLK    EN       DOUT            DIN
`EN_FF(clock, working, load_ctrl_pre1, load_ctrl_pre0)
`EN_FF(clock, working, load_ctrl,      load_ctrl_pre1)
...
always_comb begin
    ...
    load_ctrl_pre0.broadcast_sel = dc_read_addr_flat[`DCACHE_ADDR_BCAST_RANGE];                  // Broadcast sel
...
logic [`INST_RANGE]                  fma_inst_s0;            // FMA inst computed 2 cycles earlier than execution
...
//         CLK    RST    EN                    DOUT                     DIN                                                         DEF
`RST_EN_FF(clock, reset, working,              fma_exec_b_col,          fma_exec_b_col_next,                                        1'b0)
`RST_EN_FF(clock, reset, working,              fma_exec_a_row,          fma_exec_a_row_next,                                        4'b0)
`RST_EN_FF(clock, reset, working,              fma_exec_a_col,          fma_exec_a_col_next,                                        4'b0)
`RST_EN_FF(clock, reset, working && ~req_int8, fma_en_next,             s0_fma_en_conv && ~req_int8 && (req_fp32 || req_fp16afp32), 1'b0)
`RST_EN_FF(clock, reset, working,              fma_mask_next,           s0_fma_mask,                                                '0)
`RST_EN_FF(clock, reset, working && ~req_int8, fma_en,                  fma_en_next,                                                1'b0)
`RST_EN_FF(clock, reset, working,              fma_exec_start_a_is_odd, fma_exec_start_a_is_odd_next,                               1'b0)
`EN_FF    (clock,        working,              fma_exec_a_rd_ptr,       fma_exec_a_rd_ptr_next)
`EN_FF    (clock,        working,              fma_exec_b_rd_ptr,       fma_exec_b_rd_ptr_next)
`EN_FF    (clock,        working && ~req_int8, fma_inst_next,           fma_inst_s0)
`RST_EN_FF(clock, reset, working,              done_sticky,             done_sticky_next,                                           1'b0)
`RST_EN_FF(clock, reset, working && req_int8,  fma_exec_hazard_wait,    fma_exec_hazard_wait_next,                                  1'b0)
...
always_comb begin
    ...
    if (!conv_skip) begin
      ...
      if (req_fp16afp32)
        fma_inst_s0[`VPU_INST_RM_SEL] = RTZ;
        //FP32 ops uses DYN rm
      else
        fma_inst_s0[`VPU_INST_RM_SEL] = `VPU_RM_DYN;
    end
        
    fma_inst_s0[`VPU_INST_RD_SEL]      = fma_exec_accum_reg;
    fma_inst_s0[`VPU_INST_REN1_RA_SEL] = conv_skip ? fma_exec_accum_reg : fma_exec_mat_a_reg;
    fma_inst_s0[`VPU_INST_REN2_RA_SEL] = conv_skip ? fma_exec_accum_reg : fma_exec_mat_b_reg;

    // Only FMA cases update the src3
    if (~first_pass) fma_inst_s0[`VPU_INST_REN3_RA_SEL] = fma_exec_accum_reg;
end
...
always_comb begin
    ...
    if (s0_fma_en) begin
        ...
        if (fma_exec_last_b_col) begin
            fma_exec_a_row_next = fma_exec_a_row + 4'b1;

             // If last A row, go to next A col
            if (fma_exec_a_row == f0_core_ctrl.tensorfma_ctrl.rows_a) begin
    ...
    end else if (s0_fma_en && fma_exec_last_b_col)
      fma_exec_a_rd_ptr_next = ~fma_exec_a_rd_ptr;
...
//         CLK    RST    EN                 DOUT                      DIN                DEF
`RST_EN_FF(clock, reset, working,           fma_valid_3_cycle,        fma_en,            1'b0)
`RST_EN_FF(clock, reset, working,           fma_valid_4_cycle,        fma_valid_3_cycle, 1'b0)
`RST_EN_FF(clock, reset, working,           fma_valid_5_cycle,        fma_valid_4_cycle, 1'b0)
`RST_EN_FF(clock, reset, working,           fma_valid_6_cycle,        fma_valid_5_cycle, 1'b0)
`EN_FF    (clock,        fma_en,            fma_accum_3_cycle,        fma_inst[11:7])
`EN_FF    (clock,        fma_valid_3_cycle, fma_accum_4_cycle,        fma_accum_3_cycle)
`EN_FF    (clock,        fma_valid_4_cycle, fma_accum_5_cycle,        fma_accum_4_cycle)
`EN_FF    (clock,        fma_valid_5_cycle, fma_accum_6_cycle,        fma_accum_5_cycle)
...
//     CLK    EN              DOUT                  DIN
`FF   (clock,                 tenb_credit_s1,       tenb_credit_s0)
`FF   (clock,                 tenb_credit_s2,       tenb_credit_s1)
`FF   (clock,                 tenb_credit_s3,       tenb_credit_s2)
`EN_FF(clock, tenb_credit_s0, tenb_credit_entry_s1, fma_exec_a_col_no_start[$clog2(`VPU_TENB_SIZE/2)-1:0])
`EN_FF(clock, tenb_credit_s1, tenb_credit_entry_s2, tenb_credit_entry_s1)
`EN_FF(clock, tenb_credit_s2, tenb_credit_entry_s3, tenb_credit_entry_s2)
...
logic [`VPU_TENA_ADDR] s1_tima_tena_raddr;    // TENA read pointer in S1
logic [`VPU_TENB_ADDR] s1_tima_tenb_raddr;    // TENB read pointer in S1
...
//         CLK    RST    EN           DOUT                   DIN                         DEF
`RST_EN_FF(clock, reset, tima_clk_en, s1_tima_valid,         s0_fma_en_conv && req_int8, 1'b0)
`EN_FF    (clock,        tima_clk_en, s1_tima_tena_raddr,    fma_exec_a_rd_ptr)
`EN_FF    (clock,        tima_clk_en, s1_tima_tenb_raddr,    { fma_exec_b_rd_ptr, 1'b0 })
`EN_FF    (clock,        tima_clk_en, s1_tima_last_pass,     s0_tima_last_pass)
`EN_FF    (clock,        tima_clk_en, s1_tima_ren3,          ~first_pass)
`EN_FF    (clock,        tima_clk_en, s1_tima_tenc_raddr,    fma_exec_a_row)
`EN_FF    (clock,        working,     s1_tena_rf_rdata_mask, ~conv_skip)
...
logic [`VPU_TENC_ADDR]                       ex_tima_tenc_raddr; // ex tima tenc raddr

//         CLK    RST    EN           DOUT                     DIN                 DEF
`RST_EN_FF(clock, reset, tima_clk_en, ex_tima_valid,           s1_tima_valid,      1'b0) 
`RST_EN_FF(clock, reset, tima_clk_en, ex_fma_mask,             fma_mask_next,       '0)
`EN_FF    (clock,        tima_clk_en, ex_tima_tenb_raddr,      s1_tima_tenb_raddr)
`EN_FF    (clock,        tima_clk_en, ex_tima_tena_raddr,      s1_tima_tena_raddr)
`EN_FF    (clock,        tima_clk_en, ex_tima_last_pass,       s1_tima_last_pass)
`EN_FF    (clock,        tima_clk_en, ex_tima_ren3,            s1_tima_ren3)
`EN_FF    (clock,        tima_clk_en, ex_tima_tenc_raddr,      s1_tima_tenc_raddr)
`EN_FF    (clock,        working,     ex_tena_rdata_mask,      s1_tena_rf_rdata_mask)
...
`RST_EN_FF(clock, reset, tima_clk_en,                  f2_tima_tenc_wen,   ex_tima_tenc_wen,                 '0)
`RST_EN_FF(clock, reset, tima_clk_en,                  f2_tima_rf_wen,     ex_tima_rf_wen,                   '0)
`RST_EN_FF(clock, reset, tima_clk_en,                  f2_tima_ren3,       ex_tima_ren3,                     1'b0)
`EN_FF    (clock,        f0_core_ctrl.tensorfma_start, ex_tima_sa,         !f0_core_ctrl.tensorfma_ctrl.u_a)
`EN_FF    (clock,        f0_core_ctrl.tensorfma_start, ex_tima_sb,         !f0_core_ctrl.tensorfma_ctrl.u_b)
`EN_FF    (clock,        tima_clk_en,                  f2_tima_tenc_raddr, ex_tima_tenc_raddr)
...
//         CLK    RESET  EN           DOUT                DIN                 DEF
`RST_EN_FF(clock, reset, tima_clk_en, f3_tima_tenc_wen,   f2_tima_tenc_wen,   '0)
`RST_EN_FF(clock, reset, tima_clk_en, f3_tima_rf_wen,     f2_tima_rf_wen,     '0)
`EN_FF    (clock,        tima_clk_en, f3_tima_tenc_waddr, f2_tima_tenc_raddr)

////////////////////////////////////////////////////////////////////////////////
// TensorTMP control
////////////////////////////////////////////////////////////////////////////////

logic [`VPU_TENTMP_ADDR] f2_tentmp_waddr;
logic                    s1_tentmp_acol_odd; // A column step is odd
logic                    ex_tentmp_acol_odd; // A column step is odd

//         CLK    RST    EN                   DOUT                  DIN                  DEF
`RST_EN_FF(clock, reset, working && req_int8, f2_tentmp_wen_early,  dc_read_tentmp_wen,  1'b0)
`EN_FF    (clock,        dc_read_tentmp_wen,  f2_tentmp_waddr,      dc_read_a_row[3:1])
`RST_EN_FF(clock, reset, working && req_int8, f3_tentmp_wen,        f2_tentmp_wen_early, 1'b0)
`EN_FF    (clock,        f2_tentmp_wen_early, f3_tentmp_waddr,      f2_tentmp_waddr)
`EN_FF    (clock,        working && req_int8, tensortmp_rf_written, tensortmp_rf_written_next)
`EN_FF    (clock,        working && req_int8, s1_tentmp_acol_odd,   fma_exec_a_col[0] & !fma_exec_start_a_is_odd)
`EN_FF    (clock,        working && req_int8, ex_tentmp_acol_odd,   s1_tentmp_acol_odd)

always_comb begin
    ex_tentmp_ren   = req_int8 && ex_tentmp_acol_odd && ~ex_tima_tenc_raddr[1];
    ex_tentmp_raddr = ex_tima_tenc_raddr[4:2]
        & {$bits(ex_tentmp_raddr){req_int8 && ex_tentmp_acol_odd}};
end
