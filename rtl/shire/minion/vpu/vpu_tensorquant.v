...
module vpu_tensorquant (
  ...
  output logic [`INST_RANGE]                           quant_inst_next,
...
////////////////////////////////////////////////////////////////////////////////
  ...
  `RST_EN_FF(clock, reset, clock_en,            enabled,                enabled_next,                1'b0)
  `RST_EN_FF(clock, reset, clock_en,            tensorquant_last,       tensorquant_last_next,       1'b0)
  `RST_EN_FF(clock, reset, tensorquant_ctrl_en, tensorquant_ctrl,       tensorquant_ctrl_next,       '0)
  ...
  `RST_EN_FF(clock, reset, clock_en,            tstore_pend_sticky,     tstore_pend_sticky_next,     1'b0)
  ...
  always_comb begin
    ...
    if (tensorquant_ctrl_next.scp_src >= `DCACHE_TL_SCP_MAX_IDX)
      tensorquant_ctrl_next.scp_src = tensorquant_ctrl_next.scp_src - `DCACHE_TL_SCP_MAX_IDX;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  logic [1:0][`DCACHE_DATA_RANGE]               scp_data;                  // Data loaded from SCP
  ...
  logic [`DCACHE_SET_RANGE]                     dcache_scp_dest_set;       // Scratchpad destination set
  logic [`DCACHE_WAY_RANGE]                     dcache_scp_dest_way;       // Scratchpad destination way
  ...
  //         CLK    RST    EN        DOUT                  DIN                        DEF
  `RST_EN_FF(clock, reset, clock_en, scp_data_en_s1,       scp_data_en_s0,            2'b0)
  `RST_EN_FF(clock, reset, clock_en, scp_data_en_s2,       scp_data_en_s1,            2'b0)
  `RST_EN_FF(clock, reset, clock_en, scp_read_pending,     scp_read_pending_next,     1'b0)
  `RST_EN_FF(clock, reset, clock_en, scp_read_set_pending, scp_read_set_pending_next, 2'b0)
  `EN_FF    (clock,        clock_en, scp_read_buf,         scp_read_buf_next)
  `EN_FF    (clock,        clock_en, id_b_col,             exec_b_col)
  `EN_FF    (clock,        clock_en, id_a_row,             exec_a_row)
  `EN_FF    (clock,        clock_en, ex_b_col,             id_b_col)
  `EN_FF    (clock,        clock_en, ex_a_row,             id_a_row)

  for (genvar gen_it = 0; gen_it < 2; gen_it++) begin
    //     CLK    EN                      DOUT              DIN
    `EN_FF(clock, scp_data_en_s2[gen_it], scp_data[gen_it], dcache_scp_data)
  end
  ...
  always_comb begin
    ...
    dcache_scp_req.addr    = dcache_scp_raddr[`DCACHE_LRAM_ADDR_RANGE];
    ...
    end else if (dcache_scp_req.read_en && dcache_scp_available) begin
      // Request sent
...
////////////////////////////////////////////////////////////////////////////////
  ...
  //         CLK    RST    EN        DOUT                        DIN                      DEF
  `RST_EN_FF(clock, reset, clock_en, quant_tena_scp_en,          quant_tena_scp_en_next,  1'b0)
  `RST_EN_FF(clock, reset, clock_en, quant_tena_zero_en,         quant_tena_zero_en_next, 1'b0)
  `RST_EN_FF(clock, reset, clock_en, quant_rdata_col,            quant_rdata_col_next,    1'b0)
  `RST_EN_FF(clock, reset, clock_en, ex_rdata_zero,              quant_tena_zero_en,      1'b0)
  `RST_EN_FF(clock, reset, clock_en, ex_rdata_col,               quant_rdata_col,         1'b0)
  `RST_EN_FF(clock, reset, clock_en, exec_hazard_wait,           exec_hazard_wait_next,   3'b000)
  `RST_EN_FF(clock, reset, clock_en, prev_is_txfma,              exec_is_txfma,           1'b0)
  `RST_EN_FF(clock, reset, clock_en, prev_is_shsw,               exec_is_shsw,            1'b0)
  `EN_FF    (clock,        exec_adv, exec_a_row,                 exec_a_row_next)
  `EN_FF    (clock,        exec_adv, exec_b_col,                 exec_b_col_next)
  `RST_FF   (clock, reset,           ex_ren,                     quant_tena_en,           1'b0)
  ...
  always_comb begin
    ...
    if (exec_curr_transform == `TQUANT_INT32_TO_FP32_CVT) begin
      quant_inst_next                        = `FCVT_PS_PW;
      quant_inst_next[`VPU_INST_RM_SEL]      = 3'b111; // Dynamic round mode
      quant_inst_next[`VPU_INST_RD_SEL]      = exec_dest_reg;
      quant_inst_next[`VPU_INST_REN1_RA_SEL] = exec_dest_reg;
      quant_en_next                          = quant_en_next && !exec_hazard_txfma_to_txfma;
    end  else if (exec_curr_transform == `TQUANT_FP32_TO_INT32_CVT) begin
    // INT32 to FP32 convert
      ...
      quant_inst_next[`VPU_INST_REN1_RA_SEL] = exec_dest_reg;
      quant_en_next                          = quant_en_next && !exec_hazard_txfma_to_txfma;
    end else if (exec_curr_transform == `TQUANT_RELU) begin
      quant_inst_next                        = `FMAX_PI;
      quant_inst_next[`VPU_INST_RD_SEL]      = exec_dest_reg;
      quant_inst_next[`VPU_INST_REN1_RA_SEL] = exec_dest_reg;
      quant_inst_next[`VPU_INST_REN2_RA_SEL] = exec_dest_reg;
      quant_en_next                          = quant_en_next && !exec_hazard_txfma_to_short;
      quant_tena_zero_en_next                = quant_en_next;
    end else if ((exec_curr_transform == `TQUANT_INT32_ADD_ROW) 
        || (exec_curr_transform == `TQUANT_INT32_ADD_COL)) begin
    ...
    end else if ((exec_curr_transform == `TQUANT_FP32_MUL_ROW) 
        || (exec_curr_transform == `TQUANT_FP32_MUL_COL)) begin
    // FP32 Mul row-wise/col-wise
      ...
      quant_en_next                          = quant_en_next && !scp_read_pending && !exec_hazard_txfma_to_txfma; // Can execute if data already read from SCP
      quant_tena_scp_en_next                 = quant_en_next;
    end else if (exec_curr_transform == `TQUANT_SAT_INT8) begin
    // Saturation to INT8
      quant_inst_next                        = `FSAT8_PI;
      quant_inst_next[`VPU_INST_RD_SEL]      = exec_dest_reg;
      quant_inst_next[`VPU_INST_REN1_RA_SEL] = exec_dest_reg;
      quant_en_next                          = quant_en_next && !exec_hazard_txfma_to_short;
    end else if (exec_curr_transform == `TQUANT_SAT_UINT8) begin
    // Saturation to UINT8
    ...
    // the execution is not done in the same order as others
      ...
      quant_inst_next[`VPU_INST_RD_SEL]      = {exec_dest_reg[4:1], (tensorquant_ctrl.start_reg[0] & ~exec_b_col)} - {4'b0, (tensorquant_ctrl.start_reg[0] & exec_b_col)};
      quant_inst_next[`VPU_INST_REN1_RA_SEL] = exec_dest_reg;
      quant_en_next                          = quant_en_next && !exec_hazard_txfma_to_short;
    ...
    if (tensorquant_ctrl.cols >= 2'b10) begin
      if (exec_a_row == tensorquant_ctrl.rows) begin
    ...
    if (exec_last_op_trans || f0_core_ctrl.tensorquant_start) begin
      exec_a_row_next = 4'b0;
    ...
    else if ((((quant_en_next && exec_last_op_trans) 
        || new_req) && (tensorquant_ctrl_next.rows < 3'b010))) begin
