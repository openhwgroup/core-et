...
module vpu_lane (
  ...
  input  logic [`FREG_ADDR_RANGE]             id_regfile_raddr1,
  input  logic [`FREG_ADDR_RANGE]             id_regfile_raddr2,
  input  logic [`FREG_ADDR_RANGE]             id_regfile_raddr3,
  input  logic [2:0]                          id_regfile_ren,
  input  logic [2:0]                          id_regfile_thread_id,
  input  logic                                id_trans_busy,
  // EX/F1 data and control inputs
  input  logic [`FREG_ADDR_RANGE]             ex_regfile_raddr1,
  input  logic [`FREG_ADDR_RANGE]             ex_regfile_raddr2,
  input  logic [`FREG_ADDR_RANGE]             ex_regfile_raddr3,
  ...
  input  logic[`VPU_TENB_ADDR]                ex_tima_tenb_raddr,
  ...
  input  logic [`VPU_TENC_ADDR]               f2_tima_tenc_raddr,
  ...
  input  logic [`VPU_TENB_ADDR]               f3_tenb_regfile_waddr_l,
  input  logic [`VPU_NUM_TIMA-1:0]            f3_tima_tenc_wen,
  input  logic [`VPU_TENC_ADDR]               f3_tima_tenc_waddr,
...
// Signal Declarations
logic [2:0][`FREG_ADDR_RANGE]                 id_rf_raddr;                  // Read addr for the RF
...
logic [2:0][`FREG_ADDR_RANGE]                 ex_rf_bypass_raddr;           // Read addr for the RF
logic [`VPU_NUM_TIMA-1:0][`VPU_TENB_ADDR]     ex_tenb_rf_raddr;             // Read addr for the tenb RF
logic                                         f2_tenb_rf_wen;
vpu_output_t                                  f3_sh_sw_bits;
logic [`VPU_NUM_TIMA-1:0][`VPU_DATA_S_SZ-1:0] f3_tima_data;
logic [`VPU_TENB_ADDR]                        f3_tenb_rf_waddr;             // Write addr for the tenb RF
logic                                         f3_tenb_pass;
...
logic [1:0][`FREG_ADDR_RANGE]                 f8_rf_waddr;                  // Write addr for the RF
...
logic [`TIMA_CLOCK_EN_DELAY-1:0]              tima_clock_valid_delay_next;
  ...
  `RST_EN_FF(clock, reset, trans_rom_clock_valid_delay_en, trans_rom_clock_valid_delay, trans_rom_clock_valid_delay_next, {`TRANS_ROM_CLOCK_EN_DELAY{1'b0}})
  `RST_EN_FF(clock, reset, sh_sw_clock_valid_delay_en,     sh_sw_clock_valid_delay,     sh_sw_clock_valid_delay_next,     {`SH_SW_CLOCK_EN_DELAY{1'b0}})
  ...
  // Clock for TXFMA
  et_clk_gate cgate_txfma(
    .enclk ( clock_txfma                              ),
    .en    ( clock_en_txfma | chicken_bit_vpulane     ),
    .clk   ( clock                                    ),
    .te    ( 1'b0                                     )
  );

  // Clock for TRANS ROM
  et_clk_gate cgate_trans_rom (
    .enclk ( clock_trans_rom                          ),
    .en    ( clock_en_trans_rom | chicken_bit_vpulane ),
    .clk   ( clock                                    ),
    .te    ( 1'b0                                     )
  );

  // Clock for SH SW
  et_clk_gate cgate_sh_sw (
    .enclk ( clock_sh_sw                              ),
    .en    ( clock_en_sh_sw | chicken_bit_vpulane     ),
    .clk   ( clock                                    ),
    .te    ( 1'b0                                     )
  );

  // Clock for bypass
  et_clk_gate cgate_bypass (
    .enclk ( clock_bypass                             ),
    .en    ( clock_en_bypass | chicken_bit_vpulane    ),
    .clk   ( clock                                    ),
    .te    ( 1'b0                                     )
  );
`else
...
vpu_rf  rf (
  ...
  .clock       ( clock                ),
  // Read ports
  .rd_en       ( id_regfile_ren_ovr   ),
  .rd_thrid    ( id_rf_thread_id      ),
  .rd_addr     ( id_rf_raddr          ),
  .rd_data     ( ex_rf_rdata          ),
  // Write ports
  .wr_en       ( id_regfile_wen_ovr   ),
  .wr_thrid    ( f8_rf_thrid[1:0]     ),
  .wr_mask     ( f8_rf_wmask[1:0]     ),
  .wr_addr     ( f8_rf_waddr[1:0]     ),
  .wr_data     ( f8_rf_wdata[1:0]     )
);
...
assign ex_bypass_tenb_rf_rdata = (ex_tenb_rf_raddr[0][0] ? 
    ex_tenb_rf_rdata[1] : ex_tenb_rf_rdata[0]) & {`VPU_DATA_S_SZ{!ex_tima_valid_unqual}}; // Selects data from correct TENB
...
vpu_bypass bypass (
  ...
  .clock                       ( clock_bypass                ),
  // EX/F1 inputs  
  .ex_rf_raddr                 ( ex_rf_bypass_raddr          ),
  .ex_regfile_thread_id        ( ex_regfile_thread_id        ),
  .ex_rf_rdata                 ( ex_rf_rdata                 ),
  .ex_req_lane                 ( ex_req_lane                 ),
  .ex_fromint_data             ( ex_fromint_data             ),
  .ex_tena_rf_rdata            ( ex_bypass_tena_rf_rdata     ),
  .ex_tenb_rf_rdata            ( ex_bypass_tenb_rf_rdata     ),
...
vpu_txfma_trans_top txfma_trans_top (
...
assign ex_tenb_rf_raddr[0] = ex_tima_valid_unqual ? ex_tima_tenb_raddr                                       : ex_regfile_raddr2[`VPU_TENB_ADDR];
assign ex_tenb_rf_raddr[1] = ex_tima_valid_unqual ?
    { ex_tima_tenb_raddr[$clog2(`VPU_TENB_SIZE)-1:1], 1'b1 } : ex_regfile_raddr2[`VPU_TENB_ADDR]; // Second port always read odd entries for TIMA operations

for(genvar gen_it = 0; gen_it < `VPU_NUM_TIMA; gen_it++) begin : LANE_TIMA
  ...
  vpu_lane_tima lane_tima (
    // System signals
    .TE               ( 1'b0                                                 ),     
    .clk              ( clock                                                ),
    .tima_clk_en      ( clock_en_tima | chicken_bit_vputima                  ),      
    // F1 inputs
    .f1_tima_mul_en   ( ex_tima_valid[gen_it]                                ),
    .tima_in1_signed  ( ex_tima_sa                                           ),
    .tima_in2_signed  ( ex_tima_sb                                           ),      
    // F2 inputs    
    .f2_tima_en       ( f2_tima_valid[gen_it]                                ),
    .f2_tima_in3_en   ( f2_tima_ren3                                         ),
    .f2_tima_zap_opdc ( !f2_tima_ren3                                        ),
    // TEN A inputs    
    .tena_rd_data     ( ex_tena_rf_tima_rdata                                ),
    .tenb_wr_en_early ( f2_tenb_rf_wen                                       ),
    .tenb_wr_en       ( f3_tenb_wr_en                                        ),    
    .tenb_rd_addr     ( ex_tenb_rf_raddr[gen_it][$clog2(`VPU_TENB_SIZE)-1:1] ),
    .tenb_wr_addr     ( f3_tenb_rf_waddr[$clog2(`VPU_TENB_SIZE)-1:1]         ),
    .tenb_wr_data     ( f3_tenb_rf_wdata                                     ),    
    // TEN C inputs
    .tenc_rd_addr     ( f2_tima_tenc_raddr                                   ),
    .tenc_wr_addr     ( f3_tima_tenc_waddr                                   ),
    .tenc_wr_en       ( f3_tima_tenc_wen[gen_it] & f3_tima_valid[gen_it]     ),    
    // F3 outputs    
    .tenb_out         ( ex_tenb_rf_rdata[gen_it]                             ),
    .tima_out         ( f3_tima_data[gen_it]                                 )
  );
end
...
vpu_sh_sw sh_sw (
  ...
  .ex_dtype        ( ex_req.sigs.dtype ),
  .f2_swizz_in1    ( f2_fswizz_rdata1  ),
  .f2_swizz_in2    ( f2_fswizz_rdata2  ),
  .f2_swizz_in3    ( f2_fswizz_rdata3  ),
  .f2_fswizz_rdata ( f2_fswizz_rdata   ),
  .f3_out_bits     ( f3_sh_sw_bits     )
);
...
//         CLK    RST    EN                            DOUT           DIN            DEF
`RST_EN_FF(clock, reset, dcache_scp_resp.fill_is_tenb, f3_tenb_pass,  ~f3_tenb_pass, 1'b0)
`RST_FF   (clock, reset,                               f2_tima_valid, ex_tima_valid, 1'b0)
`RST_FF   (clock, reset,                               f3_tima_valid, f2_tima_valid, 1'b0)
