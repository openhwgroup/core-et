...
et_clk_gate cgate_txfma0_opa_frf (
  .clk   ( txfma_clk        ), 
  .en    ( opa_clken_frfa_h ),
  .enclk ( opa_frf_clk      ),  
  .te    ( test_enable      )
...
et_clk_gate cgate_txfma0_opb_frf (
  .clk   ( txfma_clk        ), 
  .en    ( opb_clken_frfa_h ),
  .enclk ( opb_frf_clk      ),  
  .te    ( test_enable      )
...
et_clk_gate cgate_txfma0_opc_frf (
  .clk   ( txfma_clk        ), 
  .en    ( opc_clken_frfa_h ),
  .enclk ( opc_frf_clk      ),  
  .te    ( test_enable      )
...
et_clk_gate cgate_txfma0_trans_frf (
  .clk   ( txfma_clk            ), 
  .en    ( transop_clken_frfa_h ),
  .enclk ( trans_frf_clk        ),  
  .te    ( 1'b0                 )
);

et_clk_gate cgate_txfma0_ctrl_frf (
  .clk   ( txfma_clk          ), 
  .en    ( txfma_clken_frfa_h ),
  .enclk ( ctrl_frf_clk       ),  
  .te    ( 1'b0               )
);

et_clk_gate cgate_txfma0_ctrl_f0 (
  .clk   ( txfma_clk          ), 
  .en    ( txfma_clken_f0a_h  ),
  .enclk ( ctrl_f0_clk        ),  
  .te    ( 1'b0               )
);

et_clk_gate cgate_txfma0_multype_f0 (
  .clk   ( txfma_clk           ), 
  .en    ( multype_clken_f0a_h ),
  .enclk ( multype_f0_clk      ),  
  .te    ( 1'b0                )
);

et_clk_gate cgate_txfma0_multype_f1 (
  .clk   ( txfma_clk           ), 
  .en    ( multype_clken_f1a_h ),
  .enclk ( multype_f1_clk      ),  
  .te    ( 1'b0                )
);

et_clk_gate cgate_txfma0_ctrl_f1 (
  .clk   ( txfma_clk          ), 
  .en    ( txfma_clken_f1a_h  ),
  .enclk ( ctrl_f1_clk        ),  
  .te    ( 1'b0               )
);

et_clk_gate cgate_txfma0_ctrl_f2 (
  .clk   ( txfma_clk          ), 
  .en    ( txfma_clken_f2a_h  ),
  .enclk ( ctrl_f2_clk        ),  
  .te    ( 1'b0               )
);

et_clk_gate cgate_txfma0_ctrl_f3 (
  .clk   ( txfma_clk          ), 
  .en    ( txfma_clken_f3a_h  ),
  .enclk ( ctrl_f3_clk        ),  
  .te    ( 1'b0               )
);

et_clk_gate cgate_txfma0_ctrl_f4 (
  .clk   ( txfma_clk          ), 
  .en    ( txfma_clken_f4a_h  ),
  .enclk ( ctrl_f4_clk        ),  
  .te    ( 1'b0               )
);

et_clk_gate cgate_txfma0_ctrl_f5 (
  .clk   ( txfma_clk          ), 
  .en    ( txfma_clken_f5a_h  ),
  .enclk ( ctrl_f5_clk        ),  
  .te    ( 1'b0               )
);
