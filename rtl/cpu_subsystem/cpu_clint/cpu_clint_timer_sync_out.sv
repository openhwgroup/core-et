...
module cpu_clint_timer_sync_out (
    input logic clock,
    input logic clock_m,
    input logic reset_m,
    // mtime
...
fifo_gcd #(
    .ELEM_SIZE(64),
    .NUM_ELEMS(2) // 1GHz pop, 200MHz or 1GHz pop
) mtime_sync (
    .clock_push (clock_m), // comes in on mtime clk i.e. cpu_subsystem OR external
    .reset_push (reset_m),
    .clock_pop (clock), // goes out on cpu_subsystem
