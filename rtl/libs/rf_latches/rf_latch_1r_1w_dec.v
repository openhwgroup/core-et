...
module rf_latch_1r_1w_dec (/*AUTOARG*/
   // Outputs
   rd_data_a,
   // Inputs
   rd_addr_a,
   ...
   /*
    ...
    2) The decoded read address produces a combinatorial output in the the same clock
   ...
   output reg [WIDTH-1:0]  rd_data_a; // Read Port A

   input [ENTRIES-1:0] rd_addr_a; // Read Address for port A

   input [WIDTH-1:0]   wr_data_a; // Write Data - port A
   input               wr_data_a_en_1p; // Write Data Enable - port A (from Ph2 latch)
   input [ENTRIES-1:0] wr_addr_a;  // Write Address - port A
...
`ifdef ENABLE_VERILATOR_MODEL
   ...
   always_ff@(posedge clk) begin
      if (wr_en_a)
      begin
        for(integer k = 0; k < ENTRIES; k++)
          if(wr_addr_a[k]) rf[k] <= wr_data_a_del;
      end
   end
...
`else
   ...
   assign dec_wr_en =  wr_addr_a & {ENTRIES{wr_en_a || test_en}};
...
`endif
   ...
   always_comb
   begin
      // Muxes the correct entry
      rd_data_a = {WIDTH{1'b0}};
      for(integer k = 0; k < ENTRIES; k++)
          if(rd_addr_a[k]) rd_data_a |= rf[k];
   end
