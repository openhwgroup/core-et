...
// This module implements an LRU arbiter for a queue
module queue_arb_lru
#(
...
(
    ...
    // Update priority
    input  logic                       prio_up,
    input  logic [NUM_CLIENTS_LOG-1:0] prio_entry,
    // Bidding
...
begin
    winner = '0;
    // For all the clients
    for(integer unsigned i = 0; i < NUM_CLIENTS; i++)
    begin
        ...
        for(integer unsigned j = 0; j < i; j++)
        begin
        ...
        for(integer unsigned j = i + 1; j < NUM_CLIENTS; j++)
        begin
