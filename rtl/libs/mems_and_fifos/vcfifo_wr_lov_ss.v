...
// vcfifo_wr_hiv_ss - voltage change write low voltage semisync fifo
...
module vcfifo_wr_lov_ss #(parameter
   ELEM_SIZE       = 32,  // WIDTH
   NUM_ELEMS       = 3,   // DEPTH
   ...
   //----------------------------------------------------------------------------------
   // Write -> Read
   vclevel_shft_l2h #(.WIDTH(1))     ls_l2h_push_d1         (.out_hv (push_d1_hv),      .inp_lv (push_d1_lv));
   vclevel_shft_l2h #(.WIDTH(WIDTH)) ls_l2h_push_data_d1    (.out_hv (push_data_d1_hv), .inp_lv (push_data_d1_lv));
   // Read -> Write
   vclevel_shft_h2l #(.WIDTH(1))     ls_h2l_pop_d1_pd_hv    (.out_lv (pop_d1_lv),       .inp_hv (pop_d1_hv));
   // Resets
   vclevel_shft_l2h #(.WIDTH(1))     ls_l2h_reset_push      (.out_hv (reset_push_hv),   .inp_lv (reset_push));
   vclevel_shft_h2l #(.WIDTH(1))     ls_h2l_reset_pop_pd_hv (.out_lv (reset_pop_lv),    .inp_hv (reset_pop));
