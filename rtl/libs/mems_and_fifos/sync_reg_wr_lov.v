...
// sync_reg_wr_hiv
//   
//  This module is used for getting slow changing data signals from a low voltage domain
//  to a high voltage domain.
//
...
module sync_reg_wr_lov #(parameter
   WIDTH = 1,
   ...
   //----------------------------------------------------------------------------------
   // Write -> Read
   vclevel_shft_l2h #(.WIDTH(1))     ls_l2h_wr_cnt_gcd_d1 (.out_hv (wr_cnt_gcd_d1_hv),  .inp_lv (wr_cnt_gcd_d1_lv));
   vclevel_shft_l2h #(.WIDTH(WIDTH)) ls_l2h_wr_data_d1    (.out_hv (wr_data_d1_hv),     .inp_lv (wr_data_d1_lv));
   vclevel_shft_h2l #(.WIDTH(1))     ls_h2l_rd_cnt_gcd_d1 (.out_lv (rd_cnt_gcd_lv),     .inp_hv (rd_cnt_gcd_hv));
