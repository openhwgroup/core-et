...
| `shire_top` | `shire_top` public native compute-shire wrapper | 46 directed integration checks | 607,200 comparisons vs retained top-wiring slice | Done — instantiates one `shire_channel_wrap` plus `NumNeigh` `neigh_top` instances, exposes native AXI/reset/DFT/interrupt/status/control boundary, and covers reset fanout, SYS/APB, DFT/RAM, neighborhood/cache/uncached/ICache/FLB/cooperative/power seams with removed third-party/generated surfaces omitted per contract |
