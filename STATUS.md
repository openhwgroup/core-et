...
| `shire_channel_wrap` | `shire_channel_wrap` retained Ainekko-owned shell behavior | 58 directed wrapper checks | 294,161 comparisons vs original retained wrapper slice | Done — native shell wraps `shire_channel`, retained reset/clock/DFT/SBM/APB/AXI/cache/uncached/power/status/control/RAM-config seams, real original-vs-new wrapper cosim with explicit retained-input toggle coverage, and intentionally omits removed debug/sensor/hard-macro surfaces |
| `shire_top` | `shire_top` public native compute-shire wrapper | 46 directed integration checks | 566,632 comparisons vs retained top-wiring slice | Done — instantiates one `shire_channel_wrap` plus `NumNeigh` `neigh_top` instances, exposes native AXI/reset/DFT/interrupt/status/control boundary, and covers reset fanout, SYS/APB, DFT/RAM, neighborhood/cache/uncached/ICache/FLB/cooperative/power seams with removed third-party/generated surfaces omitted per contract |
