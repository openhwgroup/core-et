...
//
// Unit test for rbox_fp_adder_2stg.
//
// Verifies the 2-stage pipelined FP adder. Stage 1 registered on valid_i,
// stage 2 combinational. Drive inputs + valid on cycle N, read output on
// cycle N+1.

#include "Vrbox_fp_adder_2stg_tb.h"
#include "sim_ctrl.h"
#include <cstdint>

using DUT = Vrbox_fp_adder_2stg_tb;
...
int main(int argc, char** argv) {
    ...
    // -- Test 1: 0 + 0 = 0 (pipeline: latch then read) --
    printf("test 1: 0 + 0\n");
    set_depth(sim.dut.get(), true,  false, false, true, false, 0, false, 0, 0, 0);
    set_depth(sim.dut.get(), false, false, false, true, false, 0, false, 0, 0, 0);
    sim.dut->valid_i = 1;
    ...
    sim.dut->valid_i = 0;
    // Stage 2 output is now combinationally available
