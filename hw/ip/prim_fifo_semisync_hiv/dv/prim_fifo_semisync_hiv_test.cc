...
//
// prim_fifo_semisync_hiv — standalone Verilator unit test.

#include "Vprim_fifo_semisync_hiv.h"
#include "sim_ctrl.h"
...
using DUT = Vprim_fifo_semisync_hiv;
...
int main(int argc, char** argv) {
    ...
    printf("test 5: HIV DFT controls write-side reset bypass\n");
    push_burst(sim, 0xF0000000u, 3);
    sim.check(sim.dut->ready_o == 0, "full before HV DFT write reset");
    sim.dut->dft_hv_i = 0x10;  // scanmode=1, scan_reset_n=0
    tick_both(sim);
    sim.check(sim.dut->ready_o == 0, "ready masked while HV scan reset asserted");
    sim.dut->dft_hv_i = 0x18;  // scanmode=1, scan_reset_n=1
    tick_both(sim);
    sim.dut->dft_hv_i = 0;
    wait_both(sim, 4);
    sim.check(sim.dut->ready_o == 1, "HV DFT reset clears write-side credits");
    functional_reset(sim);

    printf("test 6: LV DFT controls read-side reset bypass\n");
    push_one(sim, 0x12345678u);
    sim.check(sim.dut->valid_o == 1, "valid before LV DFT read reset");
    sim.dut->dft_lv_i = 0x10;  // scanmode=1, scan_reset_n=0
    tick_both(sim);
    sim.check(sim.dut->valid_o == 0, "valid masked while LV scan reset asserted");
