...
(
  ...
  output logic [CsrSatpModeSz-1:0]     dbg_req_satp_mode_o,
  output logic [1:0]                   dbg_req_prv_o,
  output logic [VaSize-1:VaUntransSize] dbg_req_addr_o,
  output logic [$bits(ptw_dbg_sm_t)-1:0] dbg_sm_signals_o
);

  minion_ptw_req req_data [5];
  ...
  always_comb begin
    for (int i = 0; i < 5; i++) begin
      req_data[i] = '0;
      req_data[i].satp_mode = req_satp_mode_i[i];
      req_data[i].satp_ppn  = req_satp_ppn_i[i];
      req_data[i].prv       = req_prv_i[i];
      req_data[i].addr      = req_addr_i[i];
    end
    ...
    dbg_state_o              = dbg_sm_signals.state;
    dbg_req_satp_mode_o      = dbg_sm_signals.req_satp_mode;
    dbg_req_prv_o            = dbg_sm_signals.req_prv;
    dbg_req_addr_o           = dbg_sm_signals.req_addr;
