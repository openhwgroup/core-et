...
// Xilinx implementation using an explicit FF-bank realization.
...
module prim_rf_1r1w_diff #(
  ...
  (* ram_style = "registers" *) logic [WWidth-1:0] rf_q [Entries];
  logic [WWidth-1:0] wr_data_del_q;
  logic [Entries-1:0] wr_sel;
  ...
  for (genvar g = 0; g < Entries; g++) begin : gen_rf
    localparam logic [WAddrW-1:0] EntryIdx = g;
    assign wr_sel[g] = (wr_addr_i == EntryIdx);

    for (genvar j = 0; j < R2WRatio; j++) begin : gen_slice
      always_ff @(posedge clk_i) begin
        if (wr_sel[g] && wr_en_i[j]) begin
          rf_q[g][j*RWidth +: RWidth] <= wr_data_del_q[j*RWidth +: RWidth];
        end
      end
    end
  end
