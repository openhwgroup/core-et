...
#(
  ...
  // CDC synchronizers.  The original WR_LOV vcfifo samples write-count CDC
  // stages on the read clock directly.  The read-count CDC path into the write
  // domain is sampled on the inverted write clock, then registered once on the
  // write clock in vcfifo_wr_pd_gcd.
  ...
  always_ff @(posedge clk_wr_i or negedge rst_wr_n) begin
    if (!rst_wr_n) rcnt_gray_sync_q <= '0;
    else           rcnt_gray_sync_q <= rcnt_sync[SyncStages-1];
  end
