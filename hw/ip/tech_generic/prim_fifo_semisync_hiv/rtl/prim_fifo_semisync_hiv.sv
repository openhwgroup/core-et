...
//
// Technology primitive: semi-synchronous HV->LV FIFO.
// Generic implementation preserving the CORE-ET vcfifo_wr_hiv_ss handshake.

module prim_fifo_semisync_hiv
  import dft_pkg::*;
#(
  ...
  assign rst_wr_async_ni = dft_hv_i.scanmode ? dft_hv_i.scan_reset_n : (rst_wr_ni & rst_rd_ni);
  assign rst_rd_async_ni = dft_lv_i.scanmode ? dft_lv_i.scan_reset_n : (rst_wr_ni & rst_rd_ni);
  ...
  assign rst_wr_sync = dft_hv_i.scanmode ? ~dft_hv_i.scan_reset_n : rst_wr_meta_q[ResetSyncStages-1];
  assign rst_rd_sync = dft_lv_i.scanmode ? ~dft_lv_i.scan_reset_n : rst_rd_meta_q[ResetSyncStages-1];
