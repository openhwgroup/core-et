...
//
// SECDED ECC decoder/corrector. Pure combinational.
//
// Decodes {ecc, data} input, corrects single-bit errors, detects double-bit
// errors. Uses syndrome-based lookup table for error bit identification.
//
...
//
// Replaces: ecc_corr (etcore-soc)
//
// Differences from original:
//   - lowRISC naming (_i/_o)
//   - Parameter naming (DataWidth instead of DATA_SIZE)
//   - Instantiates prim_ecc_enc instead of ecc_gen
...
module prim_ecc_dec #(
  ...
  input  logic [DataAndEccWidth-1:0] data_and_ecc_i, // {ecc, data}
  output logic [DataWidth-1:0]       data_o,         // corrected data
  output logic                       sbe_o,          // single-bit error
  output logic                       dbe_o           // double-bit error (uncorrectable)
);
  ...
  assign {ecc, data} = data_and_ecc_i;

  // Regenerate expected ECC from data
  logic [DataWidth-1:0] data_unused;
  logic [EccWidth-1:0]  expected_ecc;
  prim_ecc_enc #(.DataWidth(DataWidth)) u_expected_ecc (
    .data_i        (data),
    .data_and_ecc_o({expected_ecc, data_unused})
  );

  // Syndrome and error detection
  logic [EccWidth-1:0] syndrome;
  logic                any_err;
  assign syndrome = ecc ^ expected_ecc;
  assign any_err  = |syndrome;

  // Syndrome lookup → bit flip mask
  logic [MaxDataWidth-1:0] int_bit_flip;
  ...
  // Extract relevant bits and apply correction
  logic [DataWidth-1:0] bit_flip;
  if (DataWidth == MaxDataWidth) begin : gen_max
    assign bit_flip = int_bit_flip;
  end else if ((DataWidth >= MinDataWidth) && (DataWidth <= MaxDataWidth)) begin : gen_trunc
    assign bit_flip = int_bit_flip[0 +: DataWidth];
  end else begin : gen_bad
    assign bit_flip = 'x;
  end

  assign data_o = data ^ bit_flip;
  assign sbe_o  = ^syndrome;          // odd parity → single-bit error (correctable)
  assign dbe_o  = any_err && !sbe_o;  // even parity with error → double-bit (uncorrectable)

  logic unused_ok;
  assign unused_ok = &{1'b0, int_bit_flip, data_unused};

endmodule : prim_ecc_dec
