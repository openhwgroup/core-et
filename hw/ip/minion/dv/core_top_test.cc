...
//
// Smoke test for minion core_top.
...
int main(int argc, char** argv) {
    ...
    sim.check(sim.dut->trace_min_reset_o == 0, "trace min_reset clears after reset release");
    sim.check((sim.dut->debug_have_reset_o & 0x1) == 0x1, "thread0 reports have_reset after reset release");
    ...
    sim.check(saw_fetch, "frontend issues an icache request after reset");
    if (saw_fetch) {
