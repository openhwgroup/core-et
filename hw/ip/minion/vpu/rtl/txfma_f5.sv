...
always_ff @(posedge ctrl_f5_clk) begin
  mergd_frac_res_hi_f6a_h <= mergd_frac_res_hi_f5a_h;
  ftz_f6a_l <= ftz_f5a_l;
  frac_zero_f6a_h <= frac_zero_f5a_h;
  rnd_l_ph_f6a_h <= rnd_l_ph_f5a_h;
  rnd_r_ph_f6a_h <= rnd_r_ph_f5a_h;
  rnd_l_ps_f6a_h <= rnd_l_ps_f5a_h;
  rnd_r_ps_f6a_h <= rnd_r_ps_f5a_h;
  rnd_l_int_f6a_h <= rnd_l_int_f5a_h;
  rnd_r_int_f6a_h <= rnd_r_int_f5a_h;
end

// Locally flopped control signals I/P from txfma_c4
always_ff @(posedge ctrl_f4_clk) begin
  enc_nsamt_f5a_h <= enc_nsamt_f4a_h;
end
...
(
  .op_i     ( comb_add_f5a_h       ),
  .shfamnt_i( enc_nsamt_f5a_h      ),
  .op_norm_o( norm_shift_out_f5a_h )
);
