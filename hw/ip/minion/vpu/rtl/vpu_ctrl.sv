...
/* verilator lint_off WIDTHEXPAND */  // Imported original VPU control uses macro-sized constants and narrow packed fields that Verilator widens noisily.
/* verilator lint_off WIDTHTRUNC */  // The preserved original decode/control arithmetic intentionally truncates back into packed control fields.
/* verilator lint_off UNOPTFLAT */  // ID/stall/replay feedback is preserved from the original multi-stage control path; Verilator flattens false loops here.
/* verilator lint_off UNUSEDSIGNAL */  // The original controller keeps wide helper/debug/control signals live even when some packed bits are unused in this build.
module vpu_ctrl
#(
...
(
  ...
  input  logic [XREG_SIZE-1:0]                                    ex_core_fromint_data,
  // F2 inputs
  ...
  output logic [VPU_LANE_NUM-1:0]                              f7_regfile_wen,
  // F8 outputs
...
logic [INST_SIZE-1:0]                                               id_core_inst;
logic [INST_SIZE-1:0]                                               id_core_inst_next;
logic                                                             id_core_valid_int;
logic [FREG_ADDR_SIZE-1:0]                                          id_ra1;
logic [FREG_ADDR_SIZE-1:0]                                          id_ra2;
...
logic [INST_SIZE-1:0]                                               id_ml_inst_next;
...
logic [FREG_ADDR_SIZE-1:0]                                          ex_ra1;
logic [FREG_ADDR_SIZE-1:0]                                          ex_ra2;
logic [FREG_ADDR_SIZE-1:0]                                          ex_ra3;
logic [VPU_TENA_ADDR_W-1:0]                                            ex_ra1_ten;
logic [VPU_TENA_ADDR_W-1:0]                                            ex_tima_tena_raddr;
logic [VPU_TENB_ADDR_W-1:0]                                            ex_ra2_ten;
...
logic                                                             ex_mask_valid;
logic [INST_SIZE-1:0]                                               ex_core_inst;
...
logic [VPU_TENA_ADDR_W-1:0]                                            ex_tena_raddr;
logic                                                             ex_tima_ren3;
logic                                                             ex_tima_last_pass;
logic                                                             ex_tentmp_ren;
logic                                                             ex_tquant_ren;
logic [VPU_TENTMP_ADDR_W-1:0]                                          ex_tentmp_raddr;
...
logic                                                             f2_wen;
logic [FREG_ADDR_SIZE-1:0]                                          f2_regfile_waddr;
...
logic                                                             f3_tentmp_wen;
logic [VPU_TENTMP_ADDR_W-1:0]                                          f3_tentmp_waddr;
logic                                                             f4_wen;
...
logic [VPU_LANE_NUM-1:0]                                         f7_regfile_wen_orig;
...
always_ff @(posedge clock_aon) begin
  if (id_core_inst_en) begin
    id_core_inst <= id_core_inst_next;
  end
  id_inst_from_core <= !(id_ml_inst_en_next | id_trans_insert_en_next);
  if (id_ml_inst_en_next) begin
    id_ml_mask <= id_ml_mask_next;
  end
end
...
uinst_decoder
(
  .id_inst_i ( id_core_inst        ),
  .id_sigs_o ( id_decoder_sigs_int )
);
...
begin
  ...
  if  (!ex_core_gscing) begin
    ...
    if (id_decoder_sigs_sel.ren1) begin
      if (!id_decoder_sigs_sel.swap12) begin
        id_ra1 = id_core_inst_int[VPU_INST_REN1_RA_SEL_MSB:VPU_INST_REN1_RA_SEL_LSB];
      end

      if (id_decoder_sigs_sel.swap12) begin
        id_ra2 = id_core_inst_int[VPU_INST_REN1_RA_SEL_MSB:VPU_INST_REN1_RA_SEL_LSB];
        id_vpu_ctrl.sigs.ren1 = 0;
        id_vpu_ctrl.sigs.ren2 = 1;
      end

      if (id_decoder_sigs_sel.swap13) begin
        id_ra3 = id_core_inst_int[VPU_INST_REN1_RA_SEL_MSB:VPU_INST_REN1_RA_SEL_LSB];
    ...
    if (id_decoder_sigs_sel.ren2) begin
      if (id_decoder_sigs_sel.swap12) begin
        id_ra1 = id_core_inst_int[VPU_INST_REN2_RA_SEL_MSB:VPU_INST_REN2_RA_SEL_LSB];
        id_vpu_ctrl.sigs.ren1 = 1;
        id_vpu_ctrl.sigs.ren2 = 0;
      end

      if (id_decoder_sigs_sel.swap23) begin
        id_ra3 = id_core_inst_int[VPU_INST_REN2_RA_SEL_MSB:VPU_INST_REN2_RA_SEL_LSB];
        id_vpu_ctrl.sigs.ren2 = 0;
        id_vpu_ctrl.sigs.ren3 = 1;
      end

      if (!id_decoder_sigs_sel.swap12 && !id_decoder_sigs_sel.swap23)
        id_ra2 = id_core_inst_int[VPU_INST_REN2_RA_SEL_MSB:VPU_INST_REN2_RA_SEL_LSB];
    end

    // ren3
    if (id_decoder_sigs_sel.ren3) begin
      id_ra3 = id_core_inst_int[VPU_INST_REN3_RA_SEL_MSB:VPU_INST_REN3_RA_SEL_LSB];
    end

    // fsetm
    if (id_decoder_sigs_sel.cvt && (id_decoder_sigs_sel.cmd == VPU_FCMD_FSETM)) begin
      id_ra1 = id_core_inst_int[VPU_INST_MASK_FRA_SEL_MSB:VPU_INST_MASK_FRA_SEL_LSB];
    end
    ...
    if (id_decoder_sigs_sel.maskop) begin
      id_ra1 = {2'b0,id_core_inst_int[VPU_INST_MASK_MRA1_SEL_MSB:VPU_INST_MASK_MRA1_SEL_LSB]};
      id_ra2 = {2'b0,id_core_inst_int[VPU_INST_MASK_MRA2_SEL_MSB:VPU_INST_MASK_MRA2_SEL_LSB]};
    end

    // use rd as ra1 (scatter and scatter on 32-byte blocks) or ra3
    if (id_decoder_sigs_sel.rend) begin
      if (id_decoder_sigs_sel.ren3)
        id_ra3 = id_core_inst_int[VPU_INST_RD_SEL_MSB:VPU_INST_RD_SEL_LSB];
      else
        id_ra1 = id_core_inst_int[VPU_INST_RD_SEL_MSB:VPU_INST_RD_SEL_LSB];
    end
  ...
  id_vpu_ctrl.typ = id_use_prev_sigs ? '0
                  :                    id_core_inst_int[VPU_INST_TYP_SEL_MSB:VPU_INST_TYP_SEL_LSB];



    // Rounding mode, sel rm from CSR when ex_core_inst[VPU_INST_RM_SEL_MSB:VPU_INST_RM_SEL_LSB] == '7'
  id_vpu_ctrl.rm = !id_vpu_ctrl.sigs.round ?                                                     RTZ                                 // Round Towards Zero if no round flag
                 : (id_core_inst_int[VPU_INST_RM_SEL_MSB:VPU_INST_RM_SEL_LSB] == VPU_RM_DYN | id_vpu_ctrl.sigs.gcvt) ? id_fcsr_rm                          // Dynamic rm
                 :                                                                               id_core_inst_int[VPU_INST_RM_SEL_MSB:VPU_INST_RM_SEL_LSB]; //instruction
...
begin
  ...
  //trans scoreboard
  for(integer i = 0; i < TRANS_SCOREBOARD_SLOTS; i++)
  begin
...
begin
  ...
  if (EnableExtraTrans) begin
    unique case (id_vpu_ctrl.sigs.cmd)
...
always_ff @(posedge clock_aon) begin
  if (reset) begin
    ex_ml_inst_val <= 1'b0;
    ex_ctrl_valid <= 1'b0;
  end else if (!ex_core_gscing) begin
    ex_ml_inst_val <= id_ml_inst_en;
    ex_ctrl_valid <= id_core_valid_int;
  end
end

always_ff @(posedge clock_aon) begin
  if (id_core_valid_int) begin
    ex_ra1 <= id_ra1;
    ex_ra2 <= id_ra2;
    ex_ra3 <= id_ra3;
    ex_core_inst <= id_core_inst_int;
    ex_ml_mask <= id_ml_mask;
    ex_tensorfma_en <= id_ml_inst_fma_en;
    ex_tena_regfile_bypass_en <= id_tena_regfile_bypass_en;
    ex_tenb_regfile_bypass_en <= id_tenb_regfile_bypass_en;
  end
end
...
always_ff @(posedge clock_aon) begin
  ...
  end else if (id_core_valid_int && !id_use_prev_sigs) begin
    ex_vpu_ctrl <= id_vpu_ctrl;
    ex_bypass_force_ctrl <= id_bypass_force_ctrl;
  end
end

always_ff @(posedge clock_aon) begin
  ex_regfile_thread_id <= id_regfile_thread_id;
  ex_mova_mx <= id_core_valid_int && (id_vpu_ctrl.sigs.cmd==VPU_FCMD_MOVA_M_X);
end


  // zero mask info
always_ff @(posedge clock_sec) begin
  if (f3_tena_regfile_wen_l) begin
    ex_tensora_zero_mask <= ex_tensora_zero_mask_next;
  end
  if (f3_tentmp_wen) begin
    ex_tensortmp_zero_mask <= ex_tensortmp_zero_mask_next;
  end
  if (f3_tenb_regfile_wen_l | dcache_scp_resp.fill_is_tenb) begin
    ex_tensorb_zero_mask <= ex_tensorb_zero_mask_next;
  end
end
...
assign ex_ra1_ten = ex_ra1[VPU_TENA_ADDR_W-1:0];
assign ex_ra2_ten = ex_ra2[VPU_TENB_ADDR_W-1:0];
...
begin
  for(integer l = 0; l < VPU_LANE_NUM; l++)
  begin
    for(integer t = 0; t < VPU_NUM_TIMA; t++)
    begin
...
always_comb begin
  ...
  if (ex_tensorfma_en && (&ex_txfma_valid)) begin
...
assign ex_use_prev_sigs = ex_tensorfma_en & (ex_req.sigs == f2_req.sigs) & f2_txfma_valid_all_en;
  ...
  // Store data lane enable register
...
always_ff @(posedge clock_sec) begin
  if (reset) begin
    f2_ctrl_valid_qual <= 1'b0;
    f2_ctrl_valid <= 1'b0;
    f2_ctrl_killed_prev <= 1'b0;
    f2_txfma_valid_all_en <= 1'b0;
    f2_wen <= 1'b0;
  end else begin
    f2_ctrl_valid_qual <= (ex_ctrl_valid_qual && (!ex_core_kill || ex_ml_inst_val || ex_req.sigs.trans));
    f2_ctrl_valid <= ex_ctrl_valid_kill;
    f2_ctrl_killed_prev <= ex_ctrl_killed;
    f2_txfma_valid_all_en <= ex_txfma_valid_all_en;
    f2_wen <= (ex_wen && (!ex_core_kill || ex_ml_inst_val || ex_req.sigs.trans));
  end
end

always_ff @(posedge clock_sec) begin
  f2_ml_inst_val <= ex_ml_inst_val;
  f2_mova_mx <= ex_mova_mx;
end

always_ff @(posedge clock_sec) begin
  if (ex_ctrl_valid_kill) begin
    f2_regfile_waddr <= ex_core_inst[VPU_INST_RD_SEL_MSB:VPU_INST_RD_SEL_LSB];
    f2_ml_mask <= ex_ml_mask_to_f2;
    f2_thread_id <= (ex_thread_id_int && !ex_ml_inst_val);
    f2_ml_use_prev_sigs <= ex_use_prev_sigs;
    f2_lane_idx <= ex_lane_idx;
  end
end

// Some signals in "f2_req.sigs" are used to stall, so ensure they are properly reset here and in previous stages
always_ff @(posedge clock_sec) begin
  if (reset) begin
    f2_req <= '0;
  end else if (ex_ctrl_valid_kill && !ex_use_prev_sigs) begin
    f2_req <= ex_req;
  end
end

always_ff @(posedge clock_sec) begin
  if (ex_ctrl_valid_kill && !ex_use_prev_sigs && ex_store_data_lane_en) begin
    f2_gsc_src <= ex_core_gsc_src;
    f2_store_data_lane <= ex_store_data_lane;
  end
end
...
always_ff @(posedge clock_sec) begin
  if (reset) begin
    f3_ctrl_valid_qual <= 1'b0;
    f3_ctrl_valid <= 1'b0;
    f3_ctrl_killed_prev <= 1'b0;
    f3_ml_inst_val <= 1'b0;
    f3_wen <= 1'b0;
  end else begin
    f3_ctrl_valid_qual <= (f2_ctrl_valid_qual & !f2_kill);
    f3_ctrl_valid <= f2_ctrl_valid;
    f3_ctrl_killed_prev <= f2_ctrl_killed;
    f3_ml_inst_val <= f2_ml_inst_val;
    f3_wen <= (f2_wen & !f2_kill);
  end
end

always_ff @(posedge clock_sec) begin
  f3_thread_id <= f2_thread_id;
  f3_mova_mx <= f2_mova_mx;
  f3_regfile_wmask <= f2_regfile_wmask_to_f3;
end

always_ff @(posedge clock_sec) begin
  if (f2_ctrl_valid_qual) begin
    f3_ml_use_prev_sigs <= f2_ml_use_prev_sigs;
    f3_lane_idx <= f2_lane_idx;
  end
  if (f2_ctrl_valid) begin
    f3_regfile_waddr <= f2_regfile_waddr;
  end
end
...
always_ff @(posedge clock_sec) begin
  if (reset) begin
    f3_ctrl_sigs <= '0;
  end else if (f2_ctrl_valid && !f2_ml_use_prev_sigs) begin
    f3_ctrl_sigs <= f2_req.sigs;
  end
end
...
begin
  ...
  if (wb_dmem_resp_val_masked) begin
    if (wb_dmem_resp_is_gather) begin
      ...
      f3_regfile_wdata_l = (wb_dmem_resp.typ == minion_pkg::DcTypePsBr) ?
          {VPU_LANE_NUM{wb_dmem_resp.data[VPU_DATA_S_SZ-1:0]}} : wb_dmem_resp.data;
    end
...
begin
  ...
  if (f3_tena_regfile_wen_l)
      f3_tena_regfile_waddr_l = id_tensorfma_load_ctrl.waddr[VPU_TENA_ADDR_W-1:0];

  if (f3_tenb_regfile_wen_l)
      f3_tenb_regfile_waddr_l = id_tensorfma_load_ctrl.waddr[VPU_TENB_ADDR_W-1:0];
...
// zero data detection
always_comb begin
  f3_zero_data_a = {VPU_LANE_NUM{(f3_tena_regfile_wdata_l == '0)}};
  f3_zero_data_tmp = {VPU_LANE_NUM{(f3_tentmp_msb_regfile_wdata_l == '0)}};
  for (int i=0; i<VPU_LANE_NUM; i=i+1) begin
    f3_zero_data_b[i] = (f3_tenb_regfile_wdata_l[VPU_DATA_S_SZ*i +: VPU_DATA_S_SZ] == '0);
  end
end
...
always_ff @(posedge clock_sec) begin
  if (reset) begin
    f4_ctrl_sigs <= '0;
    f4_ctrl_valid_qual <= 1'b0;
    f4_wen <= 1'b0;
    f4_ctrl_killed_prev <= 1'b0;
    f4_ctrl_valid <= 1'b0;
    f4_ml_inst_val <= 1'b0;
  end else begin
    if (f3_ctrl_valid && !f3_ml_use_prev_sigs) begin
      f4_ctrl_sigs <= f3_ctrl_sigs_to_f4;
    end
    f4_ctrl_valid_qual <= (f3_ctrl_valid_qual & (!f3_core_kill | f3_ml_inst_val | f3_ctrl_sigs_to_f4.trans));
    f4_wen <= (f3_wen & (!f3_core_kill | f3_ml_inst_val | f3_ctrl_sigs_to_f4.trans));
    f4_ctrl_killed_prev <= f3_ctrl_killed;
    f4_ctrl_valid <= f3_ctrl_valid;
    f4_ml_inst_val <= f3_ml_inst_val;
  end
end

always_ff @(posedge clock_sec) begin
  if (f3_ctrl_valid_qual) begin
    f4_ml_use_prev_sigs <= f3_ml_use_prev_sigs;
    f4_lane_idx <= f3_lane_idx;
  end
  if (f3_wen) begin
    f4_regfile_waddr <= f3_regfile_waddr;
  end
  if (|f3_regfile_wmask_l) begin
    f4_regfile_thrid_l <= f3_regfile_thrid_l;
    f4_regfile_waddr_l <= f3_regfile_waddr_l;
    f4_regfile_wdata_l <= f3_regfile_wdata_l;
  end
  f4_regfile_wen_l <= f3_regfile_wen_l;
  f4_regfile_wmask_l <= f3_regfile_wmask_l;
  f4_regfile_wmask <= f3_regfile_wmask_to_f4;
  f4_thread_id <= f3_thread_id;
  f4_mova_mx <= f3_mova_mx;
end
...
always_ff @(posedge clock_sec) begin
  if (reset) begin
    f5_ctrl_valid_qual <= 1'b0;
    f5_ctrl_killed <= 1'b0;
    f5_wen <= 1'b0;
    f5_ml_inst_val <= 1'b0;
    f5_ctrl_sigs <= '0;
  end else begin
    f5_ctrl_valid_qual <= f4_ctrl_valid_qual;
    f5_ctrl_killed <= f4_ctrl_killed;
    f5_wen <= (f4_wen & (!f4_core_kill | f4_ml_inst_val | f4_ctrl_sigs.trans));
    f5_ml_inst_val <= f4_ml_inst_val;
    if (f4_ctrl_valid_qual && !f4_ml_use_prev_sigs) begin
      f5_ctrl_sigs <= f4_ctrl_sigs;
    end
  end
end

always_ff @(posedge clock_sec) begin
  f5_regfile_wmask <= f4_regfile_wmask_to_f5;
  f5_thread_id <= f4_thread_id;
  f5_mova_mx <= f4_mova_mx;
  if (f4_ctrl_valid_qual) begin
    f5_ml_use_prev_sigs <= f4_ml_use_prev_sigs;
    f5_lane_idx <= f4_lane_idx;
    f5_core_kill <= f4_core_kill;
  end
  if (f4_wen) begin
    f5_regfile_waddr <= f4_regfile_waddr;
  end
end
...
always_ff @(posedge clock_sec) begin
  if (reset) begin
    f6_ctrl_valid_qual <= 1'b0;
    f6_ctrl_killed <= 1'b0;
    f6_wen <= 1'b0;
    f6_ml_inst_val <= 1'b0;
    f6_ctrl_sigs <= '0;
  end else begin
    f6_ctrl_valid_qual <= f5_ctrl_valid_qual;
    f6_ctrl_killed <= f5_ctrl_killed;
    f6_wen <= (f5_wen & (!f5_core_kill | f5_ml_inst_val | f5_ctrl_sigs.trans));
    f6_ml_inst_val <= f5_ml_inst_val;
    if (f5_ctrl_valid_qual && !f5_ml_use_prev_sigs) begin
      f6_ctrl_sigs <= f5_ctrl_sigs;
    end
  end
end

always_ff @(posedge clock_sec) begin
  f6_regfile_wmask <= f5_regfile_wmask;
  f6_thread_id <= f5_thread_id;
  if (f5_ctrl_valid_qual) begin
    f6_ml_use_prev_sigs <= f5_ml_use_prev_sigs;
    f6_core_kill <= f5_core_kill;
    f6_lane_idx <= f5_lane_idx;
  end
  if (f5_wen) begin
    f6_regfile_waddr <= f5_regfile_waddr;
  end
end
...
always_ff @(posedge clock_sec) begin
  if (reset) begin
    f7_ctrl_valid_qual <= 1'b0;
    f7_ctrl_killed <= 1'b0;
    f7_wen <= 1'b0;
    f7_ctrl_sigs <= '0;
  end else begin
    f7_ctrl_valid_qual <= f6_ctrl_valid_qual;
    f7_ctrl_killed <= f6_ctrl_killed;
    f7_wen <= (f6_wen & (!f6_core_kill | f6_ml_inst_val | f6_ctrl_sigs.trans));
    if (f6_ctrl_valid_qual && !f6_ml_use_prev_sigs) begin
      f7_ctrl_sigs <= f6_ctrl_sigs;
    end
  end
end

always_ff @(posedge clock_sec) begin
  f7_regfile_wmask <= f6_regfile_wmask_to_f7;
  f7_thread_id <= f6_thread_id_to_f7;
  if (f6_ctrl_valid_qual) begin
    f7_ml_use_prev_sigs <= f6_ml_use_prev_sigs;
    f7_lane_idx <= f6_lane_idx;
  end
  if (f6_wen) begin
    f7_regfile_waddr <= f6_regfile_waddr;
  end
end

assign f7_regfile_wen_orig = f7_regfile_wmask & {VPU_LANE_NUM{f7_ctrl_valid_qual}};
// Used for early RF write enable
assign f7_regfile_wen      = f7_regfile_wen_orig;
...
always_ff @(posedge clock_sec) begin
  if (reset) begin
    f8_ctrl_sigs <= '0;
    f8_ctrl_valid_qual <= 1'b0;
    f8_ctrl_killed <= 1'b0;
    f8_wen <= 1'b0;
    f8_regfile_wmask <= '0;
    f8_thread_id <= '0;
  end else begin
    if (f7_ctrl_valid_qual && !f7_ml_use_prev_sigs) begin
      f8_ctrl_sigs <= f7_ctrl_sigs;
    end
    f8_ctrl_valid_qual <= f7_ctrl_valid_qual;
    f8_ctrl_killed <= f7_ctrl_killed;
    f8_wen <= f7_wen;
    f8_regfile_wmask <= f7_regfile_wen_orig;
    f8_thread_id <= f7_thread_id;
  end
end

always_ff @(posedge clock_sec) begin
  if (f7_ctrl_valid_qual) begin
    f8_regfile_waddr <= f7_regfile_waddr;
    f8_lane_idx <= f7_lane_idx;
  end
end
...
assign wb_dmem_resp_is_gather = minion_dcache_pkg::dcache_type_is_gsc(
    minion_dcache_pkg::dcache_type_e'(wb_dmem_resp.typ));
...
always_comb begin
  io_events = '0;
  io_events[VPU_EVENT_TFMA_WAIT_TENB] = io_event_wait_tenb;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  (
      ...
      .f2_tima_tenc_wen         ( f2_tima_tenc_wen                     ),
      .f2_tima_rf_wen           ( f2_tima_rf_wen                       ),
      .f3_tima_tenc_wen         ( f3_tima_tenc_wen                     ),
      .f3_tima_tenc_waddr       ( f3_tima_tenc_waddr                   ),
      .f3_tima_rf_wen           ( f3_tima_rf_wen                       ),
      .io_events_wait_tenb      ( io_event_wait_tenb                  ),
...
////////////////////////////////////////////////////////////////////////////////
  ...
  (
      ...
      .id_valid_i                ( id_core_valid_int                 ),
      .id_core_thread_id_i       ( id_core_thread_id                 ),
      .id_instruction_i          ( id_core_inst_int                  ),
      .ex_valid_i                ( ex_ctrl_valid                     ),
      .ex_core_thread_id_i       ( ex_thread_id                      ),
      .ex_instruction_i          ( ex_core_inst                      ),
      .ex_mask_rf0_i             ( ex_mask_rf0                       ),
      // Chicken bit vputrans clock gate enable
      .chicken_bit_vputrans_i    ( chicken_bit_vputrans              ),
      // Kill
      .f6_killed_i               ( f6_ctrl_killed                    ),
      .f7_killed_i               ( f7_ctrl_killed                    ),
      .f8_killed_i               ( f8_ctrl_killed                    ),
      // Output control
      .id_insert_en_next_o       ( id_trans_insert_en_next           ),
      .id_insert_inst_next_o     ( id_trans_insert_inst_next         ),
      .id_insert_en_o            ( id_trans_insert_en                ),
      .id_trans_thread_id_o      ( id_trans_thread_id                ),
      .ex_trans_thread_id_o      ( ex_trans_thread_id                ),
      .f6_trans_ctrl_en_o        ( f6_trans_wen_ctrl_en              ),
      .f6_trans_wmask_o          ( f6_trans_wmask                    ),
      .f6_trans_thread_id_o      ( f6_trans_thread_id                ),
      .f8_trans_fcsr_en_o        ( f8_trans_fcsr_en                  ),
      .f8_trans_ctrl_en_o        ( f8_trans_wen_ctrl_en              ),
      .id_trans_busy_o           ( id_trans_busy                     ),
      .id_trans_scoreboard_o     ( id_trans_scoreboard_slots         ),
      .state_o                   ( trans_st                          )
  );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  (
      ...
      .clk_i       ( clock_sec               ),
      // Read ports
      .rd_addr_i   ( ex_tena_raddr           ),
      .rd_data_o   ( ex_tena_rf_rdata_pre    ),
      // Write ports
      .wr_en_early_i ( f2_tena_regfile_wen_l   ),
      .wr_en_i     ( f3_tena_regfile_wen_l   ),
      .wr_addr_i   ( f3_tena_regfile_waddr_l ),
      .wr_data_i   ( f3_tena_regfile_wdata_l )
  );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  (
      ...
      .clk_i       ( clock_sec                     ),
      // Read ports
      .rd_addr_i   ( ex_tentmp_raddr               ),
      .rd_data_o   ( ex_tentmp_rf_rdata            ),
      // Write ports
      .wr_en_early_i ( f2_tentmp_wen_early           ),
      .wr_en_i     ( f3_tentmp_wen                 ),
      .wr_addr_i   ( f3_tentmp_waddr               ),
      .wr_data_i   ( f3_tentmp_msb_regfile_wdata_l )  // Always write 32MSB of 64b chunk
  );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  (
    ...
    .ex_in_valid_i         ( ex_mask_valid                                      ),
    .ex_inst_valid_i       ( ex_ctrl_valid_kill                                 ),
    .ex_regfile_raddr1_i   ( ex_regfile_raddr1[$clog2(VPU_REGMASK_NUM)-1:0]    ),
    .ex_regfile_raddr2_i   ( ex_regfile_raddr2[$clog2(VPU_REGMASK_NUM)-1:0]    ),
    .ex_cmd_i              ( ex_req.sigs.cmd                                    ),
    .ex_imm_i              ( ex_req.imm[VPU_LANE_NUM-1:0]                      ),
    .ex_fromint_i          ( ex_req.sigs.fromint                                ),
    .ex_fromint_data_i     ( ex_core_fromint_data                               ),
    .ex_thread_id_i        ( ex_thread_id_int && !ex_ml_inst_val                ),
    .ex_ignore_mask_i      ( ex_ignore_mask                                     ),
      // F2 inputs
    .f2_thread_id_i        ( f2_thread_id                                       ),
    .f2_wen_i              ( f2_mask_wen                                        ),
    .f2_core_kill_i        ( f2_core_kill                                       ),
    .f2_waddr_i            ( f2_regfile_waddr[$clog2(VPU_REGMASK_NUM)-1:0]     ),
    .f2_maskop_i           ( f2_req.sigs.maskop                                 ),
    .f2_tointm_i           ( f2_req.sigs.tointm                                 ),
      // F3 inputs
    .f3_thread_id_i        ( f3_thread_id                                       ),
    .f3_regmask_from_short_i( f3_ctrl_sigs.tointm & f3_ctrl_sigs.shsw           ),
    .f3_regmask_from_txfma_i( f3_ctrl_sigs.tointm & f3_ctrl_sigs.txfma          ),
    .f3_core_kill_i        ( f3_core_kill                                       ),
    .f3_regmask_wdata_lane_i( f3_regmask_wdata_lane                             ),
      // F4 inputs
    .f4_regmask_from_txfma_i( f4_ctrl_sigs.tointm & f4_ctrl_sigs.txfma          ),
    ...
    .f8_regmask_comp_res_i ( f8_regmask_comp_res                                ),
      // EX/F1 outputs
    .ex_mask_rf0_o         ( ex_mask_rf0                                        ),
    .ex_mask_in1_o         ( ex_mask_in1                                        ),
      // F2 ouputs
    .f2_mask_rf0_o         ( f2_mask_rf0                                        ),
      // F8 outputs
    .f8_toint_data_o       ( f8_toint_mask_rf                                   ),
    .f8_regmask_store_o    ( f8_regmask_store                                   ),
      // WB outputs
    .wb_mask_valid_o       ( id_scoreboard_mask_valid                           ),
    .wb_mask_dest_o        ( id_scoreboard_mask_dest                            )
  );
...
always_comb begin
 ...
 vpu_dbg_data[1][109:0]   = {ex_bypass_force_ctrl,          //5
                            trans_st,                       //25
                            txfma_trans_dbg,                //16
 ...
 vpu_dbg_data[2][126:0]   = {dcache_scp_resp,                     //7
 ...
 vpu_dbg_data[3][111:0] = {f0_core_ctrl.reduce_ctrl.tensorstore.src_inc,   //2
 ...
 vpu_dbg_data[4][100:0] = {f0_core_ctrl.reduce_ctrl.tensorstore_scp.stride_entry, //2
 ...
 vpu_dbg_filter[151:64] = {wb_core_ctrl.fcsr_flags, //6
