...
void run_cycle(Sim& sim, LaneModel& model, const LaneInputs& in, const std::string& label) {
    ...
    model.low_phase(in);
    sim.low();
    sim.check(sim.dut->tenb_out_o == model.tenb_out(in.tenb_rd_addr), label + " low tenb_out");
    sim.check(sim.dut->tima_out_o == model.tima_out(), label + " low tima_out");
    model.high_phase(in);
    sim.high();
    sim.check(sim.dut->tenb_out_o == model.tenb_out(in.tenb_rd_addr), label + " high tenb_out");
    sim.check(sim.dut->tima_out_o == model.tima_out(), label + " high tima_out");
}
...
int main(int argc, char** argv) {
    ...
    in = {};
    in.tima_clk_en = false;
    in.f1_tima_mul_en = true;
    in.tena_rd_data = 0xffffffffu;
    in.tenb_rd_addr = 1;
    run_cycle(sim, model, in, "gated tima clock holds state");
    ...
    in.tenc_wr_addr = 3;
    in.tenc_rd_addr = 3;
    run_cycle(sim, model, in, "write tensorc with prior tima output");
    ...
    std::array<uint32_t, 2> seeds = {0x5a1e0f33u, 0x94b1c2d7u};
