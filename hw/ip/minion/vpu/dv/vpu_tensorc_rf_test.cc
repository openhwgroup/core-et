...
int main(int argc, char** argv) {
    ...
    sim.dut->wr_addr_i = 15;
    sim.dut->wr_data_i = 0x33333333u;
    sim.dut->wr_en_early_i = 1;
    sim.dut->wr_en_i = 0;
    sim.tick();
    sim.dut->wr_en_early_i = 0;
    sim.check(read_addr(sim, 15) == 0x22222222u, "wr_en low blocks write");
    ...
    sim.dut->wr_en_early_i = 0;
    sim.dut->wr_en_i = 0;
    sim.check(read_addr(sim, 7) == 0x44444444u, "write uses low-phase captured data");
    ...
    sim.tick();
    sim.dut->wr_en_i = 0;
    sim.check(read_addr(sim, 3) == 0x44444444u, "missing early strobe reuses stale latched data");
