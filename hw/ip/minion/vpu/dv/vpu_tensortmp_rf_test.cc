...
#include "Vvpu_tensortmp_rf_tb.h"
#include "verilated.h"
...
using DUT = Vvpu_tensortmp_rf_tb;
...
int main(int argc, char** argv) {
    ...
    sim.tick();
    sim.dut->wr_en_early_i = 0;
    sim.check(read_addr(sim, 7) == 0x22222222u, "wr_en low blocks write");
    ...
    sim.dut->wr_en_early_i = 0;
    sim.dut->wr_en_i = 0;
    sim.check(read_addr(sim, 5) == 0x44444444u, "write uses low-phase captured data");
    ...
    sim.tick();
    sim.dut->wr_en_i = 0;
    sim.check(read_addr(sim, 2) == 0x44444444u, "missing early strobe reuses stale latched data");
