...
// ECP5 implementation.
//
// Preserve the generic/original overlapping-read contract while giving Yosys a
// distributed-RAM storage candidate for the write array.
...
module prim_rf_1r1w_diff #(
  ...
  if (!Ecp5SupportedSize) begin : gen_unsupported_size
    initial begin
      $error("Unsupported ECP5 prim_rf_1r1w_diff size: RWidth=%0d RAlignment=%0d WWidth=%0d Entries=%0d",
             RWidth, RAlignment, WWidth, Entries);
      $finish;
    end
    prim_rf_1r1w_diff_ecp5_unsupported_size u_compile_time_error();
  end
