//
// This module is a wrapper for the west-side gate-level memory shire.  It is used to get the
// auto-init module to work on gate netlists.
//
`include "soc.vh"

`ifdef MEMSHIRE_AUTO_INIT_ENABLE
   `ifdef DV_MEMSHIRE_WEST_SIDE_USE_GATE
      `define ET_MS_AI_WEST_TOP_PATH       memshire_west_top_memshire_top_memshire_top
      `define ET_MS_AI_WEST_DEBUG_PATH     `ET_MS_AI_WEST_TOP_PATH.ms_debug_g_ms_debug
      `define ET_MS_AI_WEST_MEM_LOGIC_PATH `ET_MS_AI_WEST_TOP_PATH.mem_logic
   `endif // ifdef DV_MEMSHIRE_WEST_SIDE_USE_GATE
`endif // ifdef MEMSHIRE_AUTO_INIT_ENABLE

`ifndef _ET_MS_AUTO_INIT_WEST_WRAPPER
   `define _ET_MS_AUTO_INIT_WEST_WRAPPER

   `ifdef ET_MS_AI_WEST_TOP_PATH

      module ms_auto_init_west_wrapper (
         ...
         assign ai_apb_prdata            = (ms_auto_init_inprog     === 1'b1)  ? `ET_MS_AI_WEST_TOP_PATH.bpam_apb_prdata :
                                                                                 {(`MEMSHIRE_BPAM_APB_DATA_WIDTH){1'b0}};
         assign ai_apb_pready            = (ms_auto_init_inprog     === 1'b1)  ? `ET_MS_AI_WEST_TOP_PATH.bpam_apb_pready : 1'b0;
         ...
         initial begin
            ...
            force `ET_MS_AI_WEST_TOP_PATH.bpam_apb_paddr[30:2]                     = ai_apb_paddr[30:2];
            force `ET_MS_AI_WEST_TOP_PATH.bpam_apb_psel                            = ai_apb_psel;
            force `ET_MS_AI_WEST_TOP_PATH.bpam_apb_pwdata                          = ai_apb_pwdata;
            force `ET_MS_AI_WEST_TOP_PATH.bpam_apb_pwrite                          = ai_apb_pwrite;
            force `ET_MS_AI_WEST_DEBUG_PATH.reset                                  = 1'b1;
            force `ET_MS_AI_WEST_MEM_LOGIC_PATH.mem_mesh_slave_AR_fifo_valid_fifo  = 1'b0;
            force `ET_MS_AI_WEST_MEM_LOGIC_PATH.mem_mesh_slave_AWW_fifo_valid_fifo = 1'b0;
            ...
            @(posedge clk_ms_pll);
            @(negedge clk_ms_pll);
            release `ET_MS_AI_WEST_TOP_PATH.bpam_apb_paddr;
            release `ET_MS_AI_WEST_TOP_PATH.bpam_apb_psel;
            release `ET_MS_AI_WEST_TOP_PATH.bpam_apb_pwdata;
            release `ET_MS_AI_WEST_TOP_PATH.bpam_apb_pwrite;
            release `ET_MS_AI_WEST_DEBUG_PATH.reset;
            release `ET_MS_AI_WEST_MEM_LOGIC_PATH.mem_mesh_slave_AR_fifo_valid_fifo;
            release `ET_MS_AI_WEST_MEM_LOGIC_PATH.mem_mesh_slave_AWW_fifo_valid_fifo;
         end
   ...
   `endif // ifdef ET_MS_AI_WEST_TOP_PATH
`endif // ifndef _ET_MS_AUTO_INIT_WEST_WRAPPER
