...
#include "Vcosim_prim_mul_div_tb.h"
#include "cosim_ctrl.h"

#include <cstdint>
#include <cstdio>

using DUT = Vcosim_prim_mul_div_tb;
...
static int issue_and_wait(CosimCtrl<DUT>& sim, uint8_t fn, uint8_t dw,
    ...
    while (cycles < max_cycles) {
        ...
        compare_outputs(sim, desc);
        cycles++;
        if (sim.dut->orig_resp_valid && sim.dut->new_resp_valid) break;
    }

    if (cycles >= max_cycles) {
        std::printf("  TIMEOUT waiting for resp_valid: %s\n", desc);
    }
...
int main(int argc, char** argv) {
    ...
    for (uint8_t dw = 0; dw <= 1; dw++) {
        for (auto fn : mul_fns) {
            for (auto& op : mul_operands) {
                std::snprintf(desc, sizeof(desc), "p1_dw%d_%s_%s",
    ...
    for (uint8_t dw = 0; dw <= 1; dw++) {
        for (auto fn : div_fns) {
            for (auto& op : div_operands) {
                std::snprintf(desc, sizeof(desc), "p2_dw%d_%s_%s",
                              dw ? 64 : 32, fn_name(fn), op.name);
    ...
    for (int trial = 0; trial < 4; trial++) {
        ...
        uint8_t dw = trial & 1;
        std::snprintf(desc, sizeof(desc), "p3_kill_%s_dw%d", fn_name(fn), dw ? 64 : 32);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            sim.tick();
            compare_outputs(sim, desc);
            if (sim.dut->orig_req_ready && sim.dut->new_req_ready) break;
        }

        std::snprintf(desc, sizeof(desc), "p3_recover_%d", trial);
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        std::snprintf(desc, sizeof(desc), "p4_rand_%d", i);
        issue_and_wait(sim, fn, dw, in1, in2, dest, desc);

        if (sim.mismatches > 50) {
            std::printf("  Too many mismatches, stopping phase 4 early at iteration %d\n", i);
            break;
        }
    }
    ...
    for (int op_idx = 0; op_idx < 10; op_idx++) {
        ...
        std::snprintf(desc, sizeof(desc), "p5_b2b_%d_%s_dw%d",
