...
//
// Co-simulation test: prim_cmp_32 (new) vs r32cmp (original).
//
// Tests 3:2 compressor at Width=1, Width=8, Width=68 with 2M random
// vectors per width variant.

#include "Vcosim_prim_cmp_32_tb.h"
#include "cosim_ctrl.h"
#include <cstdlib>
#include <cstdint>
#include <cstdio>

using DUT = Vcosim_prim_cmp_32_tb;
...
int main(int argc, char** argv) {
    ...
    // ================================================================
    // Width=1: exhaustive (8 combinations) then random padding
    // ================================================================
    ...
    for (int i = 0; i < 2000000; i++) {
        ...
        uint64_t orig_sv_lo = sim.dut->orig_w68_sv_lo;
        uint64_t new_sv_lo  = sim.dut->new_w68_sv_lo;
        uint8_t  orig_sv_hi = sim.dut->orig_w68_sv_hi;
        uint8_t  new_sv_hi  = sim.dut->new_w68_sv_hi;
        uint64_t orig_cv_lo = sim.dut->orig_w68_cv_lo;
        uint64_t new_cv_lo  = sim.dut->new_w68_cv_lo;
        uint8_t  orig_cv_hi = sim.dut->orig_w68_cv_hi;
        uint8_t  new_cv_hi  = sim.dut->new_w68_cv_hi;

        sim.compare("w68_sv_lo", orig_sv_lo, new_sv_lo);
        sim.compare("w68_sv_hi", orig_sv_hi, new_sv_hi);
        sim.compare("w68_cv_lo", orig_cv_lo, new_cv_lo);
        sim.compare("w68_cv_hi", orig_cv_hi, new_cv_hi);
        if (sim.mismatches > 50) break;
    }
