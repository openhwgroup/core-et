...
//
// Co-simulation: dcache_128x72_1r1w_lram vs minion_dcache_128x72_1r1w_lram.

#include "Vcosim_minion_dcache_128x72_1r1w_lram_tb.h"
...
using DUT = Vcosim_minion_dcache_128x72_1r1w_lram_tb;
...
void tick_and_compare(CosimCtrl<DUT>& sim) {
    sim.dut->eval();
    sim.tick();
    sim.compare("rd_data_lo", sim.dut->orig_rd_data_lo_o, sim.dut->new_rd_data_lo_o);
    sim.compare("rd_data_hi", sim.dut->orig_rd_data_hi_o, sim.dut->new_rd_data_hi_o);
}

void write_one_phase_early(CosimCtrl<DUT>& sim, uint32_t addr, uint64_t data_lo, uint64_t data_hi) {
    sim.dut->wr_addr_i = addr;
    sim.dut->wr_data_lo_i = data_lo & ((1ULL << 36) - 1ULL);
    sim.dut->wr_data_hi_i = data_hi & ((1ULL << 36) - 1ULL);
...
void init_array(CosimCtrl<DUT>& sim) {
    ...
    for (uint32_t i = 0; i < 128; ++i) {
        ...
        sim.dut->wr_enablehi_i = 0;
        sim.dut->wr_addr_i = i;
        sim.dut->wr_data_lo_i = (0x100000000ULL + i) & ((1ULL << 36) - 1ULL);
        sim.dut->wr_data_hi_i = (0x200000000ULL + i) & ((1ULL << 36) - 1ULL);
        tick_only(sim);
...
int main(int argc, char** argv) {
    ...
    sim.dut->wr_enablelo_pre_i = 1;
    sim.dut->wr_enablehi_pre_i = 1;
    sim.dut->wr_data_lo_i = 0x123456789ULL;
    sim.dut->wr_data_hi_i = 0x0ABCDEF01ULL;
    tick_and_compare(sim);
    ...
    sim.dut->rd_enable_i = 0;
    write_one_phase_early(sim, 9, 0x123456789ULL, 0x0FEDCBA98ULL);
    sim.dut->rd_enable_i = 1;
    sim.dut->rd_addr_i = 9;
    tick_and_compare(sim);
    ...
    for (int i = 0; i < 4096; ++i) {
        ...
        sim.dut->wr_data_lo_i = xorshift32(seed) & ((1ULL << 36) - 1ULL);
        sim.dut->wr_data_hi_i = xorshift32(seed) & ((1ULL << 36) - 1ULL);
        tick_and_compare(sim);
    }
