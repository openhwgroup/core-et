...
#include "Vcosim_uncacheable_noc_tol3_handler_tb.h"
...
static void compare_words(CosimCtrl<Vcosim_uncacheable_noc_tol3_handler_tb>& sim,
...
int main(int argc, char** argv) {
    CosimCtrl<Vcosim_uncacheable_noc_tol3_handler_tb> sim(argc, argv);
    ...
    for (int i = 0; i < 4096; ++i) {
        ...
        sim.dut->l3_enabled_i = (r >> 2) & 1;
        sim.dut->remote_scp_enabled_i = (r >> 3) & 1;
        sim.dut->to_l3_b_valid_i = (r >> 4) & 1;
        sim.dut->to_l3_r_valid_i = (r >> 5) & 1;
        // Sweep response codes and low ID bits.
        sim.dut->to_l3_b_bits_i = (sim.dut->to_l3_b_bits_i & ~0xffu) | (r & 0xffu);
        sim.dut->to_l3_r_bits_i[0] = (sim.dut->to_l3_r_bits_i[0] & ~0xffu) | ((r >> 1) & 0xffu);
        sim.tick();
