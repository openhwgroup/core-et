...
//
// Co-simulation test: original rbox_fp_adder_2stg vs new rbox_fp_adder_2stg.
//
// This is the most critical FP module — a 2-stage pipelined adder with
// LZA normalization. We drive both modules with identical stimulus and
// compare all output fields after the pipeline latency.

#include "Vcosim_rbox_fp_adder_2stg_tb.h"
#include "cosim_ctrl.h"

using DUT = Vcosim_rbox_fp_adder_2stg_tb;
...
int main(int argc, char** argv) {
    ...
    // Pipeline is 2 stages: set inputs + valid on cycle N, compare output on cycle N+2
    // We tick with valid=1 to load, then valid=0 and tick to get output.

    // ── Test 1: Zero + Zero ─────────────────────────────
    printf("cosim test 1: zero + zero\n");
    set_operands(sim, make_zero(), make_zero());
    sim.dut->valid_i = 1;
    sim.tick();  // stage 1 captured
    sim.dut->valid_i = 1;  // keep valid for stage 2
    tick_and_compare(sim);  // output available
