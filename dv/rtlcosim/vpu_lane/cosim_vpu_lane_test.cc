...
int main(int argc, char** argv) {
  ...
  uint32_t seed = 0x6c6e5a17u;
  for (int cycle = 0; cycle < 130000; ++cycle) {
  ...
  sim.check(full_coverage(input_ones, input_zeros),
            "vpu_lane aggregate input bus drives every bit to both 0 and 1");
