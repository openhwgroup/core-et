...
//
// Co-simulation: dcache_miss_handler vs minion_dcache_miss_handler.

#include "Vcosim_minion_dcache_miss_handler_tb.h"
#include "cosim_ctrl.h"

#include <cstdint>
#include <string>

using DUT = Vcosim_minion_dcache_miss_handler_tb;
...
struct Coverage {
  BoolCoverage req_pri_val, req_sec_val, req_tag_match, req_cacheable, req_thread_id;
  BoolCoverage req_mem_g, req_is_amo, req_is_msg, req_is_bypass;
  BoolCoverage inflight0_valid, inflight1_valid, inflight_reduce;
  ...
  NonZeroCoverage req_old_meta_state, req_old_meta_tag;
  NonZeroCoverage inflight0_way_en, inflight0_set, inflight1_way_en, inflight1_set;
  NonZeroCoverage l2_resp_id, l2_resp_opcode;
};
...
void sample_coverage(DUT* dut, Coverage& cov) {
  cov.req_pri_val.sample(dut->req_pri_val_i);
  cov.req_sec_val.sample(dut->req_sec_val_i);
  ...
  cov.inflight0_valid.sample(dut->inflight0_valid_i);
  cov.inflight1_valid.sample(dut->inflight1_valid_i);
  ...
  cov.inflight0_way_en.sample(dut->inflight0_way_en_i);
  cov.inflight0_set.sample(dut->inflight0_set_i);
  cov.inflight1_way_en.sample(dut->inflight1_way_en_i);
  cov.inflight1_set.sample(dut->inflight1_set_i);
...
void check_coverage(CosimCtrl<DUT>& sim, const Coverage& cov) {
  sim.check(cov.req_pri_val.full(), "coverage: req_pri_val exercised 0/1");
  sim.check(cov.req_sec_val.full(), "coverage: req_sec_val exercised 0/1");
  ...
  sim.check(cov.inflight0_valid.full(), "coverage: inflight0_valid exercised 0/1");
  sim.check(cov.inflight1_valid.full(), "coverage: inflight1_valid exercised 0/1");
  ...
  sim.check(cov.inflight0_way_en.full(), "coverage: inflight0_way_en exercised zero/nonzero");
  sim.check(cov.inflight0_set.full(), "coverage: inflight0_set exercised zero/nonzero");
  sim.check(cov.inflight1_way_en.full(), "coverage: inflight1_way_en exercised zero/nonzero");
  sim.check(cov.inflight1_set.full(), "coverage: inflight1_set exercised zero/nonzero");
...
void compare_outputs(CosimCtrl<DUT>& sim) {
  sim.compare("req_pri_rdy", sim.dut->orig_req_pri_rdy_o, sim.dut->new_req_pri_rdy_o);
  sim.compare("req_sec_rdy", sim.dut->orig_req_sec_rdy_o, sim.dut->new_req_sec_rdy_o);
  sim.compare("can_accept_uc", sim.dut->orig_can_accept_uc_o, sim.dut->new_can_accept_uc_o);
  sim.compare("tag_match", sim.dut->orig_tag_match_o, sim.dut->new_tag_match_o);
  sim.compare("evict_match", sim.dut->orig_evict_match_o, sim.dut->new_evict_match_o);
  sim.compare("set", sim.dut->orig_set_o, sim.dut->new_set_o);
  ...
  sim.compare("have_l2_to_ba_resp", sim.dut->orig_have_l2_to_ba_resp_o, sim.dut->new_have_l2_to_ba_resp_o);
  ...
  sim.compare("l2_evict_source", sim.dut->orig_l2_evict_source_o, sim.dut->new_l2_evict_source_o);
  sim.compare("l2_evict_wdata", sim.dut->orig_l2_evict_wdata_o, sim.dut->new_l2_evict_wdata_o);
  ...
  sim.compare("l2_miss_source", sim.dut->orig_l2_miss_source_o, sim.dut->new_l2_miss_source_o);
  sim.compare("l2_miss_wdata", sim.dut->orig_l2_miss_wdata_o, sim.dut->new_l2_miss_wdata_o);
  ...
  sim.compare("force_ba_rptr_next", sim.dut->orig_force_ba_rptr_next_o, sim.dut->new_force_ba_rptr_next_o);
  ...
  sim.compare("fill_bus_error", sim.dut->orig_fill_bus_error_o, sim.dut->new_fill_bus_error_o);
  sim.compare("uc_load_bus_error", sim.dut->orig_uc_load_bus_error_o, sim.dut->new_uc_load_bus_error_o);
...
void run_clean_miss(CosimCtrl<DUT>& sim, Coverage& cov) {
...
void run_dirty_miss(CosimCtrl<DUT>& sim, Coverage& cov) {
  clear_inputs(sim.dut.get());
  drive_base_req(sim.dut.get());
  sim.dut->req_pri_val_i = 1;
  sim.dut->req_cacheable_i = 1;
  sim.dut->req_old_meta_state_i = 3;
...
void run_uc_store(CosimCtrl<DUT>& sim, Coverage& cov) {
  clear_inputs(sim.dut.get());
  drive_base_req(sim.dut.get());
  sim.dut->req_pri_val_i = 1;
  sim.dut->req_cmd_i = 1;
...
void run_uc_load_error(CosimCtrl<DUT>& sim, Coverage& cov) {
  clear_inputs(sim.dut.get());
  drive_base_req(sim.dut.get());
  sim.dut->req_pri_val_i = 1;
  sim.dut->req_cacheable_i = 0;
  ...
  sim.dut->l2_miss_ready_i = 0;
  sim.dut->l2_resp_valid_i = 1;
  sim.dut->l2_resp_id_i = 8;
  sim.dut->l2_resp_opcode_i = 3;
  tick_and_compare(sim, &cov);
}

void run_tag_hit_upgrade(CosimCtrl<DUT>& sim, Coverage& cov) {
  clear_inputs(sim.dut.get());
  drive_base_req(sim.dut.get());
  sim.dut->req_pri_val_i = 1;
  sim.dut->req_cmd_i = 1;
  sim.dut->req_tag_match_i = 1;
  sim.dut->req_cacheable_i = 1;
  sim.dut->req_old_meta_state_i = 2;
  sim.dut->meta_write_ready_i = 1;
  tick_and_compare(sim, &cov);
  sim.dut->req_pri_val_i = 0;
  tick_and_compare(sim, &cov);
...
void run_secondary_and_conflict(CosimCtrl<DUT>& sim, Coverage& cov) {
  clear_inputs(sim.dut.get());
  drive_base_req(sim.dut.get());
  sim.dut->req_pri_val_i = 1;
  sim.dut->req_cacheable_i = 1;
  sim.dut->req_old_meta_state_i = 1;
  sim.dut->inflight0_valid_i = 1;
  sim.dut->inflight0_way_en_i = 0x2;
  sim.dut->inflight0_set_i = 4;
  tick_and_compare(sim, &cov);
...
int main(int argc, char** argv) {
  ...
  run_clean_miss(sim, coverage);
  run_dirty_miss(sim, coverage);
  run_uc_store(sim, coverage);
  run_uc_load_error(sim, coverage);
  run_tag_hit_upgrade(sim, coverage);
  run_secondary_and_conflict(sim, coverage);
  ...
  for (uint32_t seed_init : kSeeds) {
    ...
    for (int cycle = 0; cycle < 1024; ++cycle) {
      ...
      sim.dut->req_pri_val_i = xorshift32(seed) & 1u;
      sim.dut->req_sec_val_i = xorshift32(seed) & 1u;
      sim.dut->req_addr_i =
      ...
      sim.dut->inflight0_valid_i = xorshift32(seed) & 1u;
      sim.dut->inflight0_way_en_i = 1u << (xorshift32(seed) & 0x3u);
      sim.dut->inflight0_set_i = xorshift32(seed) & 0xFu;
      sim.dut->inflight1_valid_i = xorshift32(seed) & 1u;
      sim.dut->inflight1_way_en_i = 1u << (xorshift32(seed) & 0x3u);
      sim.dut->inflight1_set_i = xorshift32(seed) & 0xFu;
