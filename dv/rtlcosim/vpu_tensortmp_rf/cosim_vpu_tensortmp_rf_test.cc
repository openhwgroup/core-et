...
#include "Vcosim_vpu_tensortmp_rf_tb.h"
#include "verilated.h"
...
using DUT = Vcosim_vpu_tensortmp_rf_tb;
...
int main(int argc, char** argv) {
    ...
    write_single_tick(sim, 7, 0x22222222u);
    sim.check(read_addr(sim, 0) == 0x11111111u, "read addr 0 after write");
    sim.check(read_addr(sim, 7) == 0x22222222u, "read addr 7 after write");
    ...
    sim.dut->wr_en_early_i = 0;
    sim.dut->wr_en_i = 0;
    sim.check(read_addr(sim, 5) == 0x44444444u, "low-phase capture");

    uint32_t seed = 0x10d0d0d4u;
    for (int i = 0; i < kRandomCycles; ++i) {
      sim.dut->rd_addr_i = xorshift32(seed) & 7u;
      sim.dut->wr_addr_i = xorshift32(seed) & 7u;
    ...
    sim.check(sim.comparisons >= 50000, "vpu_tensortmp_rf cosim should exceed 50k comparisons");
    return sim.finish();
}
