...
#include "Vcosim_neigh_evict_ff_tb.h"
#include "cosim_ctrl.h"
#include <cstdint>
#include <cstdio>

using DUT = Vcosim_neigh_evict_ff_tb;

static uint32_t rng_state = 0x45766963U;
...
static void compare_outputs(CosimCtrl<DUT>& sim) {
  ...
  sim.compare("coop_evict_valid", dut->orig_coop_evict_valid_o, dut->new_coop_evict_valid_o);
  sim.compare("coop_evict_req_id", dut->orig_coop_evict_req_id_o, dut->new_coop_evict_req_id_o);
  sim.compare("coop_evict_req_source", dut->orig_coop_evict_req_source_o, dut->new_coop_evict_req_source_o);
  sim.compare("coop_evict_req_wdata", dut->orig_coop_evict_req_wdata_o, dut->new_coop_evict_req_wdata_o);
  sim.compare("coop_evict_req_opcode", dut->orig_coop_evict_req_opcode_o, dut->new_coop_evict_req_opcode_o);
  sim.compare("coop_evict_req_address", dut->orig_coop_evict_req_address_o, dut->new_coop_evict_req_address_o);
  sim.compare("coop_evict_req_data0", dut->orig_coop_evict_req_data0_o, dut->new_coop_evict_req_data0_o);
  sim.compare("coop_evict_req_data1", dut->orig_coop_evict_req_data1_o, dut->new_coop_evict_req_data1_o);
  sim.compare("coop_evict_req_data2", dut->orig_coop_evict_req_data2_o, dut->new_coop_evict_req_data2_o);
  sim.compare("coop_evict_req_data3", dut->orig_coop_evict_req_data3_o, dut->new_coop_evict_req_data3_o);
  sim.compare("coop_evict_req_size", dut->orig_coop_evict_req_size_o, dut->new_coop_evict_req_size_o);
  sim.compare("coop_evict_req_qwen", dut->orig_coop_evict_req_qwen_o, dut->new_coop_evict_req_qwen_o);
  sim.compare("coop_evict_req_subopcode", dut->orig_coop_evict_req_subopcode_o, dut->new_coop_evict_req_subopcode_o);
...
int main(int argc, char** argv) {
  ...
  printf("phase 1: directed wrapper scenarios\n");
  for (int i = 0; i < 16; ++i) {
    dut->shire_id_i = 0x40 + (i & 0x3f);
    dut->neigh_id_i = i & 0x3;
    dut->local_channel_mask_i = (i & 1) ? 0x1 : 0x2;
    dut->local_dest0_i = 1;
    dut->local_dest1_i = 5;
    dut->local_dest2_i = 7;
    ...
    dut->l2_req_address_i = (i & 1)
        ? build_local_msg_addr(dut->shire_id_i, dut->neigh_id_i, dut->local_dest0_i, 0x20 + i)
        : (0x8000 + i * 0x80);
    dut->l2_req_data0_i = 0x10 + i;
    dut->l2_req_data1_i = 0x20 + i;
    dut->l2_req_data2_i = 0x30 + i;
    dut->l2_req_data3_i = 0x40 + i;
    dut->l2_req_size_i = 5;
    dut->l2_req_qwen_i = i & 0xf;
    dut->l2_req_subopcode_i = (i * 9) & 0x7f;
    dut->l2_dcache_req_valid_i = i & 0x3;
    dut->local_message_ready_i = (i >> 1) & 1;
    dut->evict_ready_i = (i >> 2) & 1;
    dut->coop_evict_ready_i = (i >> 3) & 1;
    sim.tick();
    compare_outputs(sim);
  ...
  for (int i = 0; i < 3000; ++i) {
    ...
    dut->l2_req_wdata_i = 1;
    dut->l2_req_opcode_i = ((i & 7) == 0) ? 4 : (xorshift32() & 0x1f);
    if ((i & 7) == 0) {
      dut->l2_req_address_i =
          build_local_msg_addr(dut->shire_id_i, dut->neigh_id_i, dut->local_dest1_i, xorshift32() & 0xff);
    } else {
    ...
    dut->l2_dcache_req_valid_i = xorshift32() & 0x3;
    dut->local_message_ready_i = xorshift32() & 0x1;
    dut->evict_ready_i = xorshift32() & 0x1;
    dut->coop_evict_ready_i = xorshift32() & 0x1;
    sim.tick();
    compare_outputs(sim);
    if (sim.mismatches > 20) break;
  }
