...
#include "Vcosim_txfma_e5_tb.h"
#include "cosim_ctrl.h"
#include "../../../hw/ip/minion/vpu/dv/txfma_e_stage_model.h"

using DUT = Vcosim_txfma_e5_tb;
...
CovIdx add_cov(Coverage& cov) {
    return CovIdx{cov.add("cmd_i", 7), cov.add("rm_i", 3), cov.add("op_dtype_i", 4),
                  cov.add("sigs_i", 19), cov.add("exp_res_in_i", 10), cov.add("ea_hi_i", 8),
                  cov.add("ec_i", 8), cov.add("s_res_i", 1), cov.add("comp_op_i", 1),
                  cov.add("comp_op_true_i", 1), cov.add("nshc_i", 1),
                  cov.add("int_minmax_mux_sel_a_i", 1), cov.add("int_minmax_mux_sel_c_i", 1),
                  cov.add("e_force_to_zero_i", 1), cov.add("ediff1_zap_i", 1),
                  cov.add("e_sticky1_i", 1), cov.add("e_sticky2_i", 1), cov.add("elxd_i", 7),
                  cov.add("etrz50_i", 6), cov.add("use_elxd_i", 1),
                  cov.add("elxd_add_in2_mux_sel_i", 1), cov.add("fb_hi_zero_i", 1),
...
void drive_and_compare(CosimCtrl<DUT>& sim, Coverage& cov, const CovIdx& idx, const E5In& in) {
    drive_inputs(sim.dut.get(), in);
    observe(cov, idx, in);
    sim.dut->eval();
    const E5Out exp = model_e5(in);
    sim.compare("stky2g", sim.dut->orig_stky2g_o, sim.dut->new_stky2g_o);
    sim.compare("rmode", sim.dut->orig_rmode_o, sim.dut->new_rmode_o);
    sim.compare("exp_res", sim.dut->orig_exp_res_o, sim.dut->new_exp_res_o);
    sim.check(sim.dut->new_stky2g_o == exp.stky2g, "model stky2g");
    sim.check(sim.dut->new_rmode_o == exp.rmode, "model rmode");
...
int main(int argc, char** argv) {
    ...
    const E5In directed[] = {
        E5In{VPU_FCMD_ADD, RNE, VPU_DTYPE_F32, 0, 0x82, 0x11, 0x22, false, false, false, false, false, false, false, false, false, false, 4, 0, true, false, false, false},
        E5In{VPU_FCMD_ADD, RUP, VPU_DTYPE_F32, 0, 0x82, 0x11, 0x22, false, false, false, true, false, false, false, false, false, false, 4, 0, true, false, false, false},
        E5In{VPU_FCMD_ADD, RDN, VPU_DTYPE_F32, 0, 0x82, 0x11, 0x22, true, false, false, false, false, false, false, false, false, false, 4, 0, true, false, false, false},
        E5In{VPU_FCMD_ADD, RMM, VPU_DTYPE_F16_F32, 0, 0x70, 0x44, 0x55, false, false, false, false, false, false, false, false, false, true, 0x7f, 0x3f, true, false, false, false},
        E5In{VPU_FCMD_ADD, RNE, VPU_DTYPE_F32, 0, 0x70, 0xaa, 0xbb, false, false, false, false, false, false, true, false, false, false, 3, 1, false, false, false, false},
        E5In{VPU_TRANS_RCP_FMA2, RNE, VPU_DTYPE_F32, 0, 0x70, 0xaa, 0xbb, false, false, false, false, false, false, false, false, false, false, 3, 1, false, false, true, false},
        E5In{VPU_TRANS_RSQRT_FMA2, RNE, VPU_DTYPE_F32, 0, 0x81, 0xaa, 0xbb, false, false, false, false, false, false, false, false, false, false, 3, 1, false, false, false, false},
        E5In{VPU_TRANS_EXP_FRAC, RNE, VPU_DTYPE_F32, 0, 0x90, 0xaa, 0x22, false, false, false, false, false, false, false, false, false, false, 3, 1, false, false, false, false},
        E5In{0x7f, 0x7, 0xf, 0x7ffff, 0x3ff, 0xff, 0xff, true, true, true, true, true, true, true, true, true, true, 0x7f, 0x3f, true, true, true, true},
    };
    for (const E5In& in : directed) {
        drive_and_compare(sim, cov, idx, in);
    }
    uint32_t seed = 0xe5a55678u;
    for (int i = 0; i < 5000; ++i) {
        drive_and_compare(sim, cov, idx, random_input(seed));
    }

    cov.check(sim);
    sim.check(sim.comparisons >= 15000, "txfma_e5 should exceed 15000 comparisons");
    return sim.finish();
}
