...
module cosim_neigh_tensor_store_buffer_block_tb
  import etlink_pkg::*;
  import minion_pkg::*;
...
(
  ...
  input  logic [4*$bits(etlink_pkg::neigh_req_t)-1:0] evict_req_bits_i,
  input  logic [3:0]                                  evict_valid_i,
  input  logic                                        evict_out_ready_i,
  input  logic                                        evict_out_disable_next_i,
  input  logic [3:0]                                  tstore_min_ack_i,
  input  logic [3:0]                                  tstore_min_resp_ready_i,

  output logic [3:0]                                  orig_evict_ready_o,
  output logic [3:0]                                  new_evict_ready_o,
  output logic                                        orig_evict_out_valid_o,
  output logic                                        new_evict_out_valid_o,
  output logic                                        orig_evict_out_valid_1st_o,
  output logic                                        new_evict_out_valid_1st_o,
  output logic                                        orig_evict_out_valid_2nd_o,
  output logic                                        new_evict_out_valid_2nd_o,
  ...
  output logic [3:0]                                  orig_tstore_min_resp_valid_o,
  output logic [3:0]                                  new_tstore_min_resp_valid_o,
  output logic [4*$bits(minion_pkg::et_link_minion_rsp_info_t)-1:0] orig_tstore_min_resp_bits_o,
  output logic [4*$bits(minion_pkg::et_link_minion_rsp_info_t)-1:0] new_tstore_min_resp_bits_o,
  output logic [$bits(neigh_pkg::tstore_block_dbg_sm_t)-1:0] orig_dbg_bits_o,
  output logic [$bits(neigh_pkg::tstore_block_dbg_sm_t)-1:0] new_dbg_bits_o
);
  ...
  et_link_neigh_req_info_t [3:0] orig_evict_req;
  et_link_neigh_req_info_t orig_evict_out_req;
  et_link_minion_rsp_info_t [3:0] orig_tstore_min_resp;
  tstore_block_dbg_sm_t orig_dbg;

  etlink_pkg::neigh_req_t [3:0] new_evict_req;
  etlink_pkg::neigh_req_t new_evict_out_req;
  minion_pkg::et_link_minion_rsp_info_t [3:0] new_tstore_min_resp;
  neigh_pkg::tstore_block_dbg_sm_t new_dbg;
  ...
  neigh_tensor_store_buffer_block_orig #(
    .NUM_MINIONS   (4),
    .SOURCE_OFFSET (0)
  ) u_orig (
    .clock                  (clk_i),
    ...
    .evict_out_valid_1st    (orig_evict_out_valid_1st_o),
    .evict_out_valid_2nd    (orig_evict_out_valid_2nd_o),
  ...
  neigh_tensor_store_buffer_block u_new (
    .clk_i,
    .rst_ni,
    ...
    .evict_out_valid_1st_o    (new_evict_out_valid_1st_o),
    .evict_out_valid_2nd_o    (new_evict_out_valid_2nd_o),
...
endmodule : cosim_neigh_tensor_store_buffer_block_tb
