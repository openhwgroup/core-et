...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < kEntries; i += 2) {
        write_pair_cycle(sim,
                         static_cast<uint8_t>(i),     0x10000000u + static_cast<uint32_t>(i),     true, true, true,
                         static_cast<uint8_t>(i + 1), 0x20000000u + static_cast<uint32_t>(i + 1), true, true, true,
                         false);
    }
    ...
    if (sim.compare_reads) {
        ...
        if (sim.comparisons < 25000) {
            std::printf("  FAIL: vpu_rf cosim should exceed 25k comparisons\n");
        }
    }
    return sim.finish();
}
