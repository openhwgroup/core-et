//--------------------------------------------------------------------------------------------------
//
// Class: evl_stim_cache_etlink
//
class evl_stim_cache_etlink extends evl_cache;

   `uvm_component_utils(evl_stim_cache_etlink)


   //-----------------------------------------------------------------------------------------------
   //
   // Static Member Variables and Objects
   //
   evl_table_wrapper #(evl_neigh_agent_etlink, "IntReq") m_int_req_table;
   evl_table_wrapper #(evl_neigh_agent_etlink, "BusReq") m_bus_req_table;
   evl_table_wrapper #(evl_neigh_agent_etlink, "BusRsp") m_bus_rsp_table;


   //-----------------------------------------------------------------------------------------------
   //
   // Member Variables and Objects
   //
   evl_table_desc m_table_desc_null;


   //-----------------------------------------------------------------------------------------------
   //
   // Function: gen_int_req_index
   //
   function longint gen_int_req_index(input evl_cmd_t      cmd_in,
                                      input bit            cache_attr_in,
                                      input evl_cache_st_t model_st_in,
                                      input byte           qualifier_in);
      return longint'({ qualifier_in, model_st_in, cache_attr_in, cmd_in });
   endfunction : gen_int_req_index


   //-----------------------------------------------------------------------------------------------
   //
   // Function: gen_int_req_name
   //
   function string gen_int_req_name(input longint index_in);
      bit [($bits(evl_cmd_t)-1):0]      raw_cmd;
      bit                               raw_cache_attr;
      bit [($bits(evl_cache_st_t)-1):0] raw_model_st;
      byte                              qualifier;
      evl_cmd_t                         cmd;
      evl_cache_st_t                    model_st;
      string                            cache_attr;

      { qualifier, raw_model_st, raw_cache_attr, raw_cmd } = index_in[$bits(qualifier) + $bits(raw_model_st) + $bits(raw_cache_attr) + $bits(raw_cmd) - 1:0];
      cmd        = evl_cmd_t'(raw_cmd);
      model_st   = evl_cache_st_t'(raw_model_st);
      cache_attr = (raw_cache_attr == 1'b1) ? "cached" : "uncached";
      return $sformatf("IntReq:%p:%p:%0s:0x%x", cmd, model_st, cache_attr, qualifier);
   endfunction : gen_int_req_name


   //-----------------------------------------------------------------------------------------------
   //
   // Function: add_int_req_act
   //
   function void add_int_req_act(input evl_cmd_t         cmd_list_in[],
                                 input bit               cache_attr_list_in[],
                                 input evl_cache_st_t    model_st_list_in[],
                                 input byte              qualifier_list_in[],
                                 input evl_cache_st_t    tag_action_in,
                                 input evl_data_action_t data_action_in,
                                 input evl_bus_cmd_t     bus_cmd_in);
      longint        index;
      evl_cmd_t      cmd_list[$];
      bit            cache_attr_list[$];
      evl_cache_st_t model_st_list[$];
      byte           qualifier_list[$];
      evl_table_desc table_desc;

      table_desc = new();
      table_desc.set_tag_action(tag_action_in);
      table_desc.set_data_action(data_action_in);
      table_desc.set_bus_cmd(bus_cmd_in);

      if (cmd_list_in.size() == 0) begin
         cmd_list.push_back(NOCMD);
      end
      else begin
         foreach (cmd_list_in[ii]) begin
            cmd_list.push_back(cmd_list_in[ii]);
         end
      end

      if (cache_attr_list_in.size() == 0) begin
         cache_attr_list.push_back(1'b0);
      end
      else begin
         foreach (cache_attr_list_in[ii]) begin
            cache_attr_list.push_back(cache_attr_list_in[ii]);
         end
      end

      if (model_st_list_in.size() == 0) begin
         model_st_list.push_back(St_Ignore);
      end
      else begin
         foreach (model_st_list_in[ii]) begin
            model_st_list.push_back(model_st_list_in[ii]);
         end
      end

      if (qualifier_list_in.size() == 0) begin
         qualifier_list.push_back(0);
      end
      else begin
         foreach (qualifier_list_in[ii]) begin
            qualifier_list.push_back(qualifier_list_in[ii]);
         end
      end

      table_desc.set_index(gen_int_req_index(cmd_list[0], cache_attr_list[0], model_st_list[0], qualifier_list[0]));
      foreach (cmd_list[ii1]) begin
         foreach (cache_attr_list[ii2]) begin
            foreach (model_st_list[ii3]) begin
               foreach (qualifier_list[ii4]) begin
                  index = gen_int_req_index(cmd_list[ii1], cache_attr_list[ii2], model_st_list[ii3], qualifier_list[ii4]);
                  if (m_int_req_table == null) begin
                     m_int_req_table = new();
                     m_int_req_table.set_abstract_name($sformatf("%0s(IntReq)", get_root_abstract_name()));
                  end
                  m_int_req_table.add_table_entry(index, table_desc, gen_int_req_name(index));
               end
            end
         end
      end
   endfunction : add_int_req_act


   //-----------------------------------------------------------------------------------------------
   //
   // Function: gen_bus_req_index
   //
   function longint gen_bus_req_index(input evl_bus_cmd_t  bus_cmd_in,
                                      input evl_cache_st_t model_st_in,
                                      input byte           qualifier_in);
      return longint'({ qualifier_in, model_st_in, bus_cmd_in });
   endfunction : gen_bus_req_index


   //-----------------------------------------------------------------------------------------------
   //
   // Function: gen_bus_req_name
   //
   function string gen_bus_req_name(input longint index_in);
      bit [($bits(evl_etlink_req_opcode_t)-1):0] raw_bus_cmd;
      bit [($bits(evl_cache_st_t)-1):0]          raw_model_st;
      byte                                       qualifier;
      evl_etlink_req_opcode_t                    bus_cmd;
      evl_cache_st_t                             model_st;

      { qualifier, raw_model_st, raw_bus_cmd } = index_in[$bits(qualifier) + $bits(raw_model_st) + $bits(raw_bus_cmd) - 1:0];
      bus_cmd    = evl_etlink_req_opcode_t'(raw_bus_cmd);
      model_st   = evl_cache_st_t'(raw_model_st);
      return $sformatf("BusReq:%p:%p:0x%x", bus_cmd, model_st, qualifier);
   endfunction : gen_bus_req_name


   //-----------------------------------------------------------------------------------------------
   //
   // Function: add_bus_req_act
   //
   function void add_bus_req_act(input evl_bus_cmd_t     bus_cmd_list_in[],
                                 input evl_cache_st_t    model_st_list_in[],
                                 input byte              qualifier_list_in[],
                                 input evl_cache_st_t    tag_action_in,
                                 input evl_data_action_t data_action_in,
                                 input evl_bus_cmd_t     bus_cmd_in);
      longint         index;
      evl_bus_cmd_t   bus_cmd_list[$];
      evl_cache_st_t  model_st_list[$];
      byte            qualifier_list[$];
      evl_table_desc  table_desc;

      table_desc = new();
      table_desc.set_tag_action(tag_action_in);
      table_desc.set_data_action(data_action_in);
      table_desc.set_bus_cmd(bus_cmd_in);

      if (bus_cmd_list_in.size() == 0) begin
         bus_cmd_list.push_back(~evl_bus_cmd_t'(0));
      end
      else begin
         foreach (bus_cmd_list_in[ii]) begin
            bus_cmd_list.push_back(bus_cmd_list_in[ii]);
         end
      end

      if (model_st_list_in.size() == 0) begin
         model_st_list.push_back(St_Ignore);
      end
      else begin
         foreach (model_st_list_in[ii]) begin
            model_st_list.push_back(model_st_list_in[ii]);
         end
      end

      if (qualifier_list_in.size() == 0) begin
         qualifier_list.push_back(0);
      end
      else begin
         foreach (qualifier_list_in[ii]) begin
            qualifier_list.push_back(qualifier_list_in[ii]);
         end
      end

      table_desc.set_index(gen_bus_req_index(bus_cmd_list[0], model_st_list[0], qualifier_list[0]));
      foreach (bus_cmd_list[ii]) begin
         foreach (model_st_list[jj]) begin
            foreach (qualifier_list[kk]) begin
               index = gen_bus_req_index(bus_cmd_list[ii], model_st_list[jj], qualifier_list[kk]);
               if (m_bus_req_table == null) begin
                  m_bus_req_table = new();
                  m_bus_req_table.set_abstract_name($sformatf("%0s(BusReq)", get_root_abstract_name()));
               end
               m_bus_req_table.add_table_entry(index, table_desc, gen_bus_req_name(index));
            end
         end
      end
   endfunction : add_bus_req_act


   //-----------------------------------------------------------------------------------------------
   //
   // Function: gen_bus_rsp_index
   //
   function longint gen_bus_rsp_index(input evl_bus_cmd_t  bus_cmd_in,
                                      input evl_cache_st_t model_st_in,
                                      input byte           qualifier_in);
      return longint'({ qualifier_in, model_st_in, bus_cmd_in });
   endfunction : gen_bus_rsp_index


   //-----------------------------------------------------------------------------------------------
   //
   // Function: gen_bus_req_name
   //
   function string gen_bus_rsp_name(input longint index_in);
      bit [($bits(evl_etlink_req_opcode_t)-1):0] raw_bus_cmd;
      bit [($bits(evl_cache_st_t)-1):0]          raw_model_st;
      byte                                       qualifier;
      evl_etlink_req_opcode_t                    bus_cmd;
      evl_cache_st_t                             model_st;

      { qualifier, raw_model_st, raw_bus_cmd } = index_in[$bits(qualifier) + $bits(raw_model_st) + $bits(raw_bus_cmd) - 1:0];
      bus_cmd    = evl_etlink_req_opcode_t'(raw_bus_cmd);
      model_st   = evl_cache_st_t'(raw_model_st);
      return $sformatf("BusRsp:%p:%p:0x%x", bus_cmd, model_st, qualifier);
   endfunction : gen_bus_rsp_name


   //-----------------------------------------------------------------------------------------------
   //
   // Function: add_bus_rsp_act
   //
   function void add_bus_rsp_act(input evl_bus_cmd_t     bus_cmd_list_in[],
                                 input evl_cache_st_t    model_st_list_in[],
                                 input byte              qualifier_list_in[],
                                 input evl_cache_st_t    tag_action_in,
                                 input evl_data_action_t data_action_in);
      longint         index;
      evl_bus_cmd_t   bus_cmd_list[$];
      evl_cache_st_t  model_st_list[$];
      byte            qualifier_list[$];
      evl_table_desc  table_desc;

      table_desc = new();
      table_desc.set_tag_action(tag_action_in);
      table_desc.set_data_action(data_action_in);

      if (bus_cmd_list_in.size() == 0) begin
         bus_cmd_list.push_back(~evl_bus_cmd_t'(0));
      end
      else begin
         foreach (bus_cmd_list_in[ii]) begin
            bus_cmd_list.push_back(bus_cmd_list_in[ii]);
         end
      end

      if (model_st_list_in.size() == 0) begin
         model_st_list.push_back(St_Ignore);
      end
      else begin
         foreach (model_st_list_in[ii]) begin
            model_st_list.push_back(model_st_list_in[ii]);
         end
      end

      if (qualifier_list_in.size() == 0) begin
         qualifier_list.push_back(0);
      end
      else begin
         foreach (qualifier_list_in[ii]) begin
            qualifier_list.push_back(qualifier_list_in[ii]);
         end
      end

      table_desc.set_index(gen_bus_rsp_index(bus_cmd_list[0], model_st_list[0], qualifier_list[0]));
      foreach (bus_cmd_list[ii]) begin
         foreach (model_st_list[jj]) begin
            foreach (qualifier_list[kk]) begin
               index = gen_bus_rsp_index(bus_cmd_list[ii], model_st_list[jj], qualifier_list[kk]);
               if (m_bus_rsp_table == null) begin
                  m_bus_rsp_table = new();
                  m_bus_rsp_table.set_abstract_name($sformatf("%0s(BusRsp)", get_root_abstract_name()));
               end
               m_bus_rsp_table.add_table_entry(index, table_desc, gen_bus_rsp_name(index));
            end
         end
      end
   endfunction : add_bus_rsp_act


   //-----------------------------------------------------------------------------------------------
   //
   // Function: int_req_lookup
   //
   virtual function evl_req_desc int_req_lookup(output evl_table_desc table_desc,
                                                output evl_line_desc  line_desc,
                                                input  evl_trans_desc trans_desc_in,
                                                input  evl_req_desc   req_desc_in,
                                                input  byte           qualifier_in);
      evl_req_desc req_desc = req_desc_in;
      int          child_index;

      line_desc = get_line_desc(trans_desc_in.get_paddr());
      if (m_int_req_table == null) begin
         table_desc = m_table_desc_null;
         if (req_desc != null) begin
            req_desc.clr_bus_cmd();
         end
         return req_desc;
      end

      table_desc = m_int_req_table.get_table_entry(gen_int_req_index(trans_desc_in.get_cmd(), trans_desc_in.is_cached(), line_desc.get_cache_st(), qualifier_in));
      if (table_desc == null) begin
         `dut_error(get_abstract_name(), $sformatf("TB_ISSUE: didn't find a table match for cmd = %0p, is_cached = %0p, get_cache_st = %0p qualifier = %0p", trans_desc_in.get_cmd(), trans_desc_in.is_cached(), line_desc.get_cache_st(), qualifier_in));
         //table_desc = m_int_req_table.get_table_entry(gen_int_req_index(NOCMD, 1'b0, St_Ignore, 0));
      end
      if (table_desc == null) begin
         table_desc = m_table_desc_null;
         if (req_desc != null) begin
            req_desc.clr_bus_cmd();
         end
         return req_desc;
      end
      if (table_desc.pending_bus_cmd() == 1'b0) begin
         if (req_desc != null) begin
            req_desc.clr_bus_cmd();
         end
         return req_desc;
      end
      else if (req_desc == null) begin
         $cast(req_desc, m_template_req_desc.clone());
      end
      trans_desc_in.copy_to_desc(req_desc);
      child_index = trans_desc_in.add_child_desc(req_desc);

      req_desc.set_bus_cmd(table_desc.get_bus_cmd());
      req_desc.set_paddr(trans_desc_in.get_paddr());

      if (trans_desc_in.is_cached() == 1'b1) begin
         req_desc.set_bus_cache_attr(evl_bus_cache_attr_t'(1'b1));
      end
      else begin
         req_desc.set_bus_cache_attr(evl_bus_cache_attr_t'(1'b0));
      end

      // WriteArounds are 16, 32, or 64 bytes
      if ((req_desc.get_bus_cmd() inside { ET_LINK_REQ_WriteAround }) && (trans_desc_in.is_cached() == 1'b1)) begin
         int cmd_length = int'(trans_desc_in.get_cmd_length());
         if (cmd_length > `EVL_OWORD_BYTES) begin
            req_desc.set_bus_req_size(`EVL_LINE_ADDR_LSB);
         end
         else if (cmd_length > `EVL_QWORD_BYTES) begin
            req_desc.set_bus_req_size(`EVL_OWORD_ADDR_LSB);
         end
         else  begin
            req_desc.set_bus_req_size(`EVL_QWORD_ADDR_LSB);
         end
      end
      // Cache Op Size depends on the type.
      else if (req_desc.get_bus_cmd() inside {ET_LINK_REQ_ScpFill}) begin
           req_desc.set_bus_req_size(`EVL_DWORD_ADDR_LSB);
      end
      else if (req_desc.get_bus_cmd() inside {ET_LINK_REQ_Flush, ET_LINK_REQ_Evict, ET_LINK_REQ_Lock, ET_LINK_REQ_Unlock, ET_LINK_REQ_Prefetch}) begin
         req_desc.set_bus_req_size(4'b0000); // 1 byte
      end
      else if (req_desc.get_bus_cmd() inside {ET_LINK_REQ_Atomic}) begin
         int atomic_conf;

         atomic_conf = int'(trans_desc_in.get_bus_atomic_conf());
         if ((atomic_conf & `EVL_BUS_ATOMIC_CONF_VEC_OP) != 0) begin
            req_desc.set_bus_req_size(4'b0101); // 32 bytes
         end
         else if ((atomic_conf & `EVL_BUS_ATOMIC_CONF_DW_OP) != 0) begin
            req_desc.set_bus_req_size(4'b0011); // 8 bytes
         end
         else begin
            req_desc.set_bus_req_size(4'b0010); // 4 bytes
         end
      end
      else begin
         int cmd_length = int'(trans_desc_in.get_cmd_length());

         if (cmd_length > `EVL_OWORD_BYTES) begin
            req_desc.set_bus_req_size(`EVL_LINE_ADDR_LSB);
         end
         else if (cmd_length > `EVL_QWORD_BYTES) begin
            req_desc.set_bus_req_size(`EVL_OWORD_ADDR_LSB);
         end
         else if (cmd_length > `EVL_DWORD_BYTES) begin
            req_desc.set_bus_req_size(`EVL_QWORD_ADDR_LSB);
         end
         else if (cmd_length > `EVL_WORD_BYTES) begin
            req_desc.set_bus_req_size(`EVL_DWORD_ADDR_LSB);
         end
         else if (cmd_length > `EVL_HWORD_BYTES) begin
            req_desc.set_bus_req_size(`EVL_WORD_ADDR_LSB);
         end
         else if (cmd_length > 1) begin
            req_desc.set_bus_req_size(`EVL_HWORD_ADDR_LSB);
         end
         else begin
            req_desc.set_bus_req_size(0);
         end
      end

      return req_desc;
   endfunction : int_req_lookup


   //-----------------------------------------------------------------------------------------------
   //
   // Function: bus_rsp_lookup
   //
   virtual function void bus_rsp_lookup(output evl_table_desc table_desc,
                                        output evl_line_desc  line_desc,
                                        input  evl_req_desc   req_desc_in,
                                        input  byte           qualifier_in);
      line_desc = get_line_desc(req_desc_in.get_paddr());
      if (m_bus_rsp_table == null) begin
         table_desc = m_table_desc_null;
         return;
      end
      table_desc = m_bus_rsp_table.get_table_entry(gen_bus_rsp_index(req_desc_in.get_bus_cmd(), St_Ignore, qualifier_in));
      if (table_desc == null) begin
         table_desc = m_bus_rsp_table.get_table_entry(gen_bus_rsp_index(evl_bus_cmd_t'(`EVL_Bus_Idle), St_Ignore, 0));
      end
      if (table_desc == null) begin
         table_desc = m_table_desc_null;
         return;
      end
      return;
   endfunction : bus_rsp_lookup


   //-----------------------------------------------------------------------------------------------
   //
   // Function: bus_req_lookup
   //
   virtual function void bus_req_lookup(output evl_table_desc table_desc,
                                        output evl_line_desc  line_desc,
                                        input  evl_req_desc   req_desc_in,
                                        input  byte           qualifier_in);
      line_desc = get_line_desc(req_desc_in.get_paddr(), 1); //create data if it doesn't exist
      if (m_bus_req_table == null) begin
         table_desc = m_table_desc_null;
         return;
      end
      table_desc = m_bus_req_table.get_table_entry(gen_bus_req_index(req_desc_in.get_bus_cmd(), St_Ignore, qualifier_in));
      if (table_desc == null) begin
         table_desc = m_bus_req_table.get_table_entry(gen_bus_req_index(evl_bus_cmd_t'(`EVL_Bus_Idle), St_Ignore, 0));
      end
      if (table_desc == null) begin
         table_desc = m_table_desc_null;
         return;
      end
      return;
   endfunction : bus_req_lookup


   //-----------------------------------------------------------------------------------------------
   //
   // Function: build_phase
   //
   virtual function void build_phase(uvm_phase phase);
      super.build_phase(phase);
      m_table_desc_null = new();
   endfunction : build_phase


   //-----------------------------------------------------------------------------------------------
   //
   // Constructor
   //
   function new(input string name_in = "evl_stim_cache_etlink", input uvm_component parent_in = null);
      super.new(name_in, parent_in);
   endfunction : new

endclass : evl_stim_cache_etlink
