//--------------------------------------------------------------------------------------------------
//
// Class: evl_etlink_slave_driver
//
class evl_etlink_slave_driver extends evl_push_driver #(evl_req_desc);


  `uvm_component_utils(evl_etlink_slave_driver)
   ...
   function void drive_response(input evl_req_desc req_desc);
      ...
      rsp_opcode = evl_etlink_rsp_opcode_t'(req_desc.get_bus_rsp_type());
      ...
      m_etlink_rtl_port.put_rsp(bus_rsp);

   endfunction // drive_response


   virtual task pre_reset_phase(uvm_phase phase);
      m_etlink_rtl_port.set_rsp_drive();
   ...
   // Constructor
   //
   function new(input string name_in = "evl_etlink_slave_driver", input uvm_component parent_in = null);
      super.new(name_in, parent_in);
   endfunction : new

endclass : evl_etlink_slave_driver
