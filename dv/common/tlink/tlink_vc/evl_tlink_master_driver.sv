//--------------------------------------------------------------------------------------------------
//
// Class: evl_tlink_master_driver
//
class evl_tlink_master_driver extends evl_push_driver #(evl_req_desc);


  `uvm_component_utils(evl_tlink_master_driver)
   ...
   evl_tlink_bus_parser m_bus_parser = evl_tlink_bus_parser::get_inst();
   int                  m_data_width;
   ...
   function void drive_request(input evl_req_desc req_desc);
      ...
      evl_tlink_bus_req_s    bus_req_info;
      evl_tlink_req_opcode_t req_opcode;
      bit                    has_write_data;
      evl_delay_t            delay;
      bit                    success;
      ...
      bus_req_info.source  = req_desc.get_bus_req_id();
      ...
      if (has_write_data == 1'b1) begin
         ...
         if (line_data == null) begin
            ...
            `evl_log_mdesc(UVM_HIGH, VMOD_TLINK_DRV, get_abstract_name(), req_desc, $sformatf("Going to drive %0s", req_desc.sprint_obj()))
...
//       bus_req_info.size  = req_desc.get_bus_req_size();
...
//       bus_req_info.size      = `EVL_LINE_ADDR_LSB;
...
//       bus_req_info.size       = `EVL_DWORD_ADDR_LSB;
//       bus_req_info.qwen       = 4'b0001;
//       bus_req_info.wdata      = 1'b1;
//    end
...
//    end
      ...
      `evl_log_mdesc(UVM_HIGH, VMOD_TLINK_DRV, get_abstract_name(), req_desc, $sformatf("Driving %0s", req_desc.sprint_obj()))

      //Get any request delay
      if (req_desc.get_delay(delay, pk_bus_req_delay) == 1'b1) begin
         bus_req.set_bus_req_delay(delay);
         if (delay > 1) begin
            `evl_log_mdesc(UVM_HIGH, VMOD_TLINK_DRV, get_abstract_name(), req_desc, $sformatf("using a request delay of %0d for %0s", delay, req_desc.sprint_obj()))
         end
      end
      if (m_data_width_clog2 < req_desc.get_bus_req_size()) begin
         if (req_desc.get_delay(delay, pk_bus_req_data_gap_delay) == 1'b1) begin
            ...
            if (delay > 1) begin
               `evl_log_mdesc(UVM_HIGH, VMOD_TLINK_DRV, get_abstract_name(), req_desc, $sformatf("using a request data gap delay of %0d for %0s", delay, req_desc.sprint_obj()))
            end
         end
      end
...
//    m_tlink_rtl_port.put_req(bus_req);
...
//       m_tlink_rtl_port.set_req_drive();
         m_data_width = m_tlink_rtl_port.get_int_param(pk_wdata_bytes, 64);
      end
      else begin
         m_data_width = 64;
      end
   ...
   // Constructor
   //
   function new(input string name_in = "evl_tlink_master_driver", input uvm_component parent_in = null);
      super.new(name_in, parent_in);
   endfunction : new

endclass : evl_tlink_master_driver
