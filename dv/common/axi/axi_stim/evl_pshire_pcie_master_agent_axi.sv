//--------------------------------------------------------------------------------------------------
//
// Class: evl_pshire_pcie_master_agent_axi
//
// This class is an extension of evl_pshire_base_master_agent_axi to define a master stim agent that
// acts like a PSHIRE_PCIE master driving requests to the mesh.
//
class evl_pshire_pcie_master_agent_axi extends evl_pshire_base_master_agent_axi;

   `uvm_component_utils(evl_pshire_pcie_master_agent_axi)
...
// endfunction : final_fixup
   ...
   // Constructor
   //
   function new(input string name_in = "evl_pshire_pcie_master_agent_axi", input uvm_component parent_in = null);
      super.new(name_in, parent_in, PSHIRE_PCIE_MASTER_STIM_AGENT);
      m_bus_req_id_limit = 15;
      m_active_limit     = 15;
      m_issue_limit      = 15;
      m_max_cmd_length   = `EVL_LINE_BYTES;
      set_abstract_name("PSHIRE_PCIE_MASTER(STIM)");
      add_agent_alias(AXI_MASTER_STIM_AGENT);
      set_active_master();
   endfunction : new

endclass : evl_pshire_pcie_master_agent_axi
