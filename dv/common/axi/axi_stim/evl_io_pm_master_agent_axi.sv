//--------------------------------------------------------------------------------------------------
//
// Class: evl_io_pm_master_agent_axi
//
// This class is an extension of evl_stim_agent_axi to define a master stim agent that acts like the
// IO_PM master driving requests to the mesh.
//
class evl_io_pm_master_agent_axi extends evl_stim_agent_axi;

   `uvm_component_utils(evl_io_pm_master_agent_axi)
   ...
   // Constructor
   //
   function new(input string name_in = "evl_io_pm_master_agent_axi", input uvm_component parent_in = null);
      super.new(name_in, parent_in, IO_PM_MASTER_STIM_AGENT);
      m_stimulus_agent = 1'b1;
      set_abstract_name("IO_PM_MASTER(STIM)");
      add_agent_alias(AXI_MASTER_STIM_AGENT);
      set_active_master();
   endfunction : new

endclass : evl_io_pm_master_agent_axi
