//--------------------------------------------------------------------------------------------------
//
// Class: evl_axi_master_agent
//
class evl_axi_master_agent extends evl_agent;

   `uvm_component_utils(evl_axi_master_agent)

   evl_axi_master_sequencer m_master_sequencer;
   evl_axi_master_driver    m_master_driver;
   ...
   virtual function void set_abstract_name(input string name_in);
      ...
      if (m_master_driver != null) begin
         m_master_driver.set_abstract_name($sformatf("%0s(AXI-MDRV)", get_root_abstract_name()));
      end
      if (m_master_sequencer != null) begin
         m_master_sequencer.set_abstract_name($sformatf("%0s(AXI-MSEQ)", get_root_abstract_name()));
      end
   ...
   virtual function void build_phase(uvm_phase phase);
      ...
      m_master_driver    = evl_axi_master_driver::type_id::create("m_master_driver", this);
      m_master_sequencer = evl_axi_master_sequencer::type_id::create("m_master_sequencer", this);

      uvm_config_db#(uvm_object_wrapper)::set(this, "m_master_sequencer.run_phase", "default_sequence", evl_axi_master_seq::get_type());
   ...
   // Constructor
   //
   function new(input string name_in = "evl_axi_master_agent", input uvm_component parent_in = null);
      super.new(name_in, parent_in);
   endfunction : new

endclass : evl_axi_master_agent
