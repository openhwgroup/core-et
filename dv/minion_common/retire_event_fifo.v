...
// retire_event_fifo
// =================
// Captures event of type uevent_retire_event.
// Retires are triggered on wb_valid.
// Output data type: uevent_retire_event

module retire_event_fifo #(parameter int SOC_CYCLE_SIZE = 64,
      ...
      input  wire                      pop,
      output wire                      empty,
      output uevent_retire_event       data_out,
   ...
   uevent_retire_event fifo_data;
   uevent_retire_event fifo_data_out;
   reg                 fifo_valid;
   wire                nearly_full;
   ...
   always_ff @(posedge clock) begin
      if ((reset !== 1'b0) || (enable !== 1'b1)) begin
         last_capture <= 2'b00;
         fifo_valid   <= 1'b0;
         fifo_data    <= {`UEVENT_RETIRE_EVENT_SIZE{1'b0}};
      end
      else if (wb_valid === 1'b1) begin
   ...
   dpi_event_capture #(.DEPTH               (FIFO_DEPTH),
                       .DATA_MSB            (`UEVENT_RETIRE_EVENT_MSB),
