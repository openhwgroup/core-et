...
#define CREDIT_INC_AD           0x803400C0
...
#define CRED_INC     do { *((volatile uint64_t*) CREDIT_INC_AD   ) = 2; } while(0)
#define SET_MTIME_TARGET do { *((volatile uint64_t*) MTIME_LOCAL_TARGET_AD ) = 1; } while(0)
#define CLEAR_MTIME_TARGET do { *((volatile uint64_t*) MTIME_LOCAL_TARGET_AD ) = 0; } while(0)
...
inline uint64_t get_timer_ticks();
inline void set_timer_ticks(int cnt);
