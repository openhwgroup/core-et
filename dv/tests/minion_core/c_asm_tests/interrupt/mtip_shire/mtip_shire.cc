...
int main(){
  ...
  if(id == 0) {
    ...
    // Program and generate interrupt
    test_t1();
  } else if (id % 2 == 0) {
    // Enable interrupts and wait for interrupt
    test_wait();
...
void test_t0(){
  ...
  // All minions per shire have mtime operative 
  *((volatile uint64_t*) MTIME_LOCAL_TARGET_AD) = 0xFFFF;
...
void set_timer_interrupt(bool inject)
{
    uint64_t opcode = ((uint64_t) 0x5 << 56);
    uint64_t set = (uint64_t) (inject ? 1 : 0);
    uint64_t clear = set << 55;
    uint64_t time = ((uint64_t) 0x01 << 53);
    uint64_t val1 = opcode | clear | time;
