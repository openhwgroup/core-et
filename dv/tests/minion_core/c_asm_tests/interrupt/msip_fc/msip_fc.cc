...
// t100: will loop, waiting for FCC and trigger MSIP for t0
// t0: will inc FCC for t100, then loop waiting for interrupts to be
//     triggered while doing ld/sd, csr stall or spinlock. once the
//     interrupt is received, it will inc fcc for t100 so that it can trigger the next one
...
void test_t0(){
 ...
 // inc fcc for thread100  
  uint64_t hart_id = 100;
  uint64_t thread_id  = (hart_id & 1);
  uint64_t minion_id = ((hart_id>>1) & 0x1F);
  uint64_t shire_id  = ((hart_id>>6) & 0x3F);
  give_credit(shire_id, minion_id, thread_id, 0);
...
void test_t100(){
  ...
  for ( int i = 0; i< ITERATIONS; i++) {
    ...
    evict_va(false, to_Mem, (uint64_t) &irq_cnt);
    if ( irq_cnt > i) {
      C_TEST_FAIL;
    }
