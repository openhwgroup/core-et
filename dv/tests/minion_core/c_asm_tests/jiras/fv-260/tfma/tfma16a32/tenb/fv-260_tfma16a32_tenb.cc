...
void tensor_load_tenb(uint64_t a_num_cols) {
   volatile cl_aligned_data_t *buffer = _binary_blob_bin_start;

   uint64_t mid = get_minion_id();
   uint64_t num_lines = a_num_cols;

   //          use_tmask  use_coop  dst_start  transformation  use_tenb  addr                                    offset   num_lines  stride   id
   //          ---------  --------  ---------  --------------  --------  --------------------------------------  ------  ----------  ------   --
   tensor_load(   false,    false,         0,              0,        1,   (uint64_t)(void*)&buffer[mid].data[0],      0,  num_lines,     64,    0);

}

#define TFMA16A32 0b001
void tfma_kernel() {
    for (int b_num_col = 0; b_num_col <= 0x3; b_num_col++) {
        for (int a_num_rows = 0; a_num_rows <= 0xf; a_num_rows++) {
            for (int a_num_cols = 0; a_num_cols <= 0xf; a_num_cols++) {
                ...
                tensor_fma(        0, b_num_col, a_num_rows, a_num_cols,      0,        0,             1,             1,        1,         0,         0, TFMA16A32,          0);
            }
        }
    }
    WAIT_TENSOR_FMA;
