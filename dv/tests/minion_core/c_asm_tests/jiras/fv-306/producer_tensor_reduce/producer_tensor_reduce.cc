...
uint64_t get_tr_conf(uint64_t start_reg, uint64_t num_reg) {
...
uint64_t get_tfma_conf() {
   uint64_t use_tmask = 0;
   uint64_t b_num_col = 0x3;
   uint64_t a_num_rows = 0xf;
   uint64_t a_num_cols = 0xf;
   uint64_t offset = 0x0;
   uint64_t tenc_loc = 1;
...
void tr_tq(uint64_t tq_enc, uint64_t tr_enc) {
   __asm__ __volatile__ (
         "li x31, 64\n"
         "csrrw x0, tensor_reduce, %[tr_enc]\n"
         "csrrw x0, tensor_quant, %[tq_enc]\n"
         :
         : [tr_enc] "r" (tr_enc), [tq_enc] "r" (tq_enc)
         : "x31"
   );
   WAIT_TENSOR_REDUCE;
   WAIT_TENSOR_QUANT;
...
void tr_tfma(uint64_t tfma_enc, uint64_t tr_enc) {
   __asm__ __volatile__ (
         "li x31, 64\n"
         "csrrw x0, tensor_reduce, %[tr_enc]\n"
         "csrrw x0, tensor_fma, %[tfma_enc]\n"
         :
         : [tr_enc] "r" (tr_enc), [tfma_enc] "r" (tfma_enc)
         : "x31"
   );
   WAIT_TENSOR_REDUCE;
   WAIT_TENSOR_FMA;
...
void tr_ts(uint64_t ts_enc, uint64_t tr_enc) {
   __asm__ __volatile__ (
         "li x31, 64\n"
         "csrrw x0, tensor_reduce, %[tr_enc]\n"
         "csrrw x0, tensor_store, %[ts_enc]\n"
         :
         : [tr_enc] "r" (tr_enc), [ts_enc] "r" (ts_enc)
         : "x31"
   );
   WAIT_TENSOR_REDUCE;
   WAIT_TENSOR_STORE;
...
void ProducerTRConsumerTX(uint64_t ptr) {
   uint64_t tq_enc = get_tq_conf();
   uint64_t tfma_enc = get_tfma_conf();
   uint64_t ts_enc = get_ts_conf();
   for (uint64_t tgt_reg = 0; tgt_reg < 32; tgt_reg++) {
      init_fp_regs((void*) ptr);

      uint64_t skip = 0;
      uint64_t tr_enc = get_tr_conf(0, tgt_reg+1);

      /* TR -> TQ */
      skip = (tgt_reg == 1) || (tgt_reg == 2);
      if (!skip) tr_tq(tq_enc, tr_enc);

      /* TR -> TFMA */
      skip = ((tgt_reg+1) == 1);
      if (!skip) tr_tfma(tfma_enc, tr_enc);
