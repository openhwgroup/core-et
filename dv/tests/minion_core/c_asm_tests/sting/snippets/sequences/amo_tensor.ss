...
; PRM-7: AMO
unum_t amo_conf_seq = snp_set_et_ins_conf(vpu_rf_ptr, et_ins_conf_ptr)
;    Compute the PS sequence
unum_t op0_amo = get_seq_amo_ins(et_ins_conf_ptr)
unum_t op1_amo = get_seq_amo_ins(et_ins_conf_ptr)
unum_t op2_amo = get_seq_amo_ins(et_ins_conf_ptr)
unum_t op3_amo = get_seq_amo_ins(et_ins_conf_ptr)
unum_t op4_amo = get_seq_amo_ins(et_ins_conf_ptr)
unum_t op5_amo = get_seq_amo_ins(et_ins_conf_ptr)
;    fd, fs1, fs2 values
unum_t fs1_amo = get_et_ins_fs1_reg(et_ins_conf_ptr)
unum_t fs2_amo = get_et_ins_fs2_reg(et_ins_conf_ptr)
...
memory_t amo_fsx_mem[ET_TEST_ENV.NUM_MINIONS][128]@64:ET_DRAM_REGION
...
if (require_fs1 || require_fs2):
   ; 8 lanes == 32 bytes
   la rx_mem, amo_fsx_mem[l_mid]
   if (require_fs1): ;fs1
...
if (require_fs1 || require_fs2):
   la rx_mem, amo_fsx_mem[l_mid]
   if (require_fs1): ;fs1
      load_mem_to_vpu_lanes(reg_mem = rx_mem, imm_fp = fs1_amo)
   endif
   if (require_fs2): ;fs2
      if (require_fs1):
         addi rx_mem, rx_mem, 32
      endif
      load_mem_to_vpu_lanes(reg_mem = rx_mem, imm_fp = fs2_amo)
   endif
endif
