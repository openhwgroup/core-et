...
; PRM-5: PS
unum_t ps_conf_seq = snp_set_et_ins_conf(vpu_rf_ptr, et_ins_conf_ptr)
;    Compute the PS sequence
unum_t op0_ps = get_seq_ps_ins(et_ins_conf_ptr)
unum_t op1_ps = get_seq_ps_ins(et_ins_conf_ptr)
unum_t op2_ps = get_seq_ps_ins(et_ins_conf_ptr)
unum_t op3_ps = get_seq_ps_ins(et_ins_conf_ptr)
unum_t op4_ps = get_seq_ps_ins(et_ins_conf_ptr)
unum_t op5_ps = get_seq_ps_ins(et_ins_conf_ptr)
;    fd, fs1, fs2, fs3 values
unum_t fs1_ps = get_et_ins_fs1_reg(et_ins_conf_ptr)
unum_t fs2_ps = get_et_ins_fs2_reg(et_ins_conf_ptr)
unum_t fs3_ps = get_et_ins_fs3_reg(et_ins_conf_ptr)
...
memory_t ps_fsx_mem[ET_TEST_ENV.NUM_MINIONS][128]@64:ET_DRAM_REGION
...
if (require_fs1 || require_fs2 || require_fs3):
   la rx_mem, ps_fsx_mem[l_mid]
   if (require_fs1): ;fs1
      load_mem_to_vpu_lanes(reg_mem = rx_mem, imm_fp = fs1_ps)
   endif
   if (require_fs2): ;fs2
      if (require_fs1):
         addi rx_mem, rx_mem, 32
      endif
      load_mem_to_vpu_lanes(reg_mem = rx_mem, imm_fp = fs2_ps)
   endif
   if (require_fs3): ;fs3
      if (require_fs2):
         addi rx_mem, rx_mem, 32
      endif
      load_mem_to_vpu_lanes(reg_mem = rx_mem, imm_fp = fs3_ps)
   endif
endif
