...
; Then, it does a random tensor_ima8, followed by randomizing r_gen_reduce
; Note that tensor load B is not used
; Tensor Wait is made on the ima only if the tensor_fma writes to TenC
...
; It repeats the sequence of ima8, optional reduce, optional quant in a loop of 10 
...
@cpu: 0,2..BOARD_CONF.NUM_CPUS
    ...
    for index in (0, 10, 1):
       ...
       li x5, S_11
       csrrw  rx1,  tensor_fma, x5 
       ; tensor wait is needed only if loc_of_res is 0
       if Loc_of_res == 0:
