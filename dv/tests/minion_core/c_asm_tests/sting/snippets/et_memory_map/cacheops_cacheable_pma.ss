...
memory_t mem1[64]@64:ET_CACHEABLE_MEM_SET
...
@cpu: x
  ...
  SELECT 1 {
        ...
        10: ;lock_sw:0x7FD
            ...
            ; If the PA is already present in a way different than the way specified in xs[62:55] EMU is not able to manage because it hasn't notion of cache
            ; EMU Checker Int Register data mismatch. BEMU expects data is 0x0 but DUT reported 0x20
            ; to avoid this specific error bit5 in tensor_error I do a evict_sw previously for all 4 ways for the address set 

            set_cacheopCsrLockSw(rega = r0, regb = r1, regc = r2, mask1 = 0x18000FFFFFFFFC0, mask2 = 0x00000FFFFFFFFC0, regdestlevel = rdestlevel, ndx = 3, jmptofail = fail_cacheop)
  ...
  check_tensor_error_cacheop(rega1 = r3, should_fail = 0, on_error = check_TsErrBit7)
  ...
  if ((cacheop_inst == 1) || (cacheop_inst == 2) || (cacheop_inst == 6) ||
      ...
      ;destlevel > L3 or MEM
      addi r1, x0, 2
      bge rdestlevel, r1, fail_cacheop
  elif (cacheop_inst == 3):  ;3 lock_sw
       addi r1, x0, 2
       bge rdestlevel, r1, fail_cacheop
       check_tensor_error_lock_sw(rega1 = r1, should_fail = 0, on_error = check_TsErrBit5)
  else:
     j fail_cacheop
  endif
...
;    wait 
;    cehcktensor
;    lock set_w with @ previously failed.
