...
; ### Same coop_id stuff ###
; Set inverted mask for second tensor load cooperative
unum_t rand_neigh_mask_invert = snp_invert_coop_neigh_mask(rand_neigh_mask)
unum_t coop_mask_val_invert = snp_tensor_mask(BOARD_CONF.NUM_CPUS, coop_mask_rand, rand_neigh_mask_invert, rand_minion_mask)
unum_t in_coop_mask_invert = 0
; ##########################
...
gmemory_t  ctl_mem3[16384]@64:ET_MINION_SHIRE_MEM_SET
...
if (ET_TEST_ENV.STING_RAND == 1) :
   ...
   coop_mask_val_invert = snp_or(coop_mask_val_invert, tmp_coop_id)
...
in_coop_mask_invert = snp_cpu_in_mask(cpuId, coop_mask_val_invert)

if ((in_coop_mask != 0) | (in_coop_mask_invert != 0) | ((cpuId%2 == 1) & (th1_bias == 0))):
...
elif (in_coop_mask_invert != 0):
   li rx1, coop_mask_val_invert
endif
...
if ((cpuId%2) == 1):
   ...
   tensor_load_snip_run(num_x31_val = x31_val, csr_val = tl_csr_val, mem = ctl_mem3, addr_off = 0, trega = rx1, tregb = rx2, tregc = rx3)
if (tenb_bias == 0):
