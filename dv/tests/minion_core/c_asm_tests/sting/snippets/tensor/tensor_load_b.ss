...
; Tensor Load B that cancels the previous Tensor Load B
unum_t  tenb_cancel_bias = snp_urandrange(0, 27)
unum_t  tenb_cancel_rand = snp_urandrange(0, 12)
unum_t  tenb_cancel_offset = snp_lshift(tenb_cancel_rand, 6)
...
if (((tl_csr_val & 0x8000000000000000) != 0) | ((tfma_csr_value & 0x8000000000000000) != 0)):
   ...
   tensor_fma_wait()
if (tenb_cancel_bias == 0):
   tensor_load_snip_run(num_x31_val = x31_val, csr_val = tl_csr_val, mem = mem1, addr_off = tenb_cancel_offset, trega = rx1, tregb = rx2, tregc = rx3)
   RANDINST 0:100
endif
