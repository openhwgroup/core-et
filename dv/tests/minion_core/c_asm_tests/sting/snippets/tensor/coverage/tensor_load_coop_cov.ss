...
gnum_t tl_coop_id_cov[BOARD_CONF.NUM_CPUS]
...
; ### Tensor Mask ###
unum_t  tl_mask_val = glbl_cov_tensor_load_mask(1)
; ###################
...
unum_t  tl_csr_val_base    = glbl_cov_tensor_load_lines(tl_csr_val_masked, 1)
; Set the Cooperative Bit to 1
unum_t  tl_csr_val_tena    = snp_or(tl_csr_val_base, 0x4000000000000000)
; ###################

; ### Tensor Load tenb ###
unum_t  tenb_bias          = snp_urandrange_glbl(0, 3)
unum_t  tl_csr_tmp_tenb    = snp_or(tl_csr_val_base, 0x0010000000000000)
; Set the Cooperative Bit to 1
...
gmemory_t  mem1[16384]@64:ET_MINION_SHIRE_MEM_SET
...
if (ET_TEST_ENV.STING_RAND == 1) :
   ...
   tmp_coop_id   = snp_and(tl_coop_id_cov[cpuId], 0x01f)
...
if ((in_coop_mask != 0) | ((cpuId%2 == 1) & (th1_bias == 0))):
   ...
   tl_coop_id_cov[cpuId] = snp_add(tl_coop_id_cov[cpuId], 1)
