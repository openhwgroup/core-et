...
unum_t  tr_csr_cov      = cov_tensor_reduce_start_reg(tr_csr_val_id)
...
@cpu: x
   ...
   ; OR the values
   tr_csr_val_snd     = snp_or(tr_csr_val, tr_csr_val_snd_tmp)
   ; ####################
...
csr_return_is_0_x:
   tensor_reduce_wait()
if (mid_x == mid_y):
   check_tensor_reduce_error(rega = rx1, regb = rx2, regc = rx3, same_minion = 1, fail_reduce = fail_tr_error_x, pass_reduce = pass_tr_error_x)
else :
   check_tensor_reduce_error(rega = rx1, regb = rx2, regc = rx3, same_minion = 0, fail_reduce = fail_tr_error_x, pass_reduce = pass_tr_error_x)
endif
fail_tr_error_x:
   test_fail(regz = rx1)
pass_tr_error_x:
...
@cpu: y
   ...
   ; OR the values
   tr_csr_val_rcv     = snp_or(tr_csr_val, tr_csr_val_rcv_tmp)
   ; ####################
