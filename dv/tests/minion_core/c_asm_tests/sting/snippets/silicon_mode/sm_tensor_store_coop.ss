...
memory_t sm_ts_info[ET_TEST_ENV.NUM_MINIONS][128]@64:ET_DRAM_REGION
...
if (in_cts == 1):
   ...
   if ((cpu_id%2) == 0):
      dump_vpu_rf(_pb_rx1 = rx1, _pb_mem_addr = sm_pre_vpurf[mid])
      sm_set_cts_info(regx1 = rx1, regx2 = rx2, et_info_addr = sm_ts_info[mid], csr_enc_reg = ts_csr, x31_reg = ts_x31, pre_vpurf_addr = sm_pre_vpurf[mid], post_data_addr = sm_cts_post_data[mem_idx], post_data_off = coop_offset, phy_mid = pmid)
   endif
   ...
   if ((cpu_id%2) == 0):
      snippet_start(random_num = rand_start, rega = rx1, regb = rx2)
      selfcheck_tstore(et_info_addr = sm_ts_info[mid])
   endif
endif
