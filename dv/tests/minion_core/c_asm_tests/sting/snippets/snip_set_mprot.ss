...
unum_t rand_mprot_value = snp_urandrange(16,64)
...
@cpu: *
   ...
   if (cpu_id == x_cpu) :
     ...
     if (TEST_PROPS.TEST_MODE == RV_PARAMS.RV_MODE.MACHINE):
        ...
        set_random_mprot(rega = rx1, regb = rx2, rand_val = rand_mprot_value)
     else:  ; Call mhandler
        et_jump_to_m_mode()
     endif
...
mhandler@cpu: *
   ...
   if (TEST_PROPS.TEST_MODE == RV_PARAMS.RV_MODE.MACHINE):
      ...
      csrrs rx1, mcause, x0  ; I don't mind mcause only expected  8 and 9 or even 2 
      addi rx2, x0, 0x004  ; load esr @mprot
      set_random_mprot(rega = rx1, regb = rx2, rand_val = rand_mprot_value)
   endif
