...
void trap_handler()
{
  ...
  if (cause == ILLEGAL_INST_CAUSE) {
    ...
    if ( (inst == TENSOR_REDUCE_INST) || (inst == TENSOR_FMA_INST) || (inst == TENSOR_CONV_SIZE_INST) || (inst == TENSOR_CONV_CTRL_INST) || 
      ...
      uint64_t pending;
      __asm__ __volatile__ ("csrr %0, mscratch" : "=r" (pending));
      if (pending) {
        exp_cnt++;
        __asm__ __volatile__ ("csrwi mscratch, 0");
      }
    }
  }
...
void trap_handler_t2()
{
  ...
  if (cause == ILLEGAL_INST_CAUSE) {
    ...
    //CLEAR_MSIP_T2;   // clear IPI of Hart 2
    NEW_CLEAR_MSIP(hart_id);  // clear IPI of this Hart
    __asm__ __volatile__ ("fence\n");   // important: to ensure that the (above) CLEAR_MSIP is completed (see VERIF-3962)
  }
  else{
...
void test_t0(unsigned int hart_id) {
  ...
  // (Charles) Workaround for UST-flow with DRET=True (DRET resumes Hart0 much earlier than Hart2):
  // Before setting MINION_FEATURE ESR.TRAP_ON_ML_BIT, M0(H0) must wait for M1(H2) setting up Trap Handler, 
  // or, at least after M1(H2) completes its Boot code.  
  // H0 wait for IPI-MSIP from H2 (after setting up its Trap Handler): 
  FENCE;
  __asm__ __volatile__ ("wfi\n");  

  // write to MINION_FEATURE ESR: set TRAP_ON_ML_BIT=1; 
  __asm__ __volatile__ (
   "li t1, %[addr]\n"
   "ld t0, 0(t1)\n"
   "ori t0, t0, %[bit]\n"
  ...
  // Enable SCP
  evict_sw(0, to_L2, 0, 0, 15);
  evict_sw(0, to_L2, 1, 0, 15);
  evict_sw(0, to_L2, 2, 0, 15);  
  evict_sw(0, to_L2, 3, 0, 15);
  __asm__ __volatile__ ("csrsi mcache_control, 0x1\n");
  __asm__ __volatile__ ("fence\n");                     // VERIF-3295 (PRM-8)
  __asm__ __volatile__ ("csrsi mcache_control, 0x2\n");

  // (after setting TRAP_ON_ML_BIT=1 is completed)
  // send IPI MSIP to H2, to notify that H2 can start doing Tensor Ops;
  //TRIGGER_MSIP_T2;  
  NEW_TRIGGER_MSIP((hart_id+2));   // send IPI MSIP to the Partner (hart_id+2);

  for (int i=0; i<ITERATIONS; i++) {
    //__asm__ __volatile__ ("csrw validation3, %0" : : "r" (i) ); // TODO: REMOVE
    ...
    // set mscratch to 1 (expected to be set to 0 in xcpt handler)
    // when entering the next iteration, if still 1 => test fails
    __asm__ __volatile__ ("csrwi mscratch, 1");
  
    switch(i%16)
      {
      case 0:
        //TRIGGER_MSIP_T2;  // order sync between TensorReduce Send/Receive is unnecessary; 
  ...
  //if (exp_cnt!=(ITERATIONS+(ITERATIONS/16)*4)){
...
void test_t2(unsigned int hart_id) {
  unsigned int min_id = hart_id / 2;
  void (*mtvec_ptr)(void) = &trap_handler_t2;
  ...
  // Workaround for UST-flow with DRET=True (DRET resumes Hart0 much earlier than Hart2):
  // (after completing its BOOT code and Trap Handler)
  // send IPI MSIP to H0, to notify that H2 can start setting MINION_FEATURE_ESR.TRAP_ON_ML ... 
  FENCE; 
  NEW_TRIGGER_MSIP((hart_id-2));   // send IPI MSIP to the Partner (hart_id-2);


  // H2 must wait for IPI-MSIP from H0 (after setting TRAP_ON_ML_BIT=1 completed), before starting Tensor Ops:
  __asm__ __volatile__ ("wfi\n");  
  ...
  for (int i=0; i<ITERATIONS; i++) {
    switch(i%16)
      {
      case 0:
        //__asm__ __volatile__ ("wfi\n");  // order sync between TensorReduce Send/Receive is unnecessary
