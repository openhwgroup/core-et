...
#define PREFETCH_VA_INST	 (0x81f79073 & CSR_MASK) 
#define EVICT_VA_INST		 (0x89f79073 & CSR_MASK) 
#define FLUSH_VA_INST		 (0x8bf79073 & CSR_MASK) 
#define LOCK_VA_INST		 (0x8df51073 & CSR_MASK)
#define UNLOCK_VA_INST		 (0x8ff69073 & CSR_MASK) 
...
void trap_handler()
{
  ...
  if ( cause == ILLEGAL_INST_CAUSE) {
    ...
    inst = inst & CSR_MASK;
    if ( (inst == PREFETCH_VA_INST) || (inst == EVICT_VA_INST) || (inst == FLUSH_VA_INST) || (inst == LOCK_VA_INST) || (inst == UNLOCK_VA_INST)){ 
      exp_cnt++;
    }
    else {
      err=1;
      ...
      for (int i=0; i<ITERATIONS; i++){
        switch(i%5)
          ...
          case 0:
            ...
            //      ---------   -----    ---------------------------    ---------  ------  --
            prefetch_va(false,   0,     (uint64_t)(void*)(ptr+(8*i)),     0,         0,    0 );
            break;
          case 1:
            //      use_tmask    dst    addr                               num_lines  stride  id
            //      ---------  ------  --------------------------------    ---------  ------  --
            evict_va(false,      1,     (uint64_t)(void*)(ptr+(8*(i-1))),      0,       0,    0 );
            break;
          case 2:
            //      use_tmask    dst    addr                               num_lines  stride  id
            //      ---------  ------  --------------------------------    ---------  ------  --
            flush_va(false,      2,     (uint64_t)(void*)(ptr+(8*(i-2))),      0,       0,    0 );
            break;
          case 3:
