...
int main()
{
    ...
    for(int i=(1<<15); i<(1<<(23-2)); i+=(1024+1))
    {
        // random sign 1b
        ...
        volatile int exponent = et_get_rand_word() % 128;// 128; FUTURE: Emmanuel Marie ENFORCING FAST
        value += (exponent == 0  ) ?        1 << 23 :  // no support for denormalized
                 (exponent == 127) ?      126 << 23 :  // only fractional part!
                                     exponent << 23;
