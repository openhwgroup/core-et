...
int main()
{
   ...
   port_cfg.port_id = (uint8_t) 0;

   port_cfg.port_en       = 1;
   port_cfg.port_oob_en   = 0;
   port_cfg.umode         = 1;
   port_cfg.log_msg_size  = 3;
   port_cfg.max_msg_size  = 8;

   port_cfg.scp_way = (uint8_t) et_get_rand_word(0,1);
   port_cfg.scp_way += tid*2;
   port_cfg.scp_set = ( ((uint64_t) &rcv_buffer[mid+tid].data[tid]) >> 6) & 0xF;
   ...
   // 1.2 Lock some cache lines and then Write to PORT_CTRL register to enable the reception of port messages
   setup_rx_port(port_cfg.port_id, port_cfg.umode, port_cfg.log_msg_size, port_cfg.max_msg_size, &rcv_buffer[mid+tid].data[tid], port_cfg.scp_way, port_cfg.port_oob_en);
   ...
   if (tid == 0)
   {
      // Send a message to all the minions adjacent to yourself in the same neighborhood
      for (i=0; i<NUM_MSGS_PER_MINION; i++)
        send_dword(sid, (mid+1)%NUM_MINIONS_PER_NEIGH, 1UL, nid, (uint64_t) port_cfg.port_id, send_msg_payload[i], 0);
   ...
   // FUTURE: Emmanuel Marie change loads to be back to back in case the offset is more that a DWORD or split a DWORD load
   // to two loads.
   else
   {
      if( mid < active_minions-1 )
      {
         for (int i=0; i<NUM_MSGS_PER_MINION; i++)
         {
            ...
            //
            offset = read_port_status(port_cfg.port_id);
