...
ALWAYS_INLINE void runtest() {
    ...
    const PmuMinEvent event = pmu_event_TL_INST;
    const uint64_t gold_tl_inst = (tid == 0) ? NUM_TL_INST : 0;
    ...
    // Test kernel
    for (uint64_t i = 0; i < gold_tl_inst; i++) {
    ...
    cmp_data(event_count, gold_tl_inst);
}
