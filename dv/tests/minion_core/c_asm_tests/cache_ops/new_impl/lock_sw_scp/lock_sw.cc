...
//
// Note: This test will be in SCP_mode: only 4 sets available for lock_sw (set=12, 13, 14, 15).
//
...
**/
   ...
   for (ii = 0; ii < NUM_ITER; ++ii) {
      ...
      // SCP_mode: L1D normal cache: each Thread has only 2 sets (T0 has 12, 13; T1 has 14, 15)

      if (ii < 2) {   // SCP_mode: assume only T0 (has 2 sets = 12 and 13) 
      ...
      // after first 2 iterations are completed, unlock everything and warm up the cache again
      if (ii == 2) {
      ...
      // for (ii >= 2) : use random; don't self-checking;
