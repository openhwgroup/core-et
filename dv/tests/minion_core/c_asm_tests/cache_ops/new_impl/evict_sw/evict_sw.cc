...
**/
   ...
   // Here: make sure macro SELF_CHECK and POSTSI_UST are set together at the same time!
      ...
      // do self-checking on first several loops ii<10: try to be more predictable:
      if (ii < 10) {
         ...
         // do EVICT_SW: to ALL 4 ways (of the above "set"):
         for (uint64_t way = 0; way < 4; way++) {
            //       use_tmask  dest             way              set               num_lines          warl
            //       ---------  ---------------  ---------------  ----------------  -----------------  ----
            evict_sw(0,         my_dest,         way,             set,              0,                 rnd);
         }
         WAIT_CACHEOPS;

         // (CYE) self-checking:
         // 1) to be simple: let's only check on Evict L1->L2 and L1->L3;
         // 2) after evict_sw: (dirty) data is flushed to L2; L1 is invalidated;  
         ...
         if (my_dest == 1) {
            ...
            //else {
            //   et_printf("Test self-check: expect (evicted) data in L2: match\n");
            //}
         ...
         if (my_dest == 2) {
            ...
            // check that L3 returned the (dirty) data evicted from L1:
            ...
            // check L1 being invalidated:
            if (result[1] != 0xbeefbeef) {   // should be the new data from L3 (after amoswapl.w )
               et_printf("Test self-check: expect L1 was invalidated: mismatch\n");
               pass = false;
            } 
            //else {
    ...
    //evict_sw(use_tmask, ((rnd >> 1)%4),  (rnd >> 4) % 4,  (rnd >> 8) % 16,  (rnd >> 16) % 16,  rnd);
      evict_sw(use_tmask, dest,            way,             set,              num_lines,         rnd);
