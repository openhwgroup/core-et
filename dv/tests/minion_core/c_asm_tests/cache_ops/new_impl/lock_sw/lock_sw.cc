...
**/
   ...
   for (ii = 0; ii < NUM_ITER; ++ii) {
      ...
      // after first 16 iterations are completed, unlock everything and warm up the cache again
      if (ii == 16) {
      ...
      // for (ii >=16) : use random; don't self-checking;
      ...
      if ((rnd >> 62) < 3) {
         ...
         // Note: Due to the random, some lock_sw might conflict a set+way which has been already locked
         //       and hence will not succeed. So we cannot do self-checking (of data value = 0) here.
         // (CYE) self-checking: data value of this address should be 0 (after lock_sw):
