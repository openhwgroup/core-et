...
**/
   ...
   for (ii = 0; ii < NUM_ITER; ++ii) {
      ...
      // Do some random FLUSH VA
      //       use_tmask  dest             addr                  num_lines          stride             id          warl
      //       ---------  ---------------  --------------------  -----------------  -----------------  ----------  ----
    //flush_va(use_tmask, ((rnd >> 1)%4),  (uint64_t)base_addr,  (rnd >> 16) % 16,  64*((rnd>>24)%16), (rnd>>3)%2, rnd);
      flush_va(use_tmask, dest,            (uint64_t)base_addr,  num_lines,         stride,            id,         rnd);
   ...
   // Here: make sure macro SELF_CHECK and POSTSI_UST are set together at the same time!
      ...
      // 2) after flush_va: (dirty) data is flushed to L2; L1 is still valid (clean data);  
      // 3) do a "amoswapl.w": new data is placed in L2; (old) data is returned to "rd" (need to check);
      // 4) do a normal load: will obtain the old data from L1 (bcoz L1 is valid);

      // Evict L1->L2:
      if (((use_tmask == false) || ((tmask & 0x1UL) == 1)) && (dest == 1)) {
         // check that L2 have L1-flushed (dirty) data:
         ...
         // check L1 being still valid (clean data):
         __asm__ __volatile__
            (
             "lw a1, 0(%[base_addr])\n"
             "sw a1, 4(%[result])\n"
         ...
         if (result[1] != 0xa1a1a1a1) {   // should be the old data still valid in L1 
            et_printf("Test self-check: expect L1 was still valid (old data): mismatch\n");
            pass = false;
         } 
         //else {
         //   et_printf("Test self-check: expect L1 was still valid (old data): match\n");
         //}
      ...
      if (((use_tmask == false) || ((tmask & 0x1UL) == 1)) && (dest == 2)) {
         // check that L3 have L1-flushed (dirty) data:
         __asm__ __volatile__
            (
             ".equ NEW_VALUE, 0xbeefbeef\n"
             "li a0, NEW_VALUE\n"
