...
#define R_PU_RVTIMER            0x80C00000ul
        ...
        for(int i=0; i<3; i++) {
            for(int j=0; j<3; j++) {
               for(int k=0; k<5; k++) {
                   ...
                   if(i == 0 && k < 2) exception_expected = 0;
                   else exception_expected = 1;

                   esr_address = (uint64_t *)(R_PU_RVTIMER + k*8);

                   if(exception_expected == 1) excpt_expexted = 1;
                   *esr_address = 0x5A5A5A5A5A5A5A50 + j;
                   if(exception_expected == 1) excpt_expexted = 1;
                   second_val = *esr_address;

                   expected_val = 0x5A5A5A5A5A5A5A50 + j;

                   if(second_val != expected_val && exception_expected == 0 && k == 1) {
                       C_TEST_FAIL;
                   }
