...
void pll_mode_config_broadcast(uint8_t mode, uint8_t quadrant, uint8_t wait_for_lock)
{
   uint64_t reg_value;
   uint8_t  reg_num = 0;
...
int main()
{
    ...
    if ((shire_id == 32) && (minion_id == 0) && (thread_id == 0)) {
          ...
          pll_mode_config_broadcast(29, 1, 1);
          // Enable PLL auto config
          reg_value =  SHIRE_OTHER_PLL_AUTO_CONFIG_PCLK_SEL
                 | (0x0 << SHIRE_OTHER_PLL_AUTO_CONFIG_REG_NUM_OFF)
                 | (0x02 << SHIRE_OTHER_PLL_AUTO_CONFIG_REG_FIRST_OFF)
          ...
          //750MHz
          pll_mode_config_broadcast(19, 1, 0);
          ...
          for(int i=1; i<num_iter; i++) {
          	 new_mode = *syncAddr;
       	  	 syncAddr++;
             dram_rw(100); 
             pll_mode_config_broadcast(new_mode, 1, 0);
             while(!(read_esr(PP_MACHINE, 0, REGION_OTHER, SHIRE_PLL_READ_DATA) & 0x20000));
          }
    }
