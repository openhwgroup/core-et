...
//----------------------------------------------------------------------------------------------
//
//  This test accesses entire DRAM memory map from MASTER MINION and checks whether there was a fault at access
...
int main()
{
    ...
    wait_for_credit(0);
      ...
      if (caughtFailures != FAILURES_16GB_U_MODE_PER_ACCESS_SIZE * (sizeof(accessType)-1)) {
        #ifdef MINSHIRE_GATESIM_ENABLE
          if (shire_id == 0 and thread_id == 0) atomic_read_inc_bad(BASE_S0_GATESIM_VALID_ADDR + 8*minion_id);
        #endif
        C_TEST_FAIL;
      }  
      ...
      #ifdef MINSHIRE_GATESIM_ENABLE
        if (shire_id == 0 and thread_id == 0) atomic_read_inc_feed(BASE_S0_GATESIM_VALID_ADDR + 8*minion_id);
      #endif      
      C_TEST_PASS;
        ...
        if (shire_id == 32 and thread_id == 0) {
          check_gatesim(timeout, BASE_S0_GATESIM_VALID_ADDR + 8*minion_id);        
        }
      #endif        
      C_TEST_PASS;
    }
