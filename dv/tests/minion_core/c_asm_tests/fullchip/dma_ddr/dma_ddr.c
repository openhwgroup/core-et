...
/*   StartAddr                          Size                       BusDataW   NumAccess   PattType    Patt        MemName  */
{
  ...
  {  R_DRCT_DRAM_BASEADDR,              R_DRCT_DRAM_SIZE,          32,         0,         0,          0x1234FEDC, "DRCTDRAM" },
  {  0,                                 0,                         32,         0,         0,          0,          "JUNK"     } 
};
...
void myDmaIsr(void)
{
   ...
   // my_printf("DMA IRQ number: %d\n", irq_counter);
    ...
    if( tfr_flag ) {    // transfer is in progress
        ...
        my_printf("FATAL: Unexpected DMA IRQ!!!\n");
        while(1);
    }
...
//    my_printf("dw_dmac_init returned: %d\n", errCode);
    ...
    blockSize = 10; // Select the size of a block
    numBlocks = 1;  // Select the number of blocks

    real_src_addr = (uint32_t *) (memTable[0].startAddress + 0x8f0);
    real_dst_addr = (uint32_t *) (memTable[0].startAddress + 0x3c0);
    
    real_src_addr = (uint64_t)real_src_addr & 0xFFFFFFFFFC;
    real_dst_addr = (uint64_t)real_dst_addr & 0xFFFFFFFFFC;
