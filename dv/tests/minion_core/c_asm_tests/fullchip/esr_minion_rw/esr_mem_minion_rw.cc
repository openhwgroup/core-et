    ...
    if (shire_id == 0) {
        ...
        for(int i=0; i<3; i++) {
            ...
            while( pRegs->addr != 0xFFFFFFFFFFFF ) {
                ...
                //MEM shire access
                esr_address = ms_esr_address_dv(pRegs->PP, 0xE8, pRegs->region, pRegs->addr);

                if(exception_expected == 1) excpt_expexted = 1;
                if(pRegs->has_resetVal == 0)
                    *esr_address = 0;
                if(exception_expected == 1) excpt_expexted = 1;
                first_val = *esr_address;
                if(exception_expected == 1) excpt_expexted = 1;
                *esr_address = (0x5A5A5A5A5A5A5A5A & pRegs->w_mask) |
                               (first_val & (pRegs->w_mask ^ pRegs->r_mask));
                if(exception_expected == 1) excpt_expexted = 1;
