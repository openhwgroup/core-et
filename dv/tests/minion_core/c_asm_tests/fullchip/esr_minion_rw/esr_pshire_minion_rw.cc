...
int main()
{
    uint32_t    first_val_32;
    uint32_t    second_val_32;
    uint32_t    expected_val_32;
    uint8_t     exception_expected;
    volatile uint64_t shire_id = get_shire_id();
    volatile uint32_t   *esr_address;
    ...
    if (shire_id == 0) {
        ...
        for(int i=0; i<3; i++) {
            ...
            while( pRegs->addr != 0xFFFFFFFFFFFF ) {
                ...
                esr_address = ps_esr_address(pRegs->region, pRegs->addr);

                if(exception_expected == 1) excpt_expexted = 1;
                first_val_32 = *esr_address;
                if(exception_expected == 1) excpt_expexted = 1;
                *esr_address = (0x5A5A5A5A & pRegs->w_mask);
                if(exception_expected == 1) excpt_expexted = 1;
                second_val_32 = *esr_address;

                if(pRegs->rw_access == RO) {
                    expected_val_32 = first_val_32;
                }
                else if(pRegs->rw_access == RW) {
                    expected_val_32 = 0x5A5A5A5A & pRegs->r_mask;
                }
                else if(pRegs->rw_access == RW1C) {
                    expected_val_32 = first_val_32 & 0x5A5A5A5A;
                }
                else if(pRegs->rw_access == WO) {
                    expected_val_32 = 0x0;
                }
                else if(pRegs->rw_access == RC) {
                    expected_val_32 = 0x0; //0x5A5A5A5A & pRegs->r_mask;
                }
                else expected_val_32 = 0;

                if(second_val_32 != expected_val_32 && exception_expected == 0) {
                    C_TEST_FAIL;
                }
