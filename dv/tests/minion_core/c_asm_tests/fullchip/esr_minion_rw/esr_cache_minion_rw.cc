    ...
    if (shire_id == 0) {
        ...
        for(int i=0; i<3; i++) {
            ...
            while( pRegs->addr != 0xFFFFFFFFFFFF ) {
                ...
                if(pRegs->has_resetVal == 0) write_esr_new(pRegs->PP, 32, pRegs->region, pRegs->SUBREGION_ID, pRegs->addr, 0x0, 0);
                if(exception_expected == 1) excpt_expexted = 1;
                first_val = read_esr_new(pRegs->PP, 32, pRegs->region, pRegs->SUBREGION_ID, pRegs->addr, 0);
                if(exception_expected == 1) excpt_expexted = 1;
                write_esr_new(pRegs->PP, 32, pRegs->region, pRegs->SUBREGION_ID, pRegs->addr, (0x5A5A5A5A5A5A5A5A & pRegs->w_mask) | (first_val & (pRegs->w_mask ^ pRegs->r_mask)), 0);
                if(exception_expected == 1) excpt_expexted = 1;
                second_val = read_esr_new(pRegs->PP, 32, pRegs->region, pRegs->SUBREGION_ID, pRegs->addr, 0);
