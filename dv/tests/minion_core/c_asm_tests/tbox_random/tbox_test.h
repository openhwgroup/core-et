...
constexpr uint8_t NR_PORTS = 4;
constexpr size_t SETS_PER_PORT = 4;
constexpr size_t TEXTURE_SIZE = 128*4;
constexpr uint64_t TBOX_RSP_TOTAL_SIZE = 256*4;

typedef struct
{
  bool success;
  uint8_t rsp_data[TBOX_RSP_TOTAL_SIZE/sizeof(uint8_t)];
} dump_storage_t;
