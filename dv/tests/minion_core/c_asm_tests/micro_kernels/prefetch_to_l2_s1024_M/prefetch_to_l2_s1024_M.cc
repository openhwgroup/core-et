...
/****
 * prefetch_to_l2_s1024_M   (M stands for multi or all shires involved in prefetching)
 *This is a prefetch to L2 test (S1024 stands for stride of 1024)
 *Stride=1024 gaurantees all lines prefetched through same prefetchOP, are loaded from same Mem-Shire/MC
 *Each shires tries to prefetch from Mem-Shire/MC closest to it (all num_lines from the same MC)
...
int main()
{
   ...
   **/
   ...
   Stride=64 (implictly read from the X31[47:6] 
      //Case1: each req will go to ONLY to  ONE Mem-Shire/MC based on stride 0x400 (or 1024).Bit9-0 will be untouched in VA 
   ...
   ***************************/
   if ((hart_id & 1) == 1) //Only Thread1 (or prefetch threads to do prefetch)
   {
	   for(int i = 0; i < N_TIMES ; i++) //N_TIMES = # of times the thread1 performs the prefetch CSR
	   {
		   ...
		   prefetch_va(false,     1,   (uint64_t)(VA),  15,         1024,      0 );
		   WAIT_PREFETCH_0;
	   }
   }
   C_TEST_PASS;
