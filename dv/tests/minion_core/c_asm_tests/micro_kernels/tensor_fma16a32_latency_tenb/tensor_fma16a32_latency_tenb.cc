...
// Tensor_FMA16A32_Latency_TenB test. Makes latency sweeps for typical FP16 FMA uses
// Similar to Tensor_FMA16A32_Latency, but TensorB is loaded to the TenB buffer
// Author: emizan
// Cases tested
...
int main()
{
	...
	// Tensor A Row sweep -- TensorB load right before FMA: Maximum overlap between load and FMA, but FMA needs to wait for first line to be fetched.
	use_tmask = false;
	...
	// Tensor A Row sweep -- TensorB load before TensorA load: FMA has lines to start immediately
	for (uint64_t a_row=0; a_row <= a_num_rows; a_row++) {	    	
	    tensor_load(0, use_coop, 0, load_trans, use_tenb_1, addr, offset, num_lines, stride, id);
	    tensor_load(use_tmask, use_coop, scp_start, load_trans, use_tenb_0, addr, offset, num_lines, stride, id);
	    tensorWait(TENSOR_LOAD_WAIT_0);	  	    	    
	...
	// Tensor A Col sweep -- TensorB load right before FMA: Maximum overlap between load and FMA, but FMA needs to wait for first line to be fetched.
	// emizan: Currently fails here
