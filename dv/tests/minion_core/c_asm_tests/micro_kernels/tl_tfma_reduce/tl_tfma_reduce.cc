...
int main()
{
    ...
    // Do not read coop_csr, is_coop varibales from array to reduce mem access overhead
    uint64_t tl0_coop_csr = 0;
    uint64_t tl0_is_coop = 0;
    ...
    uint64_t reduce_start_reg = 0;
    uint64_t reduce_op = 0;
    uint64_t reduce_num_regs = 0;
    uint64_t reduce_partner = 0;
    uint64_t reduce_action = 0;
	...
	if (iter < NUM_ITER) {
	    // Do not read coop_csr, is_coop variables from array to reduce mem access overhead
	...
	reduce_start_reg = reduce_configs[reduce_minion_idx + REDUCE_START_REG_IDX + iter * REDUCE_PARAMS * TOTAL_MINIONS];
	reduce_op = reduce_configs[reduce_minion_idx + REDUCE_OP_IDX + iter * REDUCE_PARAMS * TOTAL_MINIONS];
        reduce_num_regs = reduce_configs[reduce_minion_idx + REDUCE_NUM_REGS_IDX + iter * REDUCE_PARAMS * TOTAL_MINIONS];
	reduce_partner = reduce_configs[reduce_minion_idx + REDUCE_PARTNER_IDX + iter * REDUCE_PARAMS * TOTAL_MINIONS];
	reduce_action = reduce_configs[reduce_minion_idx + REDUCE_ACTION_IDX + iter * REDUCE_PARAMS * TOTAL_MINIONS];
	
	// Tensor Reduce
	tensor_reduce(reduce_start_reg, reduce_op, reduce_num_regs, reduce_partner, reduce_action);
    }
