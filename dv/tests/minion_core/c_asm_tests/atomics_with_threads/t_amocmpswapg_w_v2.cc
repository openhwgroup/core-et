...
//#include "common.h"              // :/dv/tests/minion_core/c_asm_test/micro_kernels/common.h
...
// before amocmpswapg_w:
// do a load: to load new value into x1; 
// do a amoorg_w: to load original value of memory [base_addr] into x31;
inline __attribute__((always_inline)) void __cye_before_amocmpswapg_w(uint32_t *operands) {
    __asm__ __volatile__
        (
         "lw x1, 0(%[operands])\n"
...
inline __attribute__((always_inline)) void __cye_after_amocmpswapg_w(uint32_t *result) {
    __asm__ __volatile__
        (
         "amoorg.w x3, x0, (%[base_addr])\n"
         "sw x3, 0(%[result]) \n"
         :
         : [base_addr] "r" (base_addr), [result] "r" (result)
         : "x3", "memory"
...
int main()
{
    ...
    if(hart_id == 0 || hart_id == 2 || hart_id == 4 || hart_id == 6) {
      ...
      for(int i = 0; i < SIZE; i++) {
        __cye_before_amocmpswapg_w(operands + i);
        FENCE;
        __do_amocmpswapg_w();
        FENCE;
        __cye_after_amocmpswapg_w(result + i);
