...
inline __attribute__((always_inline)) void __famoswapl_pi(uint32_t *base_addr, uint32_t *addr_offset, uint32_t *operands, uint32_t *result) {
    __asm__ __volatile__
        (
         "li x11, 0xff\n"
         "mova.m.x x11\n"
         "flwl.ps f0, 0(%[operands])\n"
         "flwl.ps f1, 0(%[addr_offset])\n"
         "addi t0, %[base_addr], 0\n"
         "famoswapl.pi f0,f1(t0)\n"
         "flwl.ps f0, 0(t0)\n"
         "fswl.ps f0, 0(%[result]) \n"
         :
...
int main()
{
    ...
    for(int k = 0; k < 4; k++) {
       __famoswapl_pi(base_addr+k*8, addr_offset, operands+k*8, result+k*8);
       FENCE;
       ...
       // evict result[] data out of L1$ (to L2): for 2 cachelines; 
       FENCE;
       evict_va(false, 0x1, (uint64_t)(void*)(result), 1, 0x40, 0, 0);
       WAIT_CACHEOPS;
       FENCE; 
