...
//               of the Esperanto tensor cooperative load intra-neigh feature
// Note: 
...
int main() {
  ...
  #ifdef POSTSI_PARAM  // rely on +MINION_MASK setting: always 8 active minions within a Neigh, could be any Neigh;
   if (tid == 0) { 
  #else
   if ((tid == 0) && (mid < ncoop_minions) && (nid < ncoop_neighs)) {
     ...
     #ifdef POSTSI_PARAM
      tensor_coop((uint64_t)(0x1<<nid), SET_N_BITS(ncoop_minions), coop_id);
     #else
