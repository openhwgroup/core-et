...
// Preload random data (~1M); but when postsi_ust, its size reduce to 64KB. 
...
#ifdef SELF_CHECK
   selfcheck_ts(reg_stride, start_reg, row_size, nrows, addr,                stride);
#endif
}
...
#ifdef SELF_CHECK
   if ((row_size_1way == 1) && (mid == 3)) {
      wait_for_everyone(); // Pre-check sync

      // No checking need to be done

      wait_for_everyone(); // Post-check sync
   }
#endif
}
...
int main()
{
   ...
   volatile cl_data_t *buffer = &_binary_blob_bin_start;  // note: when postst_ust: its size reduce to 64KB
  ...
  #ifdef POSTSI_PARAM  // rely on +MINION_MASK setting: always 8 active minions within a Neigh, could be any Neigh; (maybe need a checker?) 
   if (tid == 0) {
  #else
   if ((mid < NUM_MINIONS) && (tid == 0)) {
  #endif

    #ifdef POSTSI_UST
      // now buffer[] is only 64KB; so change to code below:
      uint64_t rand_init = et_get_rand_word(0,64);   // this is for buffer[] size 64KB;
    #else
      uint64_t rand_init = et_get_rand_word(0,1024); // this code assumes buffer[] size 1MB
    #endif

      uint64_t addr = (uint64_t)(void*)buffer[rand_init].data[0];

      // load some VPU registers with FLQ instructions from the first array
      init_fp_regs((void*) addr);
#ifdef SELF_CHECK
      post_init_fp_evict((void*) addr);
#endif
      FENCE;
