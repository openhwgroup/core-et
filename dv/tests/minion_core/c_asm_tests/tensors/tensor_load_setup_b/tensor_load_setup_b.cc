...
//////////////////////////////////////////////////////////////////////// 
// 
// this test is a cut down and modified version of vpu test 
// vpu_fma32f1_exhaustive_tlsetupb
// it generates back to back tensor load setup b
//////////////////////////////////////////////////////////////////////// 
#include <stdio.h>
#include <inttypes.h>
#include <stdlib.h>
...
int main() {
...
#ifdef SELF_CHECK
   uint64_t x14_val, x15_val;
   __asm__ __volatile__ ( "add %[X14_VAL], x14, x0\n"
                          "add %[X15_VAL], x15, x0\n"
                          : [X14_VAL] "=r" (x14_val),
                            [X15_VAL] "=r" (x15_val)
   );
   ...
   uint64_t pre_l1scp_na[((L1SCP_NUM_LINES*L1SCP_LINE_SIZE_BYTES)/sizeof(uint64_t))+(64/sizeof(uint64_t))];
   uint64_t pre_vpu_rf_na[((VPURF_NUM_REGS*LANES_PER_VPU_REG*BYTES_PER_LANE)/sizeof(uint64_t))+(64/sizeof(uint64_t))];
   uint64_t post_vpu_rf_na[((VPURF_NUM_REGS*LANES_PER_VPU_REG*BYTES_PER_LANE)/sizeof(uint64_t))+(64/sizeof(uint64_t))];

   uint64_t * pre_l1scp = get_aligned_addr(pre_l1scp_na);
   uint64_t * pre_vpu_rf = get_aligned_addr(pre_vpu_rf_na);
   uint64_t * post_vpu_rf = get_aligned_addr(post_vpu_rf_na);

   uint64_t tfma_tenb_csr_enc, tl_tenb_csr_enc, x31_val;
   tensor_data tfma_info;

   __asm__ __volatile__ ( "add x14, %[X14_VAL], x0\n"
                          "add x15, %[X15_VAL], x0\n"
                          :
                          : [X14_VAL] "r" (x14_val),
                            [X15_VAL] "r" (x15_val)
   ...
   __asm__ __volatile__ (
        ...
        // loop 0 : 
        // <1> x tensor load A    num-lines=16
        // <n> x tensor_load_vpu  num-lines=1      
        "li x19, 0x20\n"               // loop counter
   "LBL_VPU_SEQ_START_0:\n"
      ...
      ///tensor_load_vpu
        ...
        "add x19, x19, -1\n"
#ifdef SELF_CHECK
        "bne x19,  x0, TFMA_LOOP_0\n"
        "add %[TL_ENC], x5, x0\n"
        "add %[X31_VAL], x31, x0\n"
        : [TL_ENC] "=r" (tl_tenb_csr_enc), [X31_VAL] "=r" (x31_val)
   );
   dump_l1_scp(pre_l1scp);
   dump_vpu_rf(pre_vpu_rf);
   __asm__ __volatile__ (
   "TFMA_LOOP_0:\n"
#endif
        "li x6,  0x910001 \n "
        ...
        "bne x19, x0, LBL_VPU_SEQ_START_0\n"
#ifdef SELF_CHECK
        "add %[TFMA_ENC], x6, x0\n"
        : [TFMA_ENC] "=r" (tfma_tenb_csr_enc)
#endif
   );
#ifdef SELF_CHECK
   dump_vpu_rf(post_vpu_rf);
   tfma_info = get_tfma_info(tfma_tenb_csr_enc, tl_tenb_csr_enc, x31_val, pre_vpu_rf, post_vpu_rf, pre_l1scp);
   tfma_selfchecking(&tfma_info);
#endif
   ...
   __asm__ __volatile__ (
        // loop 1 : 
        // <7> x tensor load A    num-lines=1
        // <7> x tensor_load_vpu  num-lines=1      
        "li x19, 0x1\n"               // loop counter
   "LBL_VPU_SEQ_START_1:\n"
   ...
   "TFMA_LOOP_1:\n"
        ...
        "bne x19, x0, LBL_VPU_SEQ_START_1\n"
#ifdef SELF_CHECK
        "add %[TFMA_ENC], x6, x0\n"
        : [TFMA_ENC] "=r" (tfma_tenb_csr_enc)
#endif
   ...
   __asm__ __volatile__ (
        // loop 2 : 
        // <1> x tensor load A    num-lines=1
        // <7> x tensor_load_vpu  num-lines=16      
        "li x19, 0x20\n"               // loop counter
   "LBL_VPU_SEQ_START_2:\n"
   ...
   "TFMA_LOOP_2:\n"
        ...
        "bne x19, x0, LBL_VPU_SEQ_START_2\n"
        "fence\n"
#ifdef SELF_CHECK
        "add %[TFMA_ENC], x6, x0\n"
        : [TFMA_ENC] "=r" (tfma_tenb_csr_enc)
