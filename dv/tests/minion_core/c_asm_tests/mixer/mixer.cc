...
void master_loop(int nshires)
{
    MTRand r = seedRand(1337);

    while ( 1 )
    {
        create_round(r, nshires);
        dispatch_round();
        wait_round();
    }
}
...
int main()
{
  ...
  // Master minion
  if ( shire_id == MASTER_SHIRE && minion_id == 0)
  {
      // master loop should use only shires 0 through MASTER_SHIRE-1
      master_loop(MASTER_SHIRE);
  }
  else 
  {
      slave_loop();
