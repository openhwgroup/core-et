...
inline __attribute__((always_inline)) void __zero_line(uint64_t my_addr) {
    __asm__ __volatile__
        (
         "mov.m.x m0, zero, 0xff\n"
         // store work to local L1
         "fsw.ps f0, (%[my_addr])\n"
...
int main()
{
   ...
   if( ((hart_id & 1) == 0 ) && (minion_id == 0)  ) {
      ...
      // scp address
      // Assertion about x's in scratchpad.
      //base_addr = 
      //   (1U << 31U) + // Scratchpad
      ...
      // l2 address
      //   Except(M): sbl x14,(x9) (enc: 0x10e4b03b) - Cause: 0x7 - TVal: 0x80000000
      //base_addr = 
      ...
      //__zero_line(base_addr); 
      //evict_va(0, 1, base_addr); 
      ...
      for (ii = 0; ii < 512; ++ii) {
         ...
         //byte_addr = buffer + (ii/8);
         //hline_addr = buffer + ((ii/256) * 256);
         byte_addr = base_addr + (ii/8);
         hline_addr = base_addr + ((ii/256) * 256);
         data = 1 << (ii%8);
