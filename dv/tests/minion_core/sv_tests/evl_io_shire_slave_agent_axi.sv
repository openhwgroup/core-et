//--------------------------------------------------------------------------------------------------
//
// Class: evl_io_shire_slave_agent_axi
//
// This class is an extension of evl_stim_agent_axi to define a slave stimulus agent that acts like
// NoC on the IO shire's two L3 master interfaces, etc.
//
class evl_io_shire_slave_agent_axi extends evl_stim_agent_axi;

   `uvm_component_utils(evl_io_shire_slave_agent_axi)
   ...
   int m_to_l3_first_rcv_port;
   int m_to_sys_first_rcv_port;
   int m_main_rbm_first_rcv_port;
   int m_io_pm_first_rcv_port;
// int m_uc_to_l3_first_rcv_port;
// int m_uc_to_sys_first_rcv_port;
   int m_to_l3_last_rcv_port;
   int m_to_sys_last_rcv_port;
   int m_main_rbm_last_rcv_port;
   int m_io_pm_last_rcv_port;
// int m_uc_to_l3_last_rcv_port;
// int m_uc_to_sys_last_rcv_port;

   int m_to_l3_rcv_ports[$];
   int m_to_sys_rcv_ports[$];
   int m_main_rbm_rcv_ports[$];
   int m_io_pm_rcv_ports[$];
...
// int m_uc_sys_loopback_snd_ports[$];
   ...
   virtual function void build_tables();
      ...
      //               -------------------  -----------  -----------  -------------  ----------  -----------  ---------  -----------
      add_bus_req_act( `EVL_AXI_REQ_READ,   I,           AXQ_Local,   AXI_L2_to_L3,  St_Ignore,  Data_MemRd,  RspWData,  BusIdle     );
      add_bus_req_act( `EVL_AXI_REQ_WRITE,  I,           AXQ_Local,   AXI_L2_to_L3,  St_Ignore,  Data_MemWr,  RspAck,    BusIdle     );
      ...
      //               Cmd                  Model State  Qual         Source          Tag Action  Data Action  Bus Rsp    Bus Command
      //               -------------------  -----------  -----------  --------------  ----------  -----------  ---------  -----------
      add_bus_req_act( `EVL_AXI_REQ_READ,   I,           AXQ_Local,   AXI_L3_to_Sys,  St_Ignore,  Data_MemRd,  RspWData,  BusIdle     );
      add_bus_req_act( `EVL_AXI_REQ_READ,   I,           AXQ_Remote,  AXI_L3_to_Sys,  St_Ignore,  Data_MemRd,  RspWData,  BusIdle     );
      add_bus_req_act( `EVL_AXI_REQ_WRITE,  I,           AXQ_Local,   AXI_L3_to_Sys,  St_Ignore,  Data_MemWr,  RspAck,    BusIdle     );
      add_bus_req_act( `EVL_AXI_REQ_WRITE,  I,           AXQ_Remote,  AXI_L3_to_Sys,  St_Ignore,  Data_MemWr,  RspAck,    BusIdle     );
      ...
      if (evl_verif_comp_map::get_verif_comp(SHIRE_CACHE_RTL_AGENT, ~evl_agent_id_t'(0), get_shire_id()) == null) begin
         ...
         //               Cmd                  Model State  Qual                 Source          Tag Action  Data Action  Bus Rsp    Bus Command
         //               -------------------  -----------  -------------------  --------------  ----------  -----------  ---------  -----------
         add_bus_req_act( `EVL_AXI_REQ_WRITE,  I,           AXQ_AtomicLocal,     AXI_L3_to_Sys,  St_Ignore,  Data_None,   RspAck,    BusIdle     );
      end
      else begin
         //
      ...
      //               Cmd                  Model State  Qual               Source         Tag Action  Data Action         Bus Rsp    Bus Command
      //               -------------------  -----------  -----------------  -------------  ----------  ------------------  ---------  --------------
      add_bus_req_act( `EVL_AXI_REQ_READ,   I,           AXQ_Local,         AXI_UC_to_L3,  St_Ignore,  Data_MemRd,         RspWData,  BusIdle        );
      add_bus_req_act( `EVL_AXI_REQ_WRITE,  I,           AXQ_Local,         AXI_UC_to_L3,  St_Ignore,  Data_MemWr,         RspAck,    BusIdle        );
      ...
      add_bus_req_act( `EVL_AXI_REQ_WRITE,  I,           AXQ_AtomicLocal,   AXI_UC_to_L3,  St_Ignore,  Data_FwdAtomicRsp,  RspAck,    BusWriteAtomic );
      ...
      //               Cmd                  Model State  Qual              Source          Tag Action  Data Action         Bus Rsp    Bus Command
      //               -------------------  -----------  ----------------  --------------  ----------  ------------------  ---------  --------------
      add_bus_req_act( `EVL_AXI_REQ_READ,   I,           AXQ_Local,        AXI_UC_to_Sys,  St_Ignore,  Data_MemRd,         RspWData,  BusIdle        );
      add_bus_req_act( `EVL_AXI_REQ_WRITE,  I,           AXQ_Local,        AXI_UC_to_Sys,  St_Ignore,  Data_MemWr,         RspAck,    BusIdle        );
      ...
      add_bus_req_act( `EVL_AXI_REQ_WRITE,  I,           AXQ_AtomicLocal,  AXI_UC_to_Sys,  St_Ignore,  Data_FwdAtomicRsp,  RspAck,    BusWriteAtomic );
...
//    if ((req_port >= m_to_l3_first_rcv_port) && (req_port <= m_to_l3_last_rcv_port)) begin
//       return AXI_L2_to_L3;
//    end
//    if ((req_port >= m_to_sys_first_rcv_port) && (req_port <= m_to_sys_last_rcv_port)) begin
...
//       snd_req_port = snd_req_port - m_to_l3_first_rcv_port;
...
//    end
   ...
   virtual function uvm_analysis_port#(evl_req_desc) get_req_desc_port(input string port_name      = "unknown",
      ...
      if (port_name == "IO_TOL3_AXI") begin
         if ((port_number < 0) || (port_number >= m_to_l3_rcv_ports.size())) begin
            return null;
         end
         return m_snd_rsp_ports[m_to_l3_rcv_ports[port_number]];
      end
      if (port_name == "IO_TOSYS_AXI") begin
         if ((port_number < 0) || (port_number >= m_to_sys_rcv_ports.size())) begin
            return null;
         end
         return m_snd_rsp_ports[m_to_sys_rcv_ports[port_number]];
      end
      if (port_name == "IO_MAIN_NOC_RBM") begin
         if ((port_number < 0) || (port_number >= m_main_rbm_rcv_ports.size())) begin
            return null;
         end
         return m_snd_rsp_ports[m_main_rbm_rcv_ports[port_number]];
      end
      if (port_name == "IO_PM_AXI") begin
         if ((port_number < 0) || (port_number >= m_io_pm_rcv_ports.size())) begin
            return null;
         end
         return m_snd_rsp_ports[m_io_pm_rcv_ports[port_number]];
      end
...
//    end
   ...
   virtual function uvm_analysis_export#(evl_req_desc) get_req_desc_export(input string port_name      = "unknown",
      ...
      if (port_name == "IO_TOL3_AXI") begin
         ...
         return m_obs_req_exports[m_to_l3_rcv_ports[port_number]];
      end
      if (port_name == "IO_TOSYS_AXI") begin
         if ((port_number < 0) || (port_number >= m_to_sys_rcv_ports.size())) begin
            return null;
         end
         return m_obs_req_exports[m_to_sys_rcv_ports[port_number]];
      end
      if (port_name == "IO_MAIN_NOC_RBM") begin
         if ((port_number < 0) || (port_number >= m_main_rbm_rcv_ports.size())) begin
            return null;
         end
         return m_obs_req_exports[m_main_rbm_rcv_ports[port_number]];
      end
      if (port_name == "IO_PM_AXI") begin
         if ((port_number < 0) || (port_number >= m_io_pm_rcv_ports.size())) begin
            return null;
         end
         return m_obs_req_exports[m_io_pm_rcv_ports[port_number]];
      end
...
//    end
   ...
   virtual function void build_phase(uvm_phase phase);
      ...
      //
      // To-L3 slave port connections
      //
      m_to_l3_first_rcv_port = m_snd_rsp_ports.size();
      for (int ii = 0; ii < m_num_l3_ports; ii++) begin
         m_to_l3_rcv_ports.push_back(m_snd_rsp_ports.size());
         m_snd_rsp_ports.push_back(null);
         m_obs_req_exports.push_back(null);
         m_obs_req_fifos.push_back(null);
         m_obs_req_mb_map.push_back(0);
         m_snd_rsp_ports[cur_rcv_port]   = new($sformatf("to_l3_snd_rsp_port_%0d", ii), this);
         m_obs_req_exports[cur_rcv_port] = new($sformatf("to_l3_obs_req_export_%0d", ii), this);
         m_obs_req_fifos[cur_rcv_port]   = new($sformatf("to_l3_obs_req_fifo_%0d", ii), this);
         cur_rcv_port++;
      end
      m_to_l3_last_rcv_port = m_snd_rsp_ports.size() - 1;

      //
      // To-sys slave port connections
      //
      m_to_sys_first_rcv_port = m_snd_rsp_ports.size();
      m_to_sys_rcv_ports.push_back(m_snd_rsp_ports.size());
      m_snd_rsp_ports.push_back(null);
      m_obs_req_exports.push_back(null);
      m_obs_req_fifos.push_back(null);
      m_obs_req_mb_map.push_back(1);
      m_snd_rsp_ports[cur_rcv_port]   = new($sformatf("to_sys_snd_rsp_port"), this);
      m_obs_req_exports[cur_rcv_port] = new($sformatf("to_sys_obs_req_export"), this);
      m_obs_req_fifos[cur_rcv_port]   = new($sformatf("to_sys_obs_req_fifo"), this);
      cur_rcv_port++;
      m_to_sys_last_rcv_port = m_snd_rsp_ports.size() - 1;

      //
      // Main NoC RBM slave port connections
      //
      m_main_rbm_first_rcv_port = m_snd_rsp_ports.size();
      m_main_rbm_rcv_ports.push_back(m_snd_rsp_ports.size());
      m_snd_rsp_ports.push_back(null);
      m_obs_req_exports.push_back(null);
      m_obs_req_fifos.push_back(null);
      m_obs_req_mb_map.push_back(2);
      m_snd_rsp_ports[cur_rcv_port]   = new($sformatf("main_rbm_snd_rsp_port"), this);
      m_obs_req_exports[cur_rcv_port] = new($sformatf("main_rbm_obs_req_export"), this);
      m_obs_req_fifos[cur_rcv_port]   = new($sformatf("main_rbm_obs_req_fifo"), this);
      cur_rcv_port++;
      m_main_rbm_last_rcv_port = m_snd_rsp_ports.size() - 1;

      //
      // PM slave port connections
      //
      m_io_pm_first_rcv_port = m_snd_rsp_ports.size();
      m_io_pm_rcv_ports.push_back(m_snd_rsp_ports.size());
      m_snd_rsp_ports.push_back(null);
      m_obs_req_exports.push_back(null);
      m_obs_req_fifos.push_back(null);
      m_obs_req_mb_map.push_back(2);
      m_snd_rsp_ports[cur_rcv_port]   = new($sformatf("io_pm_snd_rsp_port"), this);
      m_obs_req_exports[cur_rcv_port] = new($sformatf("io_pm_obs_req_export"), this);
      m_obs_req_fifos[cur_rcv_port]   = new($sformatf("io_pm_obs_req_fifo"), this);
      cur_rcv_port++;
      m_io_pm_last_rcv_port = m_snd_rsp_ports.size() - 1;
...
//    cur_snd_port++;
   ...
   // Constructor
   //
   function new(input string name_in = "evl_io_shire_slave_agent_axi", input uvm_component parent_in = null);
      ...
      super.new(name_in, parent_in, MESH_IO_SLAVE_STIM_AGENT);
...
endclass : evl_io_shire_slave_agent_axi
