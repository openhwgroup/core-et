//--------------------------------------------------------------------------------------------------
//
// Class: evl_spio_am_master_agent_axi
//
...
//
class evl_spio_am_master_agent_axi extends evl_stim_agent_axi;

   `uvm_component_utils(evl_spio_am_master_agent_axi)
   ...
   // Constructor
   //
   function new(input string name_in = "evl_spio_am_master_agent_axi", input uvm_component parent_in = null);
      super.new(name_in, parent_in, SPIO_AM_MASTER_STIM_AGENT);
      m_stimulus_agent   = 1'b1;
      m_bus_req_id_limit = 15;
      m_active_limit     = 1;
      m_issue_limit      = 1;
      set_abstract_name("SPIO_AM_MASTER(STIM)");
      add_agent_alias(AXI_MASTER_STIM_AGENT);
      set_active_master();
   endfunction : new

endclass : evl_spio_am_master_agent_axi
