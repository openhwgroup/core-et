...
//
// ULX3S board wrapper for test_frontend.

module test_frontend_ulx3s (
  ...
  test_frontend #(
    .ClkHz(10_000_000),
    .BaudRate(115200),
    // Give the host FTDI TTY time to re-enumerate after programming so the
    // one-shot trace banner is still observable on the board.
    .BootDelayCycles(50_000_000)
  ) u_demo (
    .clk_i    (clk_pll),
    .rst_ni,
    .uart_tx_o(ftdi_rxd),
